...
public class Solver {
    public void Solve()
    {
        ...
        var r = ModLong.Power(10, keta);
        var ans = new ModLong(N) * (ModLong.Power(r.Num, N) - one) / (r - one);
...
public struct ModLong : IEquatable<ModLong>
{
    ...
    public ModLong(BigInteger num)
    {
        Num = num;
    }

    public static ModLong GetInv(BigInteger f)
    {
        return ModIsPrime ? GetInvByFermat(f) : GetInvByEuclid(f);
    }

    // x^n
    public static ModLong Power(BigInteger x, BigInteger n)
    {
        x %= CONST.MOD;
        BigInteger pow = 1;
        while (n > 0)
        ...
        return new ModLong(pow);
    }

    public static ModLong GetInvByFermat(BigInteger f)
    {
        // フェルマーの小定理 a^(-1) = a^(mod-2)
        // Modが素数であることが前提
        return new ModLong(Power(f, CONST.MOD - 2).Num);
    }
    ...
    public static ModLong GetInvByEuclid(BigInteger a)
    {
        // 拡張ユークリッドの互除法
        BigInteger b = CONST.MOD, u = 1, v = 0;
        while (b != 0)
        {
            BigInteger t = a / b;
            a -= t * b;
    ...
    public override bool Equals(object obj)
    {
        return obj is ModLong @int && Num == @int.Num;
    }
    ...
    public static ModLong operator +(ModLong l, ModLong r)
    {
        l.Num += r.Num;
        if (l.Num >= CONST.MOD)
        {
    ...
    public static ModLong operator *(ModLong l, ModLong r)
    {
        return new ModLong(l.Num * r.Num % CONST.MOD);
    }
    ...
    public static ModLong operator /(ModLong l, BigInteger r)
    {
        return l * ((ModLong)r).GetInv();
    }

    public static implicit operator ModLong(BigInteger n)
    {
        n %= CONST.MOD;
        if (n < 0)
        {
