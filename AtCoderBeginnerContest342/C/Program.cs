...
public class Solver {
    public void Solve()
    {
        ...
        var cToD = new Dictionary<char, char>();

        for (int i = 0; i < Q; i++)
        {
            var cd = Rsa();
            var c = cd[0][0]; var d = cd[1][0];

            cToD.TryAdd(c, d);

            // valueがcな要素のvalueをdに変更する
            cToD.Keys
                .Where(key => cToD[key] == c)
                .ToList()
                .ForEach(key => cToD[key] = d);
        }

        foreach (var c in S)
        {
            Console.Write(cToD.GetValueOrDefault(c, c));
        }
        Console.WriteLine();
    }
