...
public class Solver {
    public void Solve()
    {
        ...
        long ans = 0;
        var remainedNToCnt = new long[200001];
        var zeroCnt = 0;
        for (int i = 0; i < N; i++)
        {
            var A = As[i];
            ...
            long remainedN = 0;
            var primeToCnt = PrimeFactors(A)
                .GroupBy(x => x)
                .ToDictionary(x => x.Key, x => x.Count());
            foreach (var (prime, cnt) in primeToCnt)
            {
                if (cnt % 2 == 1)
                {
                    if (remainedN == 0) remainedN = prime;
                    else remainedN *= prime;
                }
            }

            ans += remainedNToCnt[remainedN];
            ans += zeroCnt;
            remainedNToCnt[remainedN]++;
        }
