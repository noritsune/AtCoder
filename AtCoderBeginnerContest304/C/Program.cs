...
public class Solver {
    public void Solve()
    {
        ...
        var manPoss = new List<Vector2>();
        for (int i = 0; i < N; i++)
        {
            var XY = Ria();
            manPoss.Add(new Vector2(XY[0], XY[1]));
        }
        ...
        for (int i = 0; i < N; i++)
        {
            for (int j = i + 1; j < N; j++)
            {
                var dist = manPoss[i].DistanceTo(manPoss[j]);
                if (dist > D) continue;
                uf.Union(i, j);
            }
        }
        ...
        for (int i = 0; i < N; i++)
        {
            var isSick = uf.Same(0, i);
            Console.WriteLine(isSick ? "Yes" : "No");
        }
    }
