...
public class Solver {
    public void Solve()
    {
        ...
        {
            var nextVerts = graph.Vertices[vert];
            foreach (var nextVert in nextVerts)
            {
                uf.Union(vert, nextVert);
            }
        }

        var ngUnionSets = new HashSet<string>();
        foreach (var xy in xys)
        {
            var xRoot = uf.Find(xy.X);
            var yRoot = uf.Find(xy.Y);
            ngUnionSets.Add($"{xRoot},{yRoot}");
            ngUnionSets.Add($"{yRoot},{xRoot}");
        }
        ...
        for (int i = 0; i < Q; i++)
        {
            ...
            var unionSet = $"{uf.Find(p)},{uf.Find(q)}";
            var isOk = !ngUnionSets.Contains(unionSet);
