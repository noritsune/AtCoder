...
public class Solver {
    public void Solve()
    {
        ...
        var N = Rl();
        var stPossX = new List<long>();
        var stPossY = new List<long>();
        for (int i = 0; i < N; i++)
        {
            var pq = Rla();
            stPossX.Add(pq[0]); stPossY.Add(pq[1]);
        }
        var A = Rl();
        var As = Rla();
        var B = Rl();
        ...
        var stIdxToFragIdxX = new long[N];
        var stIdxToFragIdxY = new long[N];
        for (int i = 0; i < N; i++)
        {
            var stPosX = stPossX[i]; var stPosY = stPossY[i];
            stIdxToFragIdxX[i] = LowerBound(As, stPosX, Comparer<long>.Default);
            stIdxToFragIdxY[i] = LowerBound(Bs, stPosY, Comparer<long>.Default);
        }

        var fragIdxToStCnt = new Dictionary<string, long>();
        for (int i = 0; i < N; i++)
        {
            var fragIdx = $"{stIdxToFragIdxX[i]},{stIdxToFragIdxY[i]}";
            fragIdxToStCnt.TryAdd(fragIdx, 0);
            fragIdxToStCnt[fragIdx]++;
        }

        var fragCnt = (A + 1) * (B + 1);
        var stCnts = fragIdxToStCnt.Values.ToArray();
        var stCntMin = stCnts.Length < fragCnt ? 0 : stCnts.Min();
        var stCntMax = stCnts.Max();
        Console.WriteLine(stCntMin + " " + stCntMax);
    }
