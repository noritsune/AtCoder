...
public class Solver {
    public void Solve()
    {
        ...
        var isABCHead = new bool[N];
        for (int i = 0; i < N - 2; i++)
        {
            isABCHead[i] = S[i] == 'A' && S[i + 1] == 'B' && S[i + 2] == 'C';
        }

        var ans = isABCHead.Count(x => x);
        for (int i = 0; i < Q; i++)
        {
            var XC = Rsa();
            var X = int.Parse(XC[0]) - 1; var C = XC[1][0];

            S[X] = C;

            for (int j = Math.Max(0, X - 2); j < Math.Min(N - 2, X + 2); j++)
            {
                var newJudge = S[j] == 'A' && S[j + 1] == 'B' && S[j + 2] == 'C';
                if (isABCHead[j] != newJudge)
                {
                    isABCHead[j] = newJudge;
                    ans += newJudge ? 1 : -1;
                }
            }
