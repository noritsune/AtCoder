...
public class Solver {
    public void Solve()
    {
        ...
        // iについてそれより大きい要素が現れた最大のインデックス
        var segTree = new SegTree<int>(N + 1, Math.Max);
        segTree.Set(Hs[0], 0);

        // Hs[r] より大きなHs[l]のうちlが最大のもの
        var rToL = new int[N];
        for (int r = 1; r < N; r++)
        {
            var H = Hs[r];
            // すでに見た数の中でHより大きいものの中でインデックスが最大のもの
            var largestIdx = segTree.Query(H + 1, N + 1);
            rToL[r] = largestIdx ?? 0;
