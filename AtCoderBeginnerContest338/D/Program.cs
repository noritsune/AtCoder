...
public class Solver {
    public void Solve2()
    {
        ...
        // 橋iを壊したときと橋i+1を壊した時のツアーコストの差分
        var d = new long[N + 1];
        for (int i = 0; i < M - 1; i++)
        {
            var X1 = Xs[i]; var X2 = Xs[i + 1];
            var l = Math.Min(X1, X2); var r = Math.Max(X1, X2);
            var rTurnDist = r - l;
            var lTurnDist = N - rTurnDist;
            d[1] += rTurnDist;
            d[l] += lTurnDist - rTurnDist;
            d[r] += rTurnDist - lTurnDist;
        }
    ...
    public void Solve1()
    {
        ...
        for (int i = 0; i < M; i++)
        {
            rXs[i] = (Xs[i] + N - start    ) % N + 1;
            lXs[i] = (Xs[i] + N - start - 1) % N + 1;
        }
        ...
        for (int i = 0; i < M - 1; i++)
        {
            rDistSum += Math.Abs(rXs[i] - rXs[i + 1]);
            lDistSum += Math.Abs(lXs[i] - lXs[i + 1]);
        }

        Console.WriteLine(Math.Min(rDistSum, lDistSum));
    }
