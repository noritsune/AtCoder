    ...
    public class Solver {
        public void Solve() {
            ...
            for (int i = 1; i <= N; i++)
            {
                ...
                var visitedVerts = new List<int>{ i };
                var q = new Queue<int>();
                q.Enqueue(i);
                visited[i] = true;
                while (q.Count > 0)
                {
                    var now = q.Dequeue();
                    foreach (var next in graph.Vertices[now])
                    {
                ...
                var edgeCnt = visitedVerts.Sum(vert => outEdgeCnt[vert]);
                if (edgeCnt == visitedVerts.Count) continue;
