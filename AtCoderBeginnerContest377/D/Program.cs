...
public class Solver {
    public void Solve()
    {
        ...
        var lToSegIdx = new int?[M + 2];
        var rToSegIdx = new int?[M + 2];
        for (int i = 0; i < N; i++)
        {
            var lr = Ria();
            var (L, R) = (lr[0], lr[1]);
        ...
        var ans = 0;
        var l = 1;
        var coverSegIdxes = new HashSet<int>();
        while (l <= M)
        {
            ...
            while (true)
            {
                ...
                if (lToSegIdx[r].HasValue)
                {
                    coverSegIdxes.Add(lToSegIdx[r].Value);
                }

                if (rToSegIdx[r].HasValue && coverSegIdxes.Remove(rToSegIdx[r].Value))
                {
                    break;
                }
