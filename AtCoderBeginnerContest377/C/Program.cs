...
public class Solver {
    public void Solve()
    {
        ...
        var reachablePoss = new HashSet<string>();
        for (int i = 0; i < M; i++)
        {
            var xy = Ria();
            var (x, y) = (xy[0], xy[1]);

            reachablePoss.Add($"{x},{y}");

            foreach (var offset in offsets)
            {
                var nextX = x + offset.X; var nextY = y + offset.Y;
                if (nextX < 1 || nextX > N || nextY < 1 || nextY > N) continue;
                reachablePoss.Add($"{nextX},{nextY}");
            }
        }

        var unreachableCount = N * N - reachablePoss.Count;
