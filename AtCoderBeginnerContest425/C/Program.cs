...
public class Solver {
    public void Solve()
    {
        ...
        var dualAs = As.Concat(As).ToArray();

        var rui = new long[dualAs.Length + 1];
        for (int i = 0; i < dualAs.Length; i++)
        {
            rui[i + 1] = rui[i] + dualAs[i];
        }
        ...
        for (int _ = 0; _ < Q; _++)
        {
            ...
            switch (query[0])
            {
                case 1: {
                    var c = query[1];
                    headIdx = (headIdx + c + N) % N;
                    break;
                }
                case 2: {
                    var (l, r) = (query[1] - 1, query[2] - 1);
                    var shiftedL = l + headIdx;
                    var shiftedR = r + headIdx;
                    var ans = rui[shiftedR + 1] - rui[shiftedL];
