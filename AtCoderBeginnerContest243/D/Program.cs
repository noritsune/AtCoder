...
namespace AtCoder {
    ...
    public class Solver {
        public void Solve3()
        {
            ...
            for (int i = 1; i < N; i++)
            {
                if (S[i] == 'U' && stack.Count > 0)
                {
                    if (stack.Peek() == 'L' || stack.Peek() == 'R')
                    {
                        stack.Pop();
                        continue;
                    }
            ...
            var summarizedS = stack.ToList();
            summarizedS.Reverse();

            BigInteger currentNode = X;
            foreach (var c in summarizedS) {
                switch (c)
                {
                    case 'U':
                        currentNode /= 2;
        ...
        public void Solve2()
        {
            ...
            var binChars = Convert.ToString(X, 2).ToList();
            for (int i = 0; i < N; i++)
            {
                switch (S[i])
                {
                    case 'U':
                        binChars.RemoveAt(binChars.Count - 1);
                        break;
                    case 'L':
                        binChars.Add('0');
                        break;
            ...
            string binStr = string.Join("", binChars);
            Console.WriteLine(Convert.ToInt64(binStr, 2));
        }
        ...
        public void Solve1()
        {
            ...
            for (int i = N - 1; i >= 0; i--)
            {
                switch (S[i])
                {
                    ...
                    case 'L':
                        ...
                        rOffsetPower = Math.Max(rOffsetPower, 0);
                        rOffsetPower++;
                        break;
                    case 'R':
                        if (rOffsetPower > 0)
                        {
                            rOffsets += (BigInteger)Math.Pow(2, rOffsetPower);
                        }
            ...
            Console.WriteLine((BigInteger)Math.Pow(2, downCnt) * X + rOffsets);
        }
