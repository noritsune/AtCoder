...
namespace AtCoder {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            {
                var XYs = ria();
                poss.Add(new Vector2(XYs[0], XYs[1]));
            }
            string S = rs();
            ...
            for (int i = 0; i < N; i++)
            {
                ...
                if(S[i] == 'R')
                {
                    if(!mins.ContainsKey(y))
                    {
                        mins.Add(y, x);
                    }
                    else
                    {
                        mins[y] = Math.Min(mins[y], x);
                    }
                }
                else
                {
            ...
            for (int i = 0; i < N; i++)
            {
                ...
                if (S[i] == 'R')
                {
                    int right = maxs.ContainsKey(pos.Y) ? maxs[pos.Y] : int.MinValue;
                    isCollision = pos.X <= right;
                }
                else
                {
                    int left  = mins.ContainsKey(pos.Y) ? mins[pos.Y] : int.MaxValue;
                    isCollision = pos.X >= left;
                }
