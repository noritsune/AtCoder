...
namespace AtCoder {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            var appearIndices = new Dictionary<int, List<int>>();
            for (int i = 0; i < aArray.Length; i++)
            {
                int a = aArray[i];
                if (!appearIndices.ContainsKey(a))
                {
                    appearIndices.Add(a, new List<int>{-1});
                }
                appearIndices[a].Add(i + 1);
            }
            ...
            for (int i = 0; i < Q; i++)
            {
                ...
                if (!appearIndices.ContainsKey(x))
                {
                    Console.WriteLine(-1);
                    continue;
                }
                
                var appearIndex = appearIndices[x];
                if (appearIndex == null || k >= appearIndex.Count)
                {
                    Console.WriteLine(-1);
                }
                else
                {
                    Console.WriteLine(appearIndex[k]);
                }
            }
        }
