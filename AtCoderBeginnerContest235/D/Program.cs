...
namespace AtCoder {
    ...
    public class Solver {
        public void Solve3()
        {
            ...
            int iLimit = (int)Math.Ceiling(Math.Sqrt(N));
            
            // i回目までで戻れる数
            var operatedNs = new HashSet<long>[iLimit + 1]; 
            operatedNs[0] = new HashSet<long> { N };

            for (int i = 0; i < operatedNs.Length - 1; i++)
            {
                operatedNs[i + 1] = new HashSet<long>();
                
                foreach (var n in operatedNs[i])
                {
                    var nextNs = new List<long>();
                    if(n % a == 0) nextNs.Add(n / a);

                    var strN = n.ToString();
                    if (strN.Length >= 2 && n.ToString()[1] != '0')
                    {
                        var rotatedN = CalcReverseRotatedNum(n);
                        nextNs.Add(rotatedN);
                    }

                    foreach (var nextN in nextNs.Where(nextN => nextN <= N * 10))
                    {
                        if (nextN == 1)
                        {
        ...
        public void Solve2()
        {
            ...
            // i回目までで作れる数
            var operatedNs = new HashSet<long>[iLimit + 1]; 
            operatedNs[0] = new HashSet<long> { 1 };
            ...
            for (int i = 0; i < operatedNs.Length - 1; i++)
            {
                ...
                foreach (var n in operatedNs[i])
                {
                    ...
                    if (n >= 10 && n % 10 != 0)
                    {
                        var rotatedN = CalcRotatedNum(n);
                        nextNs.Add(rotatedN);
                    }
                    
                    foreach (var nextN in nextNs.Where(nextN => nextN <= N * a))
                    {
                        if (nextN == N)
                        {
                            Console.WriteLine(i + 1);
        ...
        public void Solve1()
        {
            ...
            for (int i = 0; i < dp.GetLength(0) - 1; i++)
            {
                for (int j = 0; j < dp.GetLength(1); j++)
                {
                    ...
                    var nextJs = new List<long>();
                    nextJs.Add(j * a);
                    if (j >= 10 && j % 10 != 0)
                    {
                        var rotatedJ = CalcRotatedNum(j);
                        nextJs.Add(rotatedJ);
                    }
                    
                    foreach (var nextJ in nextJs.Where(nextJ => nextJ <= N))
                    {
                        if (nextJ == N)
                        {
                            Console.WriteLine(i + 1);
        ...
        private static long CalcRotatedNum(long n)
        {
            string strN = n.ToString();
            var rotatedNumStr = strN[^1] + strN[..^1];
            return int.Parse(rotatedNumStr);
        }

        private static long CalcReverseRotatedNum(long n)
        {
            string strN = n.ToString();
            var rotatedNumStr = strN[1..] + strN[0];
            return int.Parse(rotatedNumStr);
        }
