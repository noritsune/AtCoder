...
namespace AtCoder {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            var multipleQIndices = new List<int>[N + 1];
            for (int i = 1; i <= N; i++) multipleQIndices[i] = new List<int>();

            for (int i = 0; i < N; i++)
            {
                var divisors = GetDivisors(Qs[i]);;
                foreach (var divisor in divisors)
                {
                    multipleQIndices[divisor].Add(i);
                }
            }

            int ansMax = int.MinValue;
            for (int pLeftIndex = 0; pLeftIndex < N; pLeftIndex++)
            {
                int ans = 1;
                
                int pLeft = Ps[pLeftIndex];
                int qRightIndex = multipleQIndices[pLeft].First();

                var suretsuP = new List<int>() { pLeft };;
                var suretsuQ = new List<int>() { Qs[qRightIndex] };;
                for (int pRightIndex = pLeftIndex + 1; pRightIndex < N; pRightIndex++)
                {
                    int pRight = Ps[pRightIndex];
                    int qRightIndexNew = UpperBound(multipleQIndices[pRight], qRightIndex, Comparer<int>.Default);
                    if(qRightIndexNew == multipleQIndices[pRight].Count)
                    {
                        break;
                    }
                    
                    qRightIndex = multipleQIndices[pRight][qRightIndexNew];
                    ans++;
                    
                    suretsuP.Add(pRight);
                    suretsuQ.Add(Qs[qRightIndex]);
                }
