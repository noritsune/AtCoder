...
namespace AtCoder {
    ...
    public class Solver {
        public void Solve2()
        {
            ...
            Console.WriteLine(string.Join(" ", As.Where(A => A != removeNum)));
        }
        ...
        public void Solve1()
        {
            ...
            var existNums = As.Distinct();
            var filteredStrs = existNums
                .Select(removeNum => As.Where(a => a != removeNum))
                .Select(filteredAs => string.Join(",", filteredAs))
                .ToList();

            filteredStrs.Sort();
            string ans = filteredStrs.First();
            Console.WriteLine(string.Join(" ", ans.Split(',')));
        }
