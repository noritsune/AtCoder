...
namespace KyoPro {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            long ngPattenCnt = 0;
            var numExistCnt = new Dictionary<int, int>();
            for (int i = 0; i < N; i++)
            {
                int A = As[i];
                if (!numExistCnt.ContainsKey(A))
                {
                    numExistCnt.Add(A, 1);
                    continue; 
                }

                ngPattenCnt += (N - numExistCnt[A] - 1) * numExistCnt[A];

                numExistCnt[A]++;
            }

            long allPatternCnt = nCk(N, 3);
            Console.WriteLine(allPatternCnt - ngPattenCnt);
        }
        ...
        public void SolveTest()
        {
            ...
            for (int i = 0; i < N; i++)
            {
                for (int j = i + 1; j < N; j++)
                {
                    for (int k = j + 1; k < N; k++)
                    {
                        var nums = new List<int>{ As[i], As[j], As[k] };
                        bool isOK = nums.Distinct().Count() == 3;
                        if (isOK)
                        {
                            ans++;
                            continue;
