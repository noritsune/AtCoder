...
namespace KyoPro {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            var numIndices = new List<int>[10];
            for (int i = 0; i < numIndices.Length; i++) numIndices[i] = new List<int>();
            ...
            foreach (var S in Ss)
            {
                for (int i = 0; i < S.Length; i++)
                {
                    ...
                    numIndices[num].Add(i);
                }
            }

            var stopSecondSets = new HashSet<int>[10];
            for (int i = 0; i < stopSecondSets.Length; i++) stopSecondSets[i] = new HashSet<int>();

            for (int i = 0; i < 10; i++)
            {
                for (int j = 0; j < N; j++)
                {
                    var stopSecondSet = stopSecondSets[i];
                    var numIndex = numIndices[i][j];

                    int stopSecond = numIndex;
                    while (stopSecondSet.Contains(stopSecond)) stopSecond += 10;
                    
                    stopSecondSet.Add(stopSecond);
                }
            }

            var maxStopSeconds = stopSecondSets.Select(stopSecondSet => stopSecondSet.Max());
            
            Console.WriteLine(maxStopSeconds.Min());
        }
