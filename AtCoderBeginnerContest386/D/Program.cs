...
public class Solver {
    public void Solve()
    {
        ...
        var BPoss = new List<(int X, int Y)>();
        var WPoss = new List<(int X, int Y)>();
        for (int i = 0; i < M; i++)
        {
            var XYC = Rsa();
            var (X, Y, C) = (int.Parse(XYC[0]) - 1, int.Parse(XYC[1]) - 1, XYC[2][0]);
            if (C == 'B') BPoss.Add((X, Y));
            else WPoss.Add((X, Y));
        }

        var rowToMaxBIdx = new int[N];
        var colToMaxBIdx = new int[N];

        foreach (var (X, Y) in BPoss)
        {
            rowToMaxBIdx[Y] = Math.Max(rowToMaxBIdx[Y], X);
            colToMaxBIdx[X] = Math.Max(colToMaxBIdx[X], Y);
        }

        foreach (var (X, Y) in WPoss)
        {
            if (rowToMaxBIdx[Y] > X || colToMaxBIdx[X] > Y)
            {
