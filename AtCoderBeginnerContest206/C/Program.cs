...
class Sol{
	...
	public void Solve(){
        ...
        Vector2[] indexAndAs = new Vector2[N];
        for (int i = 0; i < N; i++)
        {
            indexAndAs[i] = new Vector2(i, As[i]);
        }

        Array.Sort(indexAndAs, ((a, b) => a._x - b._x));
        Array.Sort(indexAndAs, ((a, b) => a._y - b._y));

        long ans = 0;
        int preA = indexAndAs.First()._y;
        List<Vector2> indexAndAs_sameA = new List<Vector2>();
        for (int i = 0; i < N; i++)
        {
            int A = indexAndAs[i]._y;
            if(A != preA) {
                indexAndAs_sameA.Sort((a, b) => a._x - b._x);

                for (int j = 0; j < indexAndAs_sameA.Count(); j++)
                {
                    int index = indexAndAs_sameA[j]._x;
                    ans += index - (indexAndAs_sameA.Count() - 1 - j);
                }

                indexAndAs_sameA = new List<Vector2>();
            }

            indexAndAs_sameA.Add(indexAndAs[i]);
