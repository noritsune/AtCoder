...
public class Solver {
    public void Solve() {
        var NMP = Rla();
        var N = NMP[0]; var M = NMP[1]; var P = NMP[2];
        var As = Rla().OrderBy(x => x).ToArray();
        var Bs = Rla().OrderBy(x => x).ToArray();

        var ruiseki = new long[M + 1];
        for (int i = 0; i < M; i++)
        {
            ruiseki[i + 1] = ruiseki[i] + Bs[i];
        }

        long ans = 0;
        foreach (var A in As)
        {
            var remainedPrice = P - A;
            var ub = UpperBound(Bs, remainedPrice, Comparer<long>.Default);
            ans += P * (M - ub);
            ans += A * ub;
            ans += ruiseki[ub];
        }
