...
public class Solver {
    public void Solve()
    {
        ...
        var tilts = new char[N];
        tilts[0] = Ps[0] < Ps[1] ? 'U' : 'D';
        for (int i = 1; i < N - 1; i++)
        {
        ...
        tilts[N - 1] = Ps[N - 2] < Ps[N - 1] ? 'U' : 'D';

        var headCnts = new long[N];
        long ans = 0;
        for (int l = 1; l < N - 2; l++)
        {
            if (tilts[l] != '^')
            ...
            // 山はlの位置に見つかったのでそこより左に最初に出現する谷を探す
            var r = l + 1;
            while(r < N && tilts[r] != 'v')
            {
                r++;
            ...
            if (headCnts[l] == 0)
            {
                while (l - headCnt >= 0 && (tilts[l - headCnt] == 'U' || tilts[l - headCnt] == 'v'))
                {
                    headCnt++;
                }
            ...
            // 谷より右にいくつの上りがあるか数える
            long tailCnt = 1;
            while (r + tailCnt < N && (tilts[r + tailCnt] == 'U' || tilts[r + tailCnt] == '^'))
            {
                tailCnt++;
            }
            tailCnt--;
