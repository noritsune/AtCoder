...
namespace util {
    ...
    public class Sol{
        ...
        public void Solve(){
            int N = ri();
            List<int[]> tlrs = new List<int[]>();
            for (int i = 0; i < N; i++) tlrs.Add(ria());
            
            int ans = 0;
            for (int i = 0; i < N - 1; i++)
            {
                int iType  = tlrs[i][0];
                double iLeft  = getFixedLeft( iType, tlrs[i][1]);
                double iRight = getFixedRight(iType, tlrs[i][2]);

                for (int j = i + 1; j < N; j++)
                {
                    int jType  = tlrs[j][0];
                    double jLeft  = getFixedLeft( jType, tlrs[j][1]);
                    double jRight = getFixedRight(jType, tlrs[j][2]);
        ...
        double getFixedLeft(int type, double left) {
            if(type == 3 || type == 4) {
                left += 0.1;
            }
            return left;
        ...
        double getFixedRight(int type, double right) {
            if(type == 2 || type == 4) {
                right -= 0.1;
            }
            return right;
        ...
        public void SolveTest(){
            ...
            int[] cnts = new int[1000000001];
            int ans = 0;
            foreach (int[] tlr in tlrs)
            {
                int type = tlr[0];
                int left = tlr[1];
                int right = tlr[2];
