...
namespace util {
    ...
    public class Sol{
        ...
        public void Solve(){
            ...
            for (int i = 0; i < N; i++) SPoints.Add(new Vector2(ria()));
            for (int i = 0; i < N; i++) TPoints.Add(new Vector2(ria()));

            int radDiv = 4;
            for (int s = 0; s < N; s++)
            {
                Vector2 Spoint = SPoints[s];

                List<Vector2> SPointCopys = new List<Vector2>(SPoints);
                for (int i = 0; i < N; i++) {
                    SPointCopys[i] -= Spoint;
                }

                for (int t = 0; t < N; t++)
                {
                    Vector2 TPoint = TPoints[t];

                    List<Vector2> TPointCopys = new List<Vector2>(TPoints);
                    for (int i = 0; i < N; i++) {
                        TPointCopys[i] -= TPoint;
                    }

                    for (int r = 0; r < radDiv; r++)
                    {
                        List<Vector2> SPointCopysR = new List<Vector2>(SPointCopys);
                        List<Vector2> TPointCopysR = new List<Vector2>(TPointCopys);

                        // 回転
                        double rad = 2 * Math.PI / radDiv * r;
                        for (int i = 0; i < N; i++)
                        {
                            SPointCopysR[i].Rotate(rad);
                            TPointCopysR[i].Rotate(rad);
                        }

                        // 一致チェック
                        List<string> SPointStrs = new List<string>();
                        foreach (Vector2 point in SPointCopysR) SPointStrs.Add(point.ToString());
                        List<string> TPointStrs = new List<string>();
                        foreach (Vector2 point in TPointCopysR) TPointStrs.Add(point.ToString());

                        bool isYes = true;
                        for (int i = 0; i < N; i++)
                        {
                            if(SPointStrs[i] != TPointStrs[i]) {
                                isYes = false;
                                break;
                            }
                        }
    ...
    public class Vector2 {
        ...
        public void Rotate(double rad) {
            ...
            this.x = (int)( tmpX * cos + tmpY + sin);
            this.y = (int)(-tmpX * sin + tmpY + cos);
        }
    }
