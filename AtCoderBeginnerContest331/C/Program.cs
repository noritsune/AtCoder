...
public class Solver {
    public void Solve() {
        ...
        var ascSortedAs = As.OrderBy(x => x).ToArray();

        var AsSum = As.Sum();
        var ruiseki = new long[N + 1];
        for (int i = 0; i < N; i++) {
            ruiseki[i + 1] = ruiseki[i] + ascSortedAs[i];
        }

        var ans = new long[N];
        for (int i = 0; i < N; i++)
        {
            var ub = UpperBound(ascSortedAs, As[i], Comparer<long>.Default);
            ans[i] = AsSum - ruiseki[ub];
        }
