...
public class Solver {
    public void Solve() {
        var NSML = Ria();
        var N = NSML[0]; var S = NSML[1]; var M = NSML[2]; var L = NSML[3];

        var priceSumMin = int.MaxValue;
        for (int sCnt = 0; sCnt <= Math.Ceiling(N / 6.0); sCnt++)
        {
            var remainedCnt = N - sCnt * 6;
            for (int mCnt = 0; mCnt <= Math.Ceiling(remainedCnt / 8.0); mCnt++)
            {
                var remainedCnt2 = remainedCnt - mCnt * 8;
                var lCnt = (int)Math.Ceiling(remainedCnt2 / 12.0);
                var priceSum = sCnt * S + mCnt * M + lCnt * L;
                priceSumMin = Math.Min(priceSumMin, priceSum);
            }
        }

        Console.WriteLine(priceSumMin);
    }
