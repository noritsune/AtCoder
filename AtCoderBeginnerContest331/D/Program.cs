...
public class Solver
{
    ...
    public void Solve2()
    {
        ...
        _N = NQ[0];
        var Q = NQ[1];
        var Ps = new string[_N];
        for (int i = 0; i < _N; i++) Ps[i] = Rs();

        _rui = new long[_N + 1, _N + 1];
        for (int i = 0; i < _N; i++)
        {
            for (int j = 0; j < _N; j++)
            {
                _rui[i + 1, j + 1] = (Ps[i][j] == 'B' ? 1 : 0)
                    + _rui[i, j + 1] + _rui[i + 1, j] - _rui[i, j];
            }
        }
    ...
    long G(int h, int w)
    {
        if (h < _N && w < _N) return _rui[h + 1, w + 1];
        var hTileCnt = h / _N; var wTileCnt = w / _N;
        var hRemain = h % _N; var wRemain = w % _N;
        long sum = 0;
        // 左上
        sum += _rui[_N, _N] * hTileCnt * wTileCnt;
        // 右上
        sum += _rui[_N, wRemain + 1] * hTileCnt;
        // 左下
        sum += _rui[hRemain + 1, _N] * wTileCnt;
        // 右下
        sum += _rui[hRemain + 1, wRemain + 1];
    ...
    public void Solve1()
    {
        ...
        var Ps = new string[N];
        for (int i = 0; i < N; i++) Ps[i] = Rs();

        var rui = new long[N + 1, N + 1];
        for (int i = 0; i < N; i++)
        {
            for (int j = 0; j < N; j++)
            {
                rui[i + 1, j + 1] = (Ps[i][j] == 'B' ? 1 : 0)
                    + rui[i, j + 1] + rui[i + 1, j] - rui[i, j];
            }
        }
        ...
        {
            var ABCD = Ria();
            var rawA = ABCD[0]; var rawB = ABCD[1]; var rawC = ABCD[2]; var rawD = ABCD[3];
            var A = rawA % N; var B = rawB % N; var C = rawC % N; var D = rawD % N;

            long blackSum = 0;
            // 左上
            if (A != 0 && B != 0) blackSum += rui[N, N] - rui[A, N] - rui[N, B] + rui[A, B];
            // 右上
            if (A != 0 && D != N - 1) blackSum += rui[N, D + 1] - rui[A, D + 1];
            // 左下
            if (B != 0 && C != N - 1) blackSum += rui[C + 1, N] - rui[C, B];
            // 右下
            if (C != N - 1 && D != N - 1) blackSum += rui[C + 1, D + 1];

            var h = rawC - rawA + 1; var w = rawD - rawB + 1;
            var hTileCnt = h / N - 1; var wTileCnt = w / N - 1;
            // 上辺
            blackSum += wTileCnt * (rui[N, N] - rui[N, B]);
            // 左辺
            blackSum += hTileCnt * (rui[N, N] - rui[C, N]);
            // 下辺
            blackSum += wTileCnt * rui[N, D];
            // 右辺
            blackSum += hTileCnt * rui[A, N];
            // 中身
            blackSum += hTileCnt * wTileCnt * rui[N, N];

            Console.WriteLine(blackSum);
        }
    }
