...
namespace KyoPro {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            var M = NM[1];
            var Xs = Rla();
            var CYs = new List<long[]>();
            for (int i = 0; i < M; i++) CYs.Add(Rla());
            
            var continueCntToBonus = new long[N + 1];
            foreach (var CY in CYs) continueCntToBonus[CY[0]] = CY[1];

            // i回目のコイントスを終えた時点で表がj回連続していた場合の最大数
            var dp = new long[N + 1, N + 1];
            var iToMax = new long[N + 1];
            for (int i = 1; i <= N; i++)
            { 
                dp[i, 0] = iToMax[i - 1];
                ...
                for (int j = 1; j <= i; j++)
                {
                    dp[i, j] = dp[i - 1, j - 1] + Xs[i - 1] + continueCntToBonus[j];
                    
                    iToMax[i] = Math.Max(iToMax[i], dp[i, j]);
                }
            }

            Console.WriteLine(iToMax[N]);
        }
