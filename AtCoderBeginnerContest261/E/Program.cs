...
namespace KyoPro {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            for (int i = 0; i < N; i++) TAs.Add(Ria());

            // i回目の操作でj桁目がkから何になるか
            var inToOut = new int[N + 1, 18, 2];
            var inToOutRui = new int[N + 1, 18, 2];
            for (int j = 0; j < inToOut.GetLength(1); j++)
            {
                inToOut[0, j, 0] = 0;
                inToOut[0, j, 1] = 1;
                ...
                for (int j = 0; j < inToOut.GetLength(1); j++)
                {
                    ...
                    for (int phi = 0; phi <= 1; phi++)
                    {
                        switch (T)
                        {
                            case 1:
                                // AND
                                inToOut[i, j, phi] = phi & aDigit;
                                inToOutRui[i, j, phi] &= inToOutRui[i, j, phi] & aDigit;
                                break;
                            case 2:
                                // OR
                                inToOut[i, j, phi] = phi | aDigit;
            ...
            var cInBinStr = Convert.ToString(C, 2);
            for (int i = 1; i <= N; i++)
            {
                var xInBinStr = "";
                for (int j = 0; j < cInBinStr.Length; j++)
                {
                    var digit = cInBinStr[j] - '0';
                    xInBinStr += inToOutRui[i - 1, j, digit];
                }

                Console.WriteLine(Convert.ToInt32(xInBinStr, 2));
            }
        }
