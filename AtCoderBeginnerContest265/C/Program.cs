...
namespace KyoPro {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            var offset = new Dictionary<char, Vector2>
            {
                { 'R', new Vector2(1, 0) },
                { 'L', new Vector2(-1, 0) },
                { 'U', new Vector2(0, -1) },
            ...
            while (queue.Count > 0)
            {
                ...
                var nextOffset = offset[grid[currentPos.Y][currentPos.X]];
                var nextPos = currentPos + nextOffset;
                if (nextPos.X < 0 || nextPos.X >= W || nextPos.Y < 0 || nextPos.Y >= H)
                {
                    Console.WriteLine($"{currentPos.Y + 1} {currentPos.X + 1}");
                    return;
                }

                if (hasVisited[nextPos.Y, nextPos.X])
                {
                    Console.WriteLine(-1);
                    return;
                }
                hasVisited[nextPos.Y, nextPos.X] = true;
