...
namespace KyoPro {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            for (int x = 0; x < N; x++)
            {
                var y = rui.BinarySearch(rui[x] + P);
                if(y < 0) continue;

                var z = rui.BinarySearch(rui[y] + Q);
                if(z < 0) continue;

                var w = rui.BinarySearch(rui[z] + R);
                if(w < 0) continue;
        ...
        public static int BinarySearch<T>(IEnumerable<T> enumerable, T target, Func<T, T, long> comparer)
        {
            ...
            while (min <= max) // 範囲内にある限り探し続ける
            {
                ...
                var  compareResult = comparer(target, array.ElementAt(mid));
