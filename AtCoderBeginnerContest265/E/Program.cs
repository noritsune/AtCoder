...
namespace KyoPro {
    ...
    public class Solver
    {
        ...
        public void Solve2()
        {
            ...
            var E = ABCDEF[4];
            var F = ABCDEF[5];
            HashSet<string> obstaclePoss = new HashSet<string>();
            for (int i = 0; i < M; i++)
            {
                var XY = Rla();
                var X = XY[0];
                var Y = XY[1];
                obstaclePoss.Add($"{X},{Y}");
            }
            ...
            dp[0, 0] = 1;
            for (long cntSum = 0; cntSum < N; cntSum++)
            {
                var nextDp = new long[cntSum + 2, cntSum + 2];

                for (long cnt0 = 0; cnt0 <= cntSum; cnt0++)
                {
                    for (long cnt1 = 0; cnt0 + cnt1 <= cntSum; cnt1++)
                    {
                        var cnt2 = cntSum - cnt0 - cnt1;
                        var x = dx[0] * cnt0 + dx[1] * cnt1 + dx[2] * cnt2;
                        var y = dy[0] * cnt0 + dy[1] * cnt1 + dy[2] * cnt2;

                        for (int dIdx = 0; dIdx < 3; dIdx++)
                        {
                            var nextX = x + dx[dIdx];
                            var nextY = y + dy[dIdx];
                            var nextPosStr = $"{nextX},{nextY}";
                            if (obstaclePoss.Contains(nextPosStr)) continue;

                            var nextCnt0 = cnt0 + (dIdx == 0 ? 1 : 0);
                            var nextCnt1 = cnt1 + (dIdx == 1 ? 1 : 0);

                            nextDp[nextCnt0, nextCnt1] += dp[cnt0, cnt1];
                            nextDp[nextCnt0, nextCnt1] %= MOD;
                        }
                    }
                }
        ...
        public void Solve1()
        {
            ...
            var dp = new Dictionary<string, long> { { "0,0", 1 } };
            for (int i = 0; i < N; i++)
            {
                var nextDp = new Dictionary<string, long>();

                foreach (var posStr in dp.Keys)
                {
                    var xy = posStr.Split(',').Select(long.Parse).ToArray();
                    var pos = new Vector2(xy[0], xy[1]);

                    foreach (var offset in offsets)
                    {
                        var nextPos = pos + offset;
                        var nextPosStr = $"{nextPos.X},{nextPos.Y}";
                        if(obstaclePoss.Contains(nextPosStr)) continue;

                        if(!nextDp.ContainsKey(nextPosStr))
                        {
                            nextDp.Add(nextPosStr, 0);
                        }

                        nextDp[nextPosStr] += dp[posStr];
                        nextDp[nextPosStr] %= MOD;
                    }
                }
