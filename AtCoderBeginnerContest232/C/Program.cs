...
namespace AtCoder {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            var toyTakahashi = new Graph<int>(Graph<int>.Type.UndirectedGraph);
            var toyAoki      = new Graph<int>(Graph<int>.Type.UndirectedGraph);
            for (int i = 1; i <= N; i++) toyTakahashi.AddVertex(i);
            for (int i = 1; i <= N; i++) toyAoki.AddVertex(i);
            foreach (var AB in ABs) toyTakahashi.AddEdge(AB.A, AB.B);
            foreach (var CD in CDs) toyAoki.AddEdge(CD.C, CD.D);

            var PsPattern = Enumerable.Range(1, N).Perm().ToArray();
            foreach (var PsEnumerable in PsPattern)
            {
                var Ps = PsEnumerable.ToArray();
                bool isSameToy = true;
                for (int i = 0; i < Ps.Length - 1; i++)
                {
                    for (int j = i + 1; j < Ps.Length; j++)
                    {
                        if (toyTakahashi.Vertices[i + 1].Contains(j + 1) != toyAoki.Vertices[Ps[i]].Contains(Ps[j]))
                        {
                            isSameToy = false;
                            break;
                        }
