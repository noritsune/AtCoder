    ...
    public class Solver {
        public void Solve()
        {
            ...
            while (q.Count > 0)
            {
                ...
                if (currentPos.x >= H || currentPos.y >= W) continue;
                if(grid[currentPos.x, currentPos.y] != '.') continue;
                
                grid[currentPos.x, currentPos.y] = '●';
                
                ans = Math.Max(ans, 1 + currentPos.x + currentPos.y);
                
                q.Enqueue(new Vector2(currentPos.x + 1, currentPos.y    ));
                q.Enqueue(new Vector2(currentPos.x    , currentPos.y + 1));
            }
            
            Console.WriteLine(ans);
            // Console.WriteLine("LoopCnt: {0}", loopCnt);
        }
