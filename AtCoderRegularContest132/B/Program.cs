...
namespace AtCoder {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            int goalIndex = Array.FindIndex(ps, num => num == n);

            int currentIndex = 0;
            bool isRightTurn = true;
            long ans = 0;
            while (ps[(currentIndex + (isRightTurn ? 0 : -1) + n) % n] != 1 || ps[(currentIndex + (isRightTurn ? -1 : 0) + n) % n] != n)
            {
                int forwardShiftCnt = isRightTurn
                    ? goalIndex - currentIndex
                    : (n - goalIndex - 1 + currentIndex) % n;
                
                int backwardShiftCnt = n - 1 - forwardShiftCnt;
                if (forwardShiftCnt == 0 || forwardShiftCnt > backwardShiftCnt)
                {
                ...
                {
                    currentIndex = (currentIndex + (isRightTurn ? forwardShiftCnt : -forwardShiftCnt) + n) % n;
                    ans += forwardShiftCnt;
                }
            }
