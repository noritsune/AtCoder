...
public class Solver {
    ...
    {
        var NHM = Ria();
        var (N, H, M) = (NHM[0], NHM[1], NHM[2]);

        // dp[n, h] = n体目のモンスターまで倒して体力の残量がhの時の最大の残り魔力
        var dp = new long?[N + 1, H + 1];
        dp[0, H] = M;
        ...
        for (int i = 1; i <= N; i++)
        {
            ...
            {
                // 魔法を使わない場合
                if (h >= A && dp[i - 1, h].HasValue)
                {
                    dp[i, h - A] = Math.Max(dp[i, h - A].GetValueOrDefault(), dp[i - 1, h].Value);
                    canKill = true;
                }
                // 魔法を使う場合
                if (dp[i - 1, h].HasValue && dp[i - 1, h] >= B)
                {
                    dp[i, h] = Math.Max(dp[i, h].GetValueOrDefault(), dp[i - 1, h].Value - B);
                    canKill = true;
                }
            }
