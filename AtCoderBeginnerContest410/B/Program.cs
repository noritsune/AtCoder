...
public class Solver {
    public void Solve()
    {
        ...
        var boxToBallCnt = new int[N];
        var ballToBox = new int[Q];
        for (int i = 0; i < Q; i++)
        {
            var X = Xs[i];
            if (X >= 1)
            ...
            {
                var minIndex = boxToBallCnt.ToList().FindIndex(cnt => cnt == boxToBallCnt.Min());
                boxToBallCnt[minIndex]++;
                ballToBox[i] = minIndex + 1;
            }
        }

        Console.WriteLine(string.Join(" ", ballToBox.Select(x => x.ToString())));
    }
