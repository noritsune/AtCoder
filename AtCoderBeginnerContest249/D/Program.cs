...
namespace KyoPro {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            var existCnts = new Dictionary<long, long>();
            foreach (var A in As)
            {
                if(!existCnts.ContainsKey(A))
                {
                    existCnts.Add(A, 0);
                }
                existCnts[A]++;
            ...
            long ans = 0;
            for (long q = 1; q <= AMax; q++)
            {
                for (long r = 1; q * r <= AMax; r++)
                {
                    long p = q * r;
                    if(!existCnts.ContainsKey(p) || !existCnts.ContainsKey(q) || !existCnts.ContainsKey(r)) continue;
                    
                    ans += existCnts[p] * existCnts[q] * existCnts[r];
                }
            }
