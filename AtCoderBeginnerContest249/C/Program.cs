...
namespace KyoPro {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            for (int choiceCnt = 1; choiceCnt <= N; choiceCnt++)
            {
                var patterns = Combination.Enumerate(nums, choiceCnt, false).ToArray();
                foreach (var pattern in patterns)
                {
                    var containCnts = new int[26];
                    foreach (var sIndex in pattern)
                    {
                        var s = Ss[sIndex];
                        foreach (var t in s) containCnts[t - 'a']++;
                    }
                    
                    var kContainsCharCnt = containCnts.Count(containCnt => containCnt == K);
                    ans = Math.Max(ans, kContainsCharCnt);
                }
            }
