    ...
    public class Solver {
        public void Solve()
        {
            ...
            foreach (var A in As)
            {
                ...
                upMin = Math.Min(upMin, cnt - max);
                upMax = Math.Max(upMax, cnt - min);
            }

            Console.WriteLine(upMax - upMin + 1);
        }
        ...
        public void SolveTest()
        {
            ...
            var cnt1Default = As.Count(x => x == 1);
            var cnt1Set = new HashSet<int>{cnt1Default};
            for (int left = 0; left < N; left++)
            {
                int cnt1 = cnt1Default;
                for (int right = left; right < N; right++)
                {
                    cnt1 = As[right] == 0 ? cnt1 + 1 : cnt1 - 1;
                    cnt1Set.Add(cnt1);
                }
            }
 
            Console.WriteLine(cnt1Set.Count);
        }
