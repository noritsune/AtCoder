...
public class Solver {
    ...
    {
        var WHT = Ria();
        var W = WHT[0]; var H = WHT[1]; var T = WHT[2];
        var sxsy = Ria();
        var sx = sxsy[0]; var sy = sxsy[1];
        var txty = Ria();
        var tx = txty[0]; var ty = txty[1];

        // 反射先にあると仮定できるtの位置群
        var virtualTs = new List<Vector2>();
        for (int r = tx; r <=  T + W; r += W) virtualTs.Add(new Vector2(r, ty));
        for (int l = tx; l >= -T - W; l -= W) virtualTs.Add(new Vector2(l, ty));
        for (int u = ty; u <=  T + H; u += H) virtualTs.Add(new Vector2(tx, u));
        for (int d = ty; d >= -T - H; d -= H) virtualTs.Add(new Vector2(tx, d));

        // sxから丁度距離Tにあるものを数える
        var sPos = new Vector2(sx, sy);
        var ans = virtualTs.Count(vT => Math.Abs(sPos.DistanceTo(vT) - T) < 1);
    ...
    public void Solve1() {
        ...
        // 時刻iに向きjで座標(k,l)にいる場合の数
        var dp = new long[T + 1, 4, W + 1, H + 1];
        var prevPatterns = new List<(int dir, int x, int y)>();
        for (int j = 0; j < 4; j++)
        {
            dp[0, j, sx, sy] = 1;
            prevPatterns.Add((j, sx, sy));
        }
        for (int i = 1; i <= T; i++)
        {
            var patterns = new List<(int dir, int x, int y)>();

            foreach (var prevPat in prevPatterns)
            {
                var (dir, x, y) = prevPat;
                var offset = offsets[dir];
                // TODO: 反射処理を書く
                var nextX = x + offset.x; var nextY = y + offset.y;
                var nextDir = dir;

                dp[i, nextDir, nextX, nextY] += dp[i - 1, dir, x, y];
            }
