...
public class Solver {
    ...
    {
        var XYAB = Rla();
        var strInit = (BigInteger)XYAB[0];
        var strLimit = (BigInteger)XYAB[1];
        var strMul = (BigInteger)XYAB[2];
        var strAdd = (BigInteger)XYAB[3];

        BigInteger exp = 0;
        BigInteger str = strInit;
        while(str * strMul <= str + strAdd && str * strMul < strLimit){
            str *= strMul;
            exp++;
        }

        exp += (strLimit - str - 1) / strAdd;
