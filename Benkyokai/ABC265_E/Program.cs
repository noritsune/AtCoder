...
public class Solver {
    ...
    public void Solve()
    {
        ...
        var ABCDEF = Ria();
        var A = ABCDEF[0]; var B = ABCDEF[1]; var C = ABCDEF[2];
        var D = ABCDEF[3]; var E = ABCDEF[4]; var F = ABCDEF[5];
        var offsets = new[]
        {
            new Vector2(A, B),
            new Vector2(C, D),
        ...
        var obstaclePosSet = new HashSet<string>();
        for (int i = 0; i < M; i++)
        {
            var XY = Ria();
            var posStr = string.Join(",", XY);
            obstaclePosSet.Add(posStr);
        }
        ...
        for (int i = 0; i < N; i++)
        {
            var newDp = new Dictionary<string, long>();
            foreach (var kv in dp)
            {
                var prevPos = kv.Key;
                var prevCnt = kv.Value;
                var prevX = int.Parse(prevPos.Split(',')[0]);
                var prevY = int.Parse(prevPos.Split(',')[1]);
                foreach (var offset in offsets)
                {
                    var nextX = prevX + offset.X;
                    var nextY = prevY + offset.Y;
                    var nextPos = $"{nextX},{nextY}";
                    if (obstaclePosSet.Contains(nextPos)) continue;

                    var nextCnt = newDp.GetValueOrDefault(nextPos);
                    newDp[nextPos] = (prevCnt + nextCnt) % CONST.MOD;
                }
            }
            dp = newDp;
        }

        var ans = dp.Values.Aggregate(0L, (acc, cur) => (acc + cur) % CONST.MOD);
