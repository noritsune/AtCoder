...
public class Solver {
    public void Solve2()
    {
        ...
        var crossSizeToCnt = new int[Math.Min(H, W) + 2];
        for (int y = 0; y < H; y++)
        {
            for (int x = 0; x < W; x++)
            {
                var crossSize = CalcCrossSize(grid, y, x);
                crossSizeToCnt[crossSize]++;
            }
        }

        Console.WriteLine(string.Join(" ", crossSizeToCnt.Skip(2)));
    }

    // そのマスを中心とした×字のサイズを計算する
    int CalcCrossSize(List<string> grid, int centerY, int centerX)
    {
        if (!IsCrossOrigin(grid, centerY, centerX)) return 0;
        ...
        while (true)
        {
            ...
            if (top < 0 || bottom >= grid.Count || left < 0 || right >= grid[0].Length) break;
    ...
    // そのマスを中心とした×字が存在するかどうかを判定する
    bool IsCrossOrigin(List<string> grid, int centerY, int centerX)
    {
        var crossOrigin = new []
        {
            "#.#",
        ...
        for (int dY = 0; dY < 3; dY++)
        {
            for (int dX = 0; dX < 3; dX++)
            {
                var y = centerY + dY - 1;
                var x = centerX + dX - 1;
                if (y < 0 || y >= grid.Count || x < 0 || x >= grid[0].Length) return false;
                if (crossOrigin[dY][dX] != grid[y][x]) return false;
            }
        }
    ...
    public void Solve1()
    {
        ...
        var isUsed = new bool[H][];
        for (int h = 0; h < H; h++)
        {
            isUsed[h] = new bool[W];
        }

        var sizeToCrossCnt = new int[Math.Min(H, W) + 1];
        for (int cI = 1; cI < H - 1; cI++)
        {
            for (int cJ = 1; cJ < W - 1; cJ++)
            {
                var sizeMax = 0;
                for (int d = 1; d <= Math.Max(H, W); d++)
                {
                    ...
                    if (left < 0 || right >= W || top < 0 || bottom >= H) break;
                    ...
                    isUsed[top]   [left]  = true;
                    isUsed[top]   [right] = true;
                    isUsed[bottom][left]  = true;
                    isUsed[bottom][right] = true;

                    sizeMax = Math.Max(sizeMax, d);
                }
                sizeToCrossCnt[sizeMax]++;
            }
        }

        Console.WriteLine(string.Join(" ", sizeToCrossCnt.Skip(1)));
    }
