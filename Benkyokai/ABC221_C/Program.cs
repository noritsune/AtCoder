...
public class Solver {
    public void Solve() {
        ...
        var allDigits = N.ToString().Select(x => int.Parse(x.ToString())).ToArray();
        var patterns = BitFullSearch(allDigits.Length);
        var mulMax = long.MinValue;
        foreach (var pattern in patterns)
        {
            var digits1 = new List<int>();
            var digits2 = new List<int>();
            for (int i = 0; i < pattern.Length; i++)
            {
                var isDigit1 = pattern[i];
                if (isDigit1) digits1.Add(allDigits[i]);
                else digits2.Add(allDigits[i]);
            }

            if (digits1.Count == 0 || digits2.Count == 0) continue;

            var descSortedDigits1 = digits1.OrderByDescending(x => x).ToArray();
            var descSortedDigits2 = digits2.OrderByDescending(x => x).ToArray();
            if (descSortedDigits1.First() == 0 || descSortedDigits2.First() == 0) continue;

            var num1 = long.Parse(string.Join("", descSortedDigits1));
            var num2 = long.Parse(string.Join("", descSortedDigits2));
            var mul = num1 * num2;
            mulMax = Math.Max(mulMax, mul);
        }

        Console.WriteLine(mulMax);
    }
