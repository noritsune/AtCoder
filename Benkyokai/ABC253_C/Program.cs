...
public class Solver {
    public void Solve()
    {
        ...
        var mulSet = new MultiSet<int>();
        var numToCnt = new Dictionary<int, int>();
        for (int i = 0; i < Q; i++)
        {
            var query = Ria();
            switch (query[0])
            {
                ...
                case 2:
                {
                    var x = query[1];
                    var c = query[2];
                    var existCnt = numToCnt.GetValueOrDefault(x, 0);
                    var removeCnt = Math.Min(existCnt, c);
                    for (int j = 0; j < removeCnt; j++)
                    {
                        mulSet.Remove(x);
                    }

                    if (numToCnt.ContainsKey(x))
                    {
                        numToCnt[x] -= removeCnt;
                    }
                ...
                case 3:
                {
                    var min = mulSet.ElementAt(0);
                    var max = mulSet.ElementAt(mulSet.Count() - 1);
                    var ans = max - min;
