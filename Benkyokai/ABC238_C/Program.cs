...
public class Solver {
    public void Solve()
    {
        ...
        if (N <= 9)
        {
            Console.WriteLine(Enumerable.Range(1, (int)N).Sum());
        }

        var digitLimit = N.ToString().Length;
        BigInteger ans = 45;
        for (int digit = 2; digit <= digitLimit; digit++)
        {
            long from = (long)Math.Pow(10, digit - 1) - 1;
            var to = digit < digitLimit
                ? (long)Math.Pow(10, digit) - 1
                : N;
            BigInteger n = to - from;
            ans += n * n;
            ans %= CONST.MOD;
            ans *= 499122177;
            ans += n;
