...
public class Solver {
    public void Solve()
    {
        ...
        var dp = new long[chokudai.Length + 1, S.Length + 1];
        for (int i = 0; i <= S.Length; i++)
        {
            dp[0, i] = 1;
        }

        for (int i = 1; i <= chokudai.Length; i++)
        {
            for (int j = 1; j <= S.Length; j++)
            {
                if (chokudai[i - 1] == S[j - 1])
                {
                    dp[i, j] = (dp[i - 1, j - 1] + dp[i, j - 1]) % CONST.MOD;
                }
                else
                {
                    dp[i, j] = dp[i, j - 1];
        ...
        Console.WriteLine(dp[chokudai.Length, S.Length]);
    }
