...
public class Solver
{
    ...
    public void Solve() {
        ...
        var isCheckedI = new bool[N + 1];
        for (int i = 0; i < N - M + 1; i++)
        {
            if (!isCheckedI[i] && IsGoodI(i, SChars))
            {
                isCheckedI[i] = true;
                q.Enqueue(i);
            }
        ...
        while (q.Any())
        {
            ...
            for (int j = -M; j < M; j++)
            {
                ...
                if (!isCheckedI[iJ] && IsGoodI(iJ, SChars))
                {
                    isCheckedI[iJ] = true;
                    q.Enqueue(iJ);
                }
        ...
        Console.WriteLine(SChars.All(c => c == '#') ? "Yes" : "No");
    }

    bool IsGoodI(int i, char[] chars)
    {
        if (i < 0) return false;

        for (int j = 0; j < _T.Length; j++)
        {
            if (i + j >= chars.Length) return false;
            if (chars[i + j] == _T[j] || chars[i + j] == '#') continue;
