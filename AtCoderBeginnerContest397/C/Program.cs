...
public class Solver {
    public void Solve()
    {
        ...
        var nToCntAfter = As.GroupBy(x => x).ToDictionary(x => x.Key, x => x.Count());
        var nToCntBefore = new Dictionary<int, int>();
        var keyCntSumMax = 0;
        foreach (var A in As)
        {
            nToCntBefore.TryAdd(A, 0);
            nToCntBefore[A]++;

            nToCntAfter[A]--;
            if (nToCntAfter[A] == 0) nToCntAfter.Remove(A);

            var keyCntSum = nToCntBefore.Count + nToCntAfter.Count;
            keyCntSumMax = Math.Max(keyCntSumMax, keyCntSum);
        }

        Console.WriteLine(keyCntSumMax);
    }
