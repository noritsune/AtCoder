...
public class NumWithIdx : IComparable
{
    public int Num;
    public int Idx;
    ...
    public int CompareTo(object? obj)
    {
        ...
        if (obj is NumWithIdx other)
        {
            return Num.CompareTo(other.Num);
        }
        throw new ArgumentException("object is not NumWithIdx");
    }
}
...
public class Solver {
    public void Solve()
    {
        ...
        var As = new NumWithIdx[N];
        var AsSet = new MultiSet<NumWithIdx>();
        for (int i = 0; i < N; i++)
        {
            As[i] = new NumWithIdx(AsTmp[i], i);
            AsSet.Insert(As[i]);
        }
        ...
        for (int i = 0; i < N; i++)
        {
            ...
            var A = As[i];
            var lb = AsSet.LowerBound(new NumWithIdx(A.Num * 2, -1));

            if (lb == AsSet.Count()) continue;
