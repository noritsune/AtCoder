...
public class Solver {
    public void Solve()
    {
        ...
        long credit = 0;
        var nonZeroCnt = 0;
        var zeroIndexToCnt = new int[N];
        for (int i = 0; i < N; i++)
        {
            var payFromCredit = Math.Min(i, nonZeroCnt);
            credit -= payFromCredit;
            As[i] += payFromCredit;

            if (As[i] > 0) nonZeroCnt++;

            var payFromA = Math.Min(As[i], N - i - 1);
            As[i] -= payFromA;
            credit += payFromA;

            nonZeroCnt -= zeroIndexToCnt[i];
            zeroIndexToCnt[i + payFromA]++;
        }
