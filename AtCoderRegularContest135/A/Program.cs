...
namespace AtCoder {
    ...
    public class NumAndParents
    {
        public long num;
        public List<long> parents;

        public NumAndParents(long num, List<long> parents)
        {
            this.num = num;
    ...
    public class Solver {
        ...
        private Dictionary<long, long> multiHistory = new Dictionary<long, long>();

        long dfs(long multi, long num)
        {
            if(multiHistory.ContainsKey(num))
            {
                multi *= multiHistory[num];
                multi %= MOD;
                return multi;
            }
            ...
            {
                multi *= num;
                if(multi >= MOD) multi %= MOD;
                return multi;
            }
            
            decimal halfN = (decimal)num / 2;
            long floor = dfs(multi, (long)Math.Floor(halfN));
            long ceil = dfs(multi, (long)Math.Ceiling(halfN));
            
            multiHistory[num] = floor * ceil;
        ...
        public void Solve2()
        {
            ...
            while (q.Count > 0)
            {
                ...
                foreach (var primeFactor in primeFactors)
                {
                    ...
                    decimal half = (decimal)primeFactor / 2;
                    q.Enqueue((long)Math.Floor(half));
                    q.Enqueue((long)Math.Ceiling(half));
                }
        ...
        public void Solve1()
        {
            ...
            while (q.Count > 0)
            {
                ...
                decimal halfN = (decimal)n / 2;
                q.Enqueue((long)Math.Floor(halfN));
                q.Enqueue((long)Math.Ceiling(halfN));
            }
