...
namespace AtCoder {
    ...
    public class Solver {
        public void Solve2()
        {
            ...
            for (int i = 0; i < N; i++)
            {
                As[i + 2] = Ss[i] - As[i + 1] - As[i];
            }

            long c1 = long.MaxValue;
            long c2 = long.MaxValue;
            long c3 = long.MaxValue;
        ...
        public void Solve1()
        {
            ...
            {
                decimal minHalf = (decimal)Math.Min(Ss[0], Ss[1]) / 2;
                ans[1] = (int)Math.Ceiling(minHalf);
                ans[2] = (int)Math.Floor(minHalf);
                for (int i = 3; i < N + 2; i++)
                {
                    int num = Ss[i - 2] - Ss[i - 3] + ans[i - 3];
                    if (num < 0)
                    {
                        Console.WriteLine("No");
                        return;
