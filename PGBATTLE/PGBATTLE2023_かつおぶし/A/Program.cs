...
public class Solver {
    public void Solve() {
        var ABCDLR = Ria();
        var A = ABCDLR[0]; var B = ABCDLR[1]; var C = ABCDLR[2]; var D = ABCDLR[3]; var L = ABCDLR[4]; var R = ABCDLR[5];

        // 3次元方程式の解を複数求める
        // Ax^3 + Bx^2 + Cx + D = 0
        var anss = SolveCubicEquation(A, B, C, D);
        var gosa = 0.00001d;
        var ansInLR = anss.First(x => L - gosa <= x && x <= R + gosa);
        Console.WriteLine(ansInLR);
    }

    double[] SolveCubicEquation(double a, double b, double c, double d){
        var anss = new double[3];
        ...
        if(D < 0.0){//three real solutions
            ...
            anss[0] = 2.0*Math.Sqrt(-p/3.0)*Math.Cos(theta/3.0)-A/3.0;
            anss[1] = 2.0*Math.Sqrt(-p/3.0)*Math.Cos((theta+2.0*Math.PI)/3.0)-A/3.0;
            anss[2] = 2.0*Math.Sqrt(-p/3.0)*Math.Cos((theta+4.0*Math.PI)/3.0)-A/3.0;
        }else{//single real solution and two imaginary solutions(c.c)
            double u = CubeRoot(-q*0.5+Math.Sqrt(D)),v = CubeRoot(-q*0.5-Math.Sqrt(D));
            anss[0] = u+v-A/3.0;
            anss[1] = -0.5*(u+v)-A/3.0;
            anss[2] = -0.5*(u+v)-A/3.0;
        }
