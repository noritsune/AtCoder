...
public class Solver {
    ...
    public void Solve1()
    {
        ...
        var smallSideCntSb = Enumerable.Repeat('1', N).ToArray();
        var bigSideCntSb = Enumerable.Repeat('0', N).ToArray();
        for (int i = 0; i < N - 1; i++)
        {
            smallSideCntSb[i] = '0';
            bigSideCntSb[i] = '1';

            var smallSideCntInBinStr = new string(smallSideCntSb);
            var bigSideCntInBinStr = new string(bigSideCntSb);
            if (smallSideCntInBinStr != XInBinStr && bigSideCntInBinStr != XInBinStr) continue;
