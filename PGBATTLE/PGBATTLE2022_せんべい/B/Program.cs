...
namespace KyoPro {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            var weightAscSortedFoods = As.Select((A, i) => (A, i)).OrderBy(x => x.A).ToArray();

            var senbeis = weightAscSortedFoods.Skip(N).Take(N);

            var ascSortedSenbeiIdxes = senbeis.Select(x => x.i + 1).OrderBy(x => x).ToArray();

            Console.WriteLine(string.Join("\n", ascSortedSenbeiIdxes));
        }
