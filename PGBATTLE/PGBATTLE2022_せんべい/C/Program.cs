...
namespace KyoPro {
    ...
    public class Solver {
        public void Solve2()
        {
            ...
            var canDelete = new bool[N];

            var l = 0;
            while(l < N)
            {
                l = Dfs(N, As, l, canDelete);
            }

            var remainedAs = As.Where((A, i) => !canDelete[i]).ToArray();
            var remainedAsSum = remainedAs.Sum();

            Console.WriteLine(remainedAsSum);
        }

        int Dfs(long N, long[] As, int l, bool[] canDelete)
        {
            if (l == N - 1)
            {
                if (As[l] == 1)
        ...
        public void Solve1()
        {
            ...
            {
                if(As[left] != 1) continue;

                canDelete[left] = true;
                var right = left + 1;

                while (right < N && As[right] == As[right - 1] + 1)
                {
                    canDelete[right] = true;
                    right++;
                }
