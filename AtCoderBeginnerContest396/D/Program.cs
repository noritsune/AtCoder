...
public class Solver
{
    ...
    public void Solve()
    {
        ...
        graph = new Graph<int>(GraphType.Undirected);
        for (int i = 0; i < N; i++)
        {
            graph.AddVertex(i + 1);
        }
        ...
        {
            var uvw = Rla();
            var (u, v, w) = ((int)uvw[0], (int)uvw[1], uvw[2]);
            graph.AddEdge(u, v);
            ws[u, v] = w;
            ws[v, u] = w;
        }
        ...
        visited[1] = true;
        Dfs(1, 0, new List<int>() {1});

        Console.WriteLine(minXOr);
    }

    void Dfs(int v, long xor, List<int> path)
    {
        if (v == N)
        {
        ...
        foreach (var to in graph.Vertices[v])
        {
            ...
            visited[to] = true;
            var nextXor = xor ^ ws[v, to];
            var nextPath = new List<int>(path) { to };
            Dfs(to, nextXor, nextPath);
            visited[to] = false;
        }
    }
    ...
    static long Rb(){return long.Parse(Console.ReadLine() ?? string.Empty);}
    ...
    static long[] Rba(char sep=' '){return Array.ConvertAll(Console.ReadLine().Split(sep),long.Parse);}
