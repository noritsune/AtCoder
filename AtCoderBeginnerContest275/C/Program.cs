...
namespace KyoPro {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            for (int i = 0; i < 9; i++) grid[i] = Rs();

            var pawnPoss = new List<Vector2>();
            for (int r = 0; r < 9; r++)
            {
                for (int c = 0; c < 9; c++)
                {
                    ...
                    pawnPoss.Add(new Vector2(r, c));
                }
            }

            long squareCnt4By = 0;
            for (int aIdx = 0; aIdx < pawnPoss.Count; aIdx++)
            {
                for (int bIdx = 0; bIdx < pawnPoss.Count; bIdx++)
                {
                    for (int cIdx = 0; cIdx < pawnPoss.Count; cIdx++)
                    {
                        for (int dIdx = 0; dIdx < pawnPoss.Count; dIdx++)
                        {
                            if(aIdx == bIdx || aIdx == cIdx || aIdx == dIdx) continue;

                            var posA = pawnPoss[aIdx];
                            var posB = pawnPoss[bIdx];
                            var posC = pawnPoss[cIdx];
                            var posD = pawnPoss[dIdx];

                            var edgeDists = new[]
                            {
                                posA.DistanceToSquared(posB),
                                posB.DistanceToSquared(posC),
                                posC.DistanceToSquared(posD),
                                posD.DistanceToSquared(posA),
                            };

                            if(edgeDists.Any(x => x != edgeDists[0])) continue;

                            var dots = new[]
                            {
                                (posA - posB).Dot(posC - posB),
                                (posB - posC).Dot(posD - posC),
                                (posC - posD).Dot(posA - posD),
                                (posD - posA).Dot(posB - posA),
                            };
                            if(dots.Any(x => x != 0)) continue;
            ...
            long squareCnt = squareCnt4By / 8;
            Console.WriteLine(squareCnt);
        }
    ...
    public class Vector2 {
        ...
        public long DistanceToSquared(Vector2 other)
        {
            var diffX = X - other.X;
            var diffY = Y - other.Y;
