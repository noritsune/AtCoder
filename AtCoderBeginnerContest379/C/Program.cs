...
public class Solver {
    public void Solve()
    {
        ...
        var xAndAs = Xs.Zip(As, (x, a) => (x, a)).OrderBy(x => x.x).ToList();

        var prevX = xAndAs[0].x;
        if (prevX != 1)
        {
            Console.WriteLine(-1);
            return;
        }
        var remainedA = xAndAs[0].a - 1;
        long ans = 0;
        foreach (var (x, a) in xAndAs.Skip(1))
        {
            var dist = x - prevX - 1;
            if (remainedA < dist)
            {
