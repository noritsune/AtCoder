...
namespace KyoPro {
    ...
    public class Solver {
        public void Solve()
        {
            var N = Ri();
            var sxsytxty = Rla();
            var startPos = new Vector2(sxsytxty[0], sxsytxty[1]);
            var targetPos = new Vector2(sxsytxty[2], sxsytxty[3]);
            var circles = new List<(Vector2 center, long radius)>();
            for (int i = 0; i < N; i++)
            {
                var xyr = Ria();
                circles.Add((new Vector2(xyr[0], xyr[1]), xyr[2]));
            }
            
            var uf = new UnionFind(N);
            for (int aIdx = 0; aIdx < N; aIdx++)
            {
                for (int bIdx = aIdx + 1; bIdx < N; bIdx++)
                {
                    if(aIdx == bIdx) continue;
                    
                    var (centerA, rA) = circles[aIdx];
                    var (centerB, rB) = circles[bIdx];
                    
                    var distSqr = centerA.DistanceToSquared(centerB);

                    var radDiffSqr = (rA - rB) * (rA - rB);
                    var radSumSqr  = (rA + rB) * (rA + rB);
                    if (distSqr < radDiffSqr || distSqr > radSumSqr) continue;
                    
                    uf.Union(aIdx, bIdx);
                }
            }

            int startCircleIdx  = -1;
            int targetCircleIdx = -1;
            for (int i = 0; i < N; i++)
            {
                var (center, radius) = circles[i];

                var distToSqrStart  = center.DistanceToSquared(startPos);
                var distToSqrTarget = center.DistanceToSquared(targetPos);
                
                if (distToSqrStart  == radius * radius) startCircleIdx  = i;
                if (distToSqrTarget == radius * radius) targetCircleIdx = i;
            }

            bool canReach = uf.Same(startCircleIdx, targetCircleIdx);
    ...
    public class Vector2 {
        ...
        public long DistanceToSquared(Vector2 other) => (X - other.X) * (X - other.X) + (Y - other.Y) * (Y - other.Y);
        public double DistanceTo(Vector2 other) => Math.Sqrt(DistanceToSquared(other));
