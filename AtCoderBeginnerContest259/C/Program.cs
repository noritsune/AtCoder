...
namespace KyoPro {
    ...
    public class CharRepeatCnt
    {
        public char C;
        public int Cnt;
        
        public CharRepeatCnt(char c, int cnt)
        {
            C = c;
            Cnt = cnt;
        }
    ...
    public class Solver {
        public void Solve()
        {
            ...
            var SCharRepeatCntList = ConvertCharRepeatCntList(S);
            var TCharRepeatCntList = ConvertCharRepeatCntList(T);

            if(SCharRepeatCntList.Count != TCharRepeatCntList.Count)
            {
                Console.WriteLine("No");
                return;
            }

            for (int i = 0; i < SCharRepeatCntList.Count; i++)
            {
                var SCharRepeatCnt = SCharRepeatCntList[i];
                var TCharRepeatCnt = TCharRepeatCntList[i];
                
                if(SCharRepeatCnt.C != TCharRepeatCnt.C
                || SCharRepeatCnt.Cnt > TCharRepeatCnt.Cnt
                || SCharRepeatCnt.Cnt < TCharRepeatCnt.Cnt && SCharRepeatCnt.Cnt < 2)
                {
                    Console.WriteLine("No");
                    return;
                }
        ...
        List<CharRepeatCnt> ConvertCharRepeatCntList(string str)
        {
            var charRepeatCntList = new List<CharRepeatCnt> { new CharRepeatCnt(str[0], 1) };
            for (int i = 1; i < str.Length; i++)
            {
                var lastElement = charRepeatCntList.Last();
                if (str[i] == lastElement.C)
                {
                    lastElement.Increment();
                }
                else
                {
                    charRepeatCntList.Add(new CharRepeatCnt(str[i], 1));
                }
            }
            
            return charRepeatCntList;
        }
