...
namespace KyoPro {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            var dInRad = d * Math.PI / 180;
            var rotatedA = a * Math.Cos(dInRad) - b * Math.Sin(dInRad);
            var rotatedB = a * Math.Sin(dInRad) + b * Math.Cos(dInRad);
            
            Console.WriteLine(rotatedA + " " + rotatedB);
        }
