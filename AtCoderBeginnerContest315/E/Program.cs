...
public class Solver {
    public void Solve()
    {
        ...
        var graphReverse = new Graph<int>(GraphType.Directed);
        for (int i = 1; i <= N; i++)
        {
            graph.AddVertex(i);
            graphReverse.AddVertex(i);
        }
        for (int i = 0; i < N; i++)
        {
            var CPs = Ria();
            var Ps = CPs.Skip(1).ToArray();
            foreach (var P in Ps)
            {
                graph.AddEdge(P, i + 1);
                graphReverse.AddEdge(i + 1, P);
            }
        }
        ...
        while (q.Any())
        {
            ...
            visited2[v] = true;
            targetNodes.Add(v);
            foreach (var neighbor in graphReverse.Vertices[v])
            {
                if (!visited2[neighbor])
                    q.Enqueue(neighbor);
            }
        }
        targetNodes.Remove(1);

        var ans = stack
            .Where(v => targetNodes.Contains(v))
            .ToArray();
        Console.WriteLine(string.Join(" ", ans));
    }


    void TopologicalSortUtil(Graph<int> graph, int v, bool[] visited, Stack<int> stack)
    {
        visited[v] = true;

        foreach (var neighbor in graph.Vertices[v])
        {
            if (!visited[neighbor])
