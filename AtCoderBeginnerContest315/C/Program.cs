...
public class Solver {
    ...
    {
        var N = Ri();
        var FSs = new List<(long F, long S)>();
        for (int i = 0; i < N; i++)
        {
            var FS = Rla();
            FSs.Add((FS[0], FS[1]));
        ...
        var sortedFSs = FSs.OrderByDescending(tuple => tuple.S).ToArray();
        var firstOne = sortedFSs[0];
        var sameScore = long.MinValue;
        for (int i = 1; i < N; i++)
        {
            var current = sortedFSs[i];
            if (current.F != firstOne.F)
            {
                var sum = firstOne.S + current.S;
                Console.WriteLine(Math.Max(sum, sameScore));
                return;
            }

            sameScore = Math.Max(sameScore, firstOne.S + current.S / 2);
        }

        Console.WriteLine(sameScore);
    }
