...
public class Solver
{
    ...
    public void Solve() {
        ...
        isDeleted = new bool[H, W];

        var deleteCnt = Dfs(grid, 0, 0, 0, H - 1, W - 1);
        var ans = H * W - deleteCnt;
        Console.WriteLine(ans);
    }

    // 消した枚数を返す
    long Dfs(char[][] grid, long deleteCntSum, int t, int l, int b, int r)
    {
        long deleteCnt = 0;
        var nextT = t; var nextL = l; var nextB = b; var nextR = r;
        ...
        if (r - l >= 1)
        {
            for (; nextT <= nextB; nextT++)
            {
                bool isBreak = false;
                for (int w = l; w <= r; w++)
                {
                    if (grid[nextT][w] == target) continue;
                    isBreak = true;
                    break;
                }
                if (isBreak) break;
                ...
                for (int w = l; w <= r; w++)
                {
                    if (isDeleted[nextT, w]) continue;
                    isDeleted[nextT, w] = true;
                    deleteCnt++;
                }
            }
        }
        ...
        if (b - t >= 1)
        {
            for (; nextL <= nextR; nextL++)
            {
                bool isBreak = false;
                for (int h = t; h <= b; h++)
                {
                    if (grid[h][nextL] == target) continue;
                    isBreak = true;
                    break;
                }
                if (isBreak) break;
                ...
                for (int h = t; h <= b; h++)
                {
                    if (isDeleted[h, nextL]) continue;
                    isDeleted[h, nextL] = true;
                    deleteCnt++;
                }
            }
        }
        ...
        if (r - l >= 1)
        {
            for (; nextB >= nextT; nextB--)
            {
                bool isBreak = false;
                for (int w = l; w <= r; w++)
                {
                    if (grid[nextB][w] == target) continue;
                    isBreak = true;
                    break;
                }
                if (isBreak) break;
                ...
                for (int w = l; w <= r; w++)
                {
                    if (isDeleted[nextB, w]) continue;
                    isDeleted[nextB, w] = true;
                    deleteCnt++;
                }
            }
        }
        ...
        if (b - t >= 1)
        {
            for (; nextR >= nextL; nextR--)
            {
                bool isBreak = false;
                for (int h = t; h <= b; h++)
                {
                    if (grid[h][nextR] == target) continue;
                    isBreak = true;
                    break;
                }
                if (isBreak) break;
                ...
                for (int h = t; h <= b; h++)
                {
                    if (isDeleted[h, nextR]) continue;
                    isDeleted[h, nextR] = true;
                    deleteCnt++;
                }
            }
        }
        ...
        if (nextT <= nextB && nextL <= nextR && deleteCnt > 0)
        {
            deleteCntSum = Dfs(grid, deleteCntSum, nextT, nextL, nextB, nextR);
        }
