...
public class Solver {
    public void Solve()
    {
        ...
        // dp[i][j] = Csのi個目までを見た時に長さjの文字列の個数
        var dp = new long[27, K + 1];
        dp[0, 0] = 1;

        var combCal = new CombCalculator(1000);
        for (int i = 0; i < 26; i++)
        {
            // j: これまで何文字使ったか
            for (int j = 0; j <= K; j++)
            {
                var limit = Math.Min(Cs[i], K - j);
                // k: i文字目を使う個数
                for (int k = 0; k <= limit; k++)
                {
                    var comb = combCal.Calc(j + k, k);
                    dp[i + 1, j + k] += dp[i, j] * comb % CONST.MOD;
                    dp[i + 1, j + k] %= CONST.MOD;
                }
            }
        }
...
public static class Combination {
    // CombCntをmemoる
    static Dictionary<(long n, long r), long> _memo = new ();
