    ...
    public class Solver {
        public void Solve2()
        {
            ...
            for (int i = 0; i < N; i++)
            {
                ...
                lastA = stack.Pop();
                var tmpAs = new List<int> { lastA };
                while (stack.Count > 0 && tmpAs.Last() == stack.Peek())
                {
                    tmpAs.Add(stack.Pop());
                }

                sameACnt = tmpAs.Count;

                foreach (var tmpA in tmpAs) stack.Push(tmpA);
        ...
        public void Solve1()
        {
            ...
            for (int i = 0; i < N; i++)
            {
                ...
                Console.WriteLine(stack.Count + sameAs.Count);
            }
        }
