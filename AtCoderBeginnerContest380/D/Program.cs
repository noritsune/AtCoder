...
public class Solver {
    public void Solve()
    {
        ...
        var anss = new List<char>();
        foreach (var K in Ks)
        {
            var cycleCnt = (K - 1) / S.Length;
            var idxInCycle = (K - 1) % S.Length;

            // cycleCntを2進数表記した時にビットが立っている個数を数える
            var c = S[(int)idxInCycle];
            if (PopCount(cycleCnt) % 2 == 0)
            {
                anss.Add(c);
            }
            else
