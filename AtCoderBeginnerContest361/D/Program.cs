...
public class Solver {
    public void Solve()
    {
        ...
        var q = new Queue<(string s, int cost, int emptyIdx)>();
        q.Enqueue((S, 0, N));
        var minCost = int.MaxValue;
        var memo = new HashSet<string>();
        while (q.Any())
        {
            var (s, cost, emptyIdx) = q.Dequeue();
            ...
            for (int i = 0; i < s.Length - 1; i++)
            {
                ...
                // i番目とi+1番目を空いているところに移動させる
                var next = s.ToCharArray();
               next[emptyIdx] = next[i]; next[emptyIdx + 1] = next[i + 1];
               next[i] = '.'; next[i + 1] = '.';

               var nextS = new string(next);
               if (!memo.Add(nextS)) continue;
                q.Enqueue((nextS, cost + 1, i));
            }
        }
