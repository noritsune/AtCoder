...
namespace AtCoder {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            {
                var xy = ria();
                poss.Add(new Vector2(xy[0], xy[1]));
            }

            bool[] isNgs = new bool[N];
            for (int i = 0; i < N - 1; i++)
            {
                for (int j = i + 1; j < N; j++)
                {
                    ...
                    var posABottomPos = new Vector2(posA.x, posA.y - 1);
                    var posBLeftPos   = new Vector2(posB.x - 1, posB.y);
                    bool isPosBUpperThanBottom = posBLeftPos.x * (long)posABottomPos.y < posBLeftPos.y * (long)posABottomPos.x;
                    
                    var posALeftPos = new Vector2(posA.x - 1, posA.y);
                    var posBBottomPos   = new Vector2(posB.x, posB.y - 1);
                    bool isPosBLowerThanLeft = posBBottomPos.x * (long)posALeftPos.y > posBBottomPos.y * (long)posALeftPos.x;

                    if (isPosBUpperThanBottom && isPosBLowerThanLeft)
                    {
                        isNgs[i] = true;
                        break;
                    }
            ...
            Console.WriteLine(isNgs.Count(isNg => !isNg));
        }
