...
namespace AtCoder {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            var frontAndBackTrains = new List<int[]>();
            for (int i = 0; i < N + 1; i++) frontAndBackTrains.Add(new []{-1, -1});
            ...
            for (int i = 0; i < Q; i++)
            {
                ...
                switch (q[0])
                {
                    case 1:
                        ...
                        frontAndBackTrains[x][1] = y;
                        frontAndBackTrains[y][0] = x;
                        break;
                    case 2:
                        x = q[1];
                        y = q[2];

                        frontAndBackTrains[x][1] = -1;
                        frontAndBackTrains[y][0] = -1;
                        break;
                    case 3:
                        x = q[1];

                        var indicesFrontThanX = new List<int>();
                        int frontSideIndex = frontAndBackTrains[x][0];
                        while (frontSideIndex != -1)
                        {
                            indicesFrontThanX.Add(frontSideIndex);
                            frontSideIndex = frontAndBackTrains[frontSideIndex][0];
                        }
                        indicesFrontThanX.Reverse();
                        
                        var indicesBackThanX = new List<int>();
                        int backSideIndex = frontAndBackTrains[x][1];
                        while (backSideIndex != -1)
                        {
                            indicesBackThanX.Add(backSideIndex);
                            backSideIndex = frontAndBackTrains[backSideIndex][1];
                        }

                        var indicesSum = indicesFrontThanX.Count + indicesBackThanX.Count + 1;
                        
                        var frontIndicesStr = string.Join(' ', indicesFrontThanX) + " ";
                        if (frontIndicesStr == " ") frontIndicesStr = "";
                        var backIndicesStr  = " " + string.Join(' ', indicesBackThanX);
                        if (backIndicesStr == " ") backIndicesStr = "";
                        
                        Console.WriteLine($"{indicesSum} {frontIndicesStr}{x}{backIndicesStr}");
                        
                        break;
                    default:
                        throw new Exception("想定外の入力です");
                }
            }
        }
