...
class Sol{
    ...
    int G1MinusG2(int num) {
        int g1 = SortByEachPlaceNumSize(num, false);
        int g2 = SortByEachPlaceNumSize(num, true);
        return g1 - g2;
    }

    int SortByEachPlaceNumSize(int num, bool isAsc) {
        List<char> sortedNumChars = num.ToString().ToList();

        if(isAsc) {
            sortedNumChars.Sort();
        } else {
            sortedNumChars.Sort((a, b) => b - a);
        }

        int sortedNum = 0;
        for (int i = 0; i < sortedNumChars.Count; i++)
        {
            sortedNum += (int)(Math.Pow(10, sortedNumChars.Count - 1 - i) * Char.GetNumericValue(sortedNumChars[i]));
        }
