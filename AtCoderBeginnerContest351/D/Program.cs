...
public class Solver {
    public void Solve2()
    {
        ...
        var xOffsets = new[] { -1, 0, 1, 0 };
        var yOffsets = new[] { 0, -1, 0, 1 };
        ...
        for (int i = 0; i < H; i++)
        {
            for (int j = 0; j < W; j++)
            {
                ...
                for (int k = 0; k < 4; k++)
                {
                    var nx = j + xOffsets[k]; var ny = i + yOffsets[k];
        ...
        // i行j列の要素番号はi * W + j
        var g = new Graph<int>(GraphType.Directed);
        for (int i = 0; i < H * W; i++) g.AddVertex(i);
        for (int y = 0; y < H; y++)
        {
            for (int x = 0; x < W; x++)
            {
                var v = y * W + x;
                if (grid[y][x] == '#' || grid[y][x] == 'x') continue;

                for (int k = 0; k < 4; k++)
                {
                    var nx = x + xOffsets[k]; var ny = y + yOffsets[k];
                    if (nx < 0 || nx >= W || ny < 0 || ny >= H) continue;
                    if (grid[ny][nx] == '#') continue;
        ...
        // 連結成分の最大の個数を数える
        var maxCount = 1;
        var visited2 = new bool[H * W];
        var q = new Queue<int>();
        for (int i = 0; i < H * W; i++)
        {
            if (!g.Vertices[i].Any() || visited2[i]) continue;
            ...
            {
                var v = q.Dequeue();
                foreach (var to in g.Vertices[v])
                {
                    if (visited.Contains(to)) continue;
                    visited.Add(to);
                    visited2[to] = true;
                    count++;
                    if (g.Vertices[to].Any())
                    {
                        q.Enqueue(to);
                    }
                }
    ...
    public void Solve1()
    {
        ...
        var maxScores = new int[H, W];
        var maxScoreMax = 0;
        for (int i = 0; i < H; i++)
        {
            for (int j = 0; j < W; j++)
            {
                var q = new Queue<(int x, int y, int score)>();
                q.Enqueue((i, j, 1));
                while (q.Any())
                {
                    var (x, y, score) = q.Dequeue();
                    maxScores[i, j] = score;
                    maxScoreMax = Math.Max(maxScoreMax, score);
                    ...
                    for (int k = 0; k < 4; k++)
                    {
                        ...
                        if (nx < 0 || nx >= W || ny < 0 || ny >= H) continue;
                        if (grid[ny][nx] == '#') continue;
                        if (maxScores[ny, nx] >= score) continue;
                        q.Enqueue((nx, ny, score + 1));
                    }
                }
            }
        }

        Console.WriteLine(maxScoreMax);
    }
