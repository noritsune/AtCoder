...
public class Solver {
    public void Solve()
    {
        var NQ = Ria();
        var (N, Q) = (NQ[0], NQ[1]);
        var cols = Rla().OrderBy(x => x).ToArray();

        var rows = new long[cols.Last()];
        var currentColIdx = 0;
        for (int i = 0; i < rows.Length; i++)
        {
            while (i > cols[currentColIdx] - 1)
            {
                // 現在の列の値がiより大きい場合は、次の列に移動
                currentColIdx++;
            }

            rows[i] = cols.Length - currentColIdx;
        }

        var rui = new long[rows.Length + 1];
        for (int i = 0; i < rows.Length; i++)
        {
            rui[i + 1] = rui[i] + rows[i];
        }
