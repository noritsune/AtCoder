...
public class Solver {
    ...
    public void Solve() {
        ...
        for (int i = 1; i <= 12; i++)
        {
            var repunit = long.Parse(string.Join("", Enumerable.Repeat(1, i)));
            repunits.Add(repunit);
        }

        var validNumSet = new HashSet<long>();
        foreach (var r1 in repunits)
        {
            foreach (var r2 in repunits)
            {
                foreach (var r3 in repunits)
                {
                    validNumSet.Add(r1 + r2 + r3);
                }
            }
        }

        var validNumList = validNumSet.ToList();
        validNumList.Sort();
        Console.WriteLine(validNumList[N - 1]);
    }
