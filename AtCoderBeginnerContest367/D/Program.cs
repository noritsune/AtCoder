...
public struct ValWithCnt
{
    public long Value;
    public long Count;

    public ValWithCnt(long value)
    {
        Value = value;
        Count = value % CONST.MOD == 0 ? 1 : 0;
    }

    public static ValWithCnt SumValue(ValWithCnt a, ValWithCnt b)
    {
        return new ValWithCnt
        {
            Value = a.Value + b.Value,
            Count = (a.Value + b.Value + CONST.MOD) % CONST.MOD == 0 ? 1 : 0
        };
    }

    public static ValWithCnt SumCount(ValWithCnt a, ValWithCnt b)
    {
        return new ValWithCnt
        {
            Value = 0,
...
public class Solver {
    ...
    {
        var NM = Rla();
        var N = (int)NM[0]; var M = NM[1];
        var As = Rla().Select(x => x % M).ToArray();

        CONST.MOD = M;

        var modMs = new LazySegTree<ValWithCnt>(N, ValWithCnt.SumCount, ValWithCnt.SumValue);
        ...
        for (int i = 0; i < N; i++)
        {
            sum += As[i];
            sum %= M;
            modMs.ApplyRange(i, i + 1, new ValWithCnt(sum));
        }

        long ans = 0;
        for (int i = 0; i < N; i++)
        {
            var sumOfZero = modMs.Query(i, i + N - 1).Value.Count;
            ans += sumOfZero;

            var diff = new ValWithCnt(-As[i]);
            modMs.ApplyRange(0, N, diff);
        }
