...
public class Solver {
    public void Solve()
    {
        ...
        var anss = new List<List<int>>();
        Dfs(new List<int>(), 0, Rs, anss, K);

        Console.WriteLine(string.Join("\n", anss.Select(x => string.Join(" ", x))));
    }


    void Dfs(List<int> ans, int sum, int[] Rs, List<List<int>> anss, int K)
    {
        // 要素iがRsi以下で長さがRsと同じ配列をanssに集めてくる
        if(ans.Count == Rs.Length)
        {
            if (sum % K == 0)
            {
                anss.Add(ans);
        ...
        for (int i = 1; i <= R; i++)
        {
            var nextAns = new List<int>(ans);
            nextAns.Add(i);
            Dfs(nextAns, sum + i, Rs, anss, K);
        }
    }
