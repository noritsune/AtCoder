...
namespace KyoPro {
    ...
    public class Solver {
        public void Solve2()
        {
            ...
            for (int i = 0; i < H1; i++) matA.Add(Ria());
            var H2W2 = Ria();
            var H2 = H2W2[0];
            var W2 = H2W2[1];
            var matB = new List<int[]>();
            for (int i = 0; i < H2; i++) matB.Add(Ria());
            ...
            var rowUsePattern = BitFullSearch(H1).ToArray();
            var colUsePattern = BitFullSearch(W1).ToArray();
            foreach (var isUseRow in rowUsePattern)
            {
                foreach (var isUseCol in colUsePattern)
                {
                    var reducedMatA = new List<List<int>>();

                    for (int rowIdx = 0; rowIdx < H1; rowIdx++)
                    {
                        if(!isUseRow[rowIdx]) continue;

                        var row = new List<int>();
                        for (int colIdx = 0; colIdx < W1; colIdx++)
                        {
                            if(!isUseCol[colIdx]) continue;
                            
                            row.Add(matA[rowIdx][colIdx]);
                        }
                        
                        reducedMatA.Add(row);
                    }

                    if (SequenceEqual(reducedMatA, matB))
                    {
        ...
        public void Solve1()
        {
            ...
            bool isRowOk = BitFullSearch(H1)
                .Select(isUse =>
                {
                    var fixedMatARows = Enumerable.Range(0, H1)
                        .Where(i => isUse[i])
                        .Select(i => matA[i])
                        .ToArray();

                    if (fixedMatARows.Count() != H2) return false;

                    return fixedMatARows
                        .Select((rowA, i) => IsAlmostSame(rowA, matB[i]))
                        .Any(isSame => isSame);
                })
                .Any(isOK => isOK);
            
            bool isColOk = BitFullSearch(W1)
                .Select(isUse =>
                {
                    var fixedMatACols = Enumerable.Range(0, W1)
                        .Where(i => isUse[i])
                        .Select(col => Enumerable.Range(0, H1).Select(row => matA[row][col]).ToArray())
                        .ToArray();

                    if (fixedMatACols.Count() != W2) return false;

                    return fixedMatACols
                        .Select((rowA, col) =>
                        {
                            var matBCols = Enumerable.Range(0, H2).Select(row => matB[row][col]).ToArray();
                            return IsAlmostSame(rowA, matBCols);
                        })
                        .Any(isSame => isSame);
                })
                .Any(isOK => isOK);

            Console.WriteLine(isRowOk && isColOk ? "Yes" : "No");
        }

        bool IsAlmostSame(int[] arr1, int[] arr2)
        {
            return BitFullSearch(arr1.Length)
                .Select(isUse => arr1.Where((n, i) => isUse[i]).ToArray())
                .Any(fixedArr1 => fixedArr1.SequenceEqual(arr2));
        }
        
        bool SequenceEqual(List<List<int>> arr1, IReadOnlyList<int[]> arr2)
        {
            if(arr1.Count != arr2.Count) return false;

            for (int i = 0; i < arr1.Count(); i++)
            {
                if(!arr1[i].SequenceEqual(arr2[i])) return false;
