...
namespace KyoPro {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            var isBlack = new bool[15, 15];
            for (int padding = 0; padding < 8; padding += 2)
            {
                DrawRectBorder(isBlack, padding);
            }

            Console.WriteLine(isBlack[R, C] ? "black" : "white");
        }

        void DrawRectBorder(bool[,] isBlack, int padding)
        {
            int begin = padding;
            int end = 14 - padding;
            for (int i = begin; i <= end; i++)
            {
                isBlack[begin, i] = true;
                isBlack[i, begin] = true;
                isBlack[end, i] = true;
                isBlack[i, end] = true;
            }
        }
