...
public class Solver {
    public void Solve()
    {
        ...
        lrs.Sort((a, b) => a.l - b.l);

        long ans = 0;
        var multiSet = new MultiSet<int>();
        foreach (var (l, r) in lrs)
        {
            var lb = multiSet.LowerBound(l);
            if (lb >= 0) ans += multiSet.Count() - lb;
            multiSet.Insert(r);
        }
...
/// </summary>
public class LazySegTree<T> where T : class
{
    /// <summary>
    /// 二分木を配列で表現したもの
    /// 葉の数がN個なら2N-1個のノードがある
    ...
    T? ExecFunc(Func<T, T, T> func, T? a, T? b)
    {
        if (a != null && b != null) return func(a, b);
        if (a != null) return a;
        if (b != null) return b;
        return null;
    }
