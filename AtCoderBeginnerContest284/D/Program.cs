...
namespace KyoPro {
    ...
    public class Solver {
        public void Solve2()
        {
            ...
            var LIMIT = 10000000;
            var primesUnderLimit = PrimesUnder(LIMIT);

            for (int _ = 0; _ < T; _++)
            {
                var N = Rl();

                HandleQuery(primesUnderLimit, N);
            }
        }

        void HandleQuery(List<long> primesUnderLimit, long N)
        {
            foreach (var q in primesUnderLimit)
            {
                if (N % q != 0) continue;

                long remained = N / q;
                if (!IsSquare(remained)) continue;

                long p = (long)Math.Sqrt(N / q);
                Console.WriteLine($"{p} {q}");
                return;
            }

            foreach (var p in primesUnderLimit)
            {
                var pp = p * p;
                if (N % pp != 0) continue;
        ...
        public void Solve1()
        {
            ...
            for (int _ = 0; _ < T; _++)
            {
                ...
                var primeFactors = PrimeFactors(N).ToArray();

                var p = primeFactors.First();
                var q = primeFactors.Last();
                if (primeFactors.Count(n => n == p) == 1)
                {
                    (p, q) = (q, p);
                }
