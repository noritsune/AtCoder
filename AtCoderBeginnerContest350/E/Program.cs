...
{
    double A, X, Y;
    Dictionary<double, double> _memo = new();

    public void Solve()
    {
        var NAXY = Rla();
        var N = NAXY[0]; A = NAXY[1]; X = NAXY[2]; Y = NAXY[3];
    ...
    double F(double n)
    {
        ...
        var res1 = X + F(Math.Floor(n / A));
        var res2 = Y * 1.2 + Enumerable.Range(2, 5).Sum(i => F(Math.Floor(n / i))) / 5.0;
