...
public class Solver {
    public void Solve()
    {
        ...
        {
            var UVW = Ria();
            var U = UVW[0] - 1; var V = UVW[1] - 1; var W = UVW[2];

            var costUV = U == 0 ? W + As[V] + As[0] : W + As[V];
            var costVU = V == 0 ? W + As[U] + As[0] : W + As[U];
            dijkstra.Add(U, V, costUV);
            dijkstra.Add(V, U, costVU);
        }

        var minCosts = dijkstra.GetMinCost(0);
        Console.WriteLine(string.Join(" ", minCosts.Skip(1)));
    }
