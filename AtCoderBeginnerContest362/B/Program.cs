...
public class Solver {
    public void Solve()
    {
        ...
        var posB = new Vector2(BxBx[0], BxBx[1]);
        var posC = new Vector2(CxCy[0], CxCy[1]);

        // 直角三角形か
        var isRightTriangle = IsRightTriangle(posA, posB, posC);
        Console.WriteLine(isRightTriangle ? "Yes" : "No");
    }

    bool IsRightTriangle(Vector2 posA, Vector2 posB, Vector2 posC)
    {
        var ab = posA.DistanceTo(posB);
        var bc = posB.DistanceTo(posC);
        var ca = posC.DistanceTo(posA);

        var sides = new double[] { ab, bc, ca };
        Array.Sort(sides);
