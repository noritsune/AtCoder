...
{
    public const long MOD = 1000000000;
}
...
public class Solver {
    public void Solve()
    {
        ...
        var lazySegTree = new LazySegTree<long>(N + 1, (a, b) => (a + b) % CONST.MOD, (a, b) => a);

        for (int i = 0; i <= N; i++)
        {
            if (i < K)
            {
                lazySegTree.ApplyRange(i, i + 1, 1);
            }
            else
            {
                var sum = lazySegTree.Query(i - K, i);
                lazySegTree.ApplyRange(i, i + 1, sum.Value);
            }
        }

        Console.WriteLine(lazySegTree.Query(N, N + 1).Value);
    }
