    ...
    public class Solver {
        public void Solve()
        {
            ...
            for (int i = 0; i < Q; i++)
            {
                ...
                Console.WriteLine(F(S, t, k - 1));
            }
        }

        char F(string S, long t, long k)
        {
            if(t == 0) return S[(int)k];
            if(k == 0) return ABCToNextABC(S[0], t);
            
            return ABCToNextABC(F(S, t - 1, k / 2), k % 2 + 1);
        }
        ...
        public void SolveTest()
        {
            ...
            var oldStr = "AB";
            Console.WriteLine(string.Join("", oldStr.ToArray()));
            for (int i = 0; i < N; i++)
            {
                string str = oldStr.Aggregate("", (current, c) => current + SFunc(c));
        ...
        private char ABCToNextABC(char c, long offset)
        {
            int cNum = char.ToUpper(c) - 'A';
            return (char)('A' + (cNum + offset) % 3);
        }  
