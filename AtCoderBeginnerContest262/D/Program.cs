...
namespace KyoPro {
    ...
    public class Solver {
        ...
        public void Solve()
        {
            ...
            long ans = 0;
            for (int i = 1; i <= N; i++) // いくつまで選べるか
            {
                var dp = new long[N + 1, i + 1, i];
                dp[0, 0, 0] = 1;
                
                for (int j = 0; j < N; j++) // 先頭からいくつ目までを見ているか
                {
                    for (int k = 0; k <= i; k++) // いくつ選ぶか
                    {
                        for (int l = 0; l < i; l++) // あまりはいくらか
                        {
                            dp[j + 1, k, l] += dp[j, k, l];
                            dp[j + 1, k, l] %= MOD;

                            if (k == i) continue; // これ以上選択できないため
                            
                            // Ajを選ぶ場合
                            var nextMod = (l + As[j]) % i;
                            dp[j + 1, k + 1, nextMod] += dp[j, k, l];
                            dp[j + 1, k + 1, nextMod] %= MOD;
                        }
                    }
                }
        ...
        public void SolveTest()
        {
            ...
            {
                var patterns = Combination.Enumerate(nums, cnt, withRepetition:false).ToArray();
                foreach (var pattern in patterns)
                {
                    var sum = pattern.Select(idx => As[idx]).Sum();
                    if (sum % cnt == 0)
                    {
                        ans++;
                    }
