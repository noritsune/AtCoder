...
class Sol{
	...
	public void Solve(){
        int N = ri();
        List<int[]> memberPowers = new List<int[]>();
        for (int i = 0; i < N; i++) memberPowers.Add(ria());

        int[] nums = generateSuretsu(memberPowers.Count);
        int[][] memberPatterns = Combination.Enumerate(nums, 3, withRepetition:false).ToArray();

        int ans = int.MinValue;
        foreach (int[] memberPattern in memberPatterns)
        {
            int[] teamPower = new int[5];
            foreach (int memberIndex in memberPattern)
            {
                int[] memberPower = memberPowers[memberIndex];
                for (int i = 0; i < 5; i++)
                {
                    teamPower[i] = Math.Max(teamPower[i], memberPower[i]);
                }
            }

            int comPower = teamPower.Min();
            ans = Math.Max(ans, comPower);
        }
