...
public class Solver {
    public void Solve()
    {
        ...
        var patterns = Combination.Enumerate(Ss, K, true);
        var unitStrings = new List<string>();
        foreach (var pattern in patterns)
        {
            var unitString = string.Join("", pattern);
            unitStrings.Add(unitString);
        }

        unitStrings.Sort();

        Console.WriteLine(unitStrings[X - 1]);
    }
