...
public class Solver {
    public void Solve()
    {
        ...
        var seg = new LazySegTree(N, Math.Max, (a, b) => a + b);
        for (int i = 0; i < N; i++)
        {
            seg.ApplyRange(i, i + 1, As[i]);
        }

        foreach (var B in Bs)
        {
            // 箱Bのボールを全部取り出す
            var ballCnt = seg.Query(B, B + 1).Value;
            seg.ApplyRange(B, B + 1, -ballCnt);

            // まずは周回分をまとめて配る
            var loopCnt = ballCnt / N;
            if (loopCnt > 0)
            {
                seg.ApplyRange(0, N, loopCnt);
            }

            var remained = ballCnt % N;
            if (remained > 0 && B < N - 1)
            {
                // 右回転の残り分を配る
                var rLen = (int)Math.Min(remained, N - B - 1);
                seg.ApplyRange(B + 1, B + 1 + rLen, 1);
                remained -= rLen;
            }
            ...
            {
                // 次の回転分も残っていたら配る
                seg.ApplyRange(0, (int)remained, 1);
            }
        }

        var ans = seg.BuildLeafs();
