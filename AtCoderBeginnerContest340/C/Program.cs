...
public class Solver
{
    ...
    long Dfs(long n)
    {
        ...
        _memo.TryGetValue(n, out var memoV);
        if (memoV != 0) return memoV;

        var nextNFloored = n % 2 == 0 ? n / 2 : (n - 1) / 2;
        var nextNCeiled = n % 2 == 0 ? n / 2 : (n + 1) / 2;
        var costSum = n + Dfs(nextNFloored) + Dfs(nextNCeiled);
        _memo[n] = costSum;
