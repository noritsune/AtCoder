...
namespace KyoPro {
    ...
    public class Solver {
        public void Solve() {
            ...
            {
                var ABCD = Rsa();
                int A = int.Parse(ABCD[0]) - 1; string B = ABCD[1];
                int C = int.Parse(ABCD[2]) - 1; string D = ABCD[3];

                var from = A * 2 + (B == "R" ? 0 : 1);
                var to = C * 2 + (D == "R" ? 0 : 1);
            ...
            var groupCnt = uf.CalcGroupCnt();
            var notLoopCnt = groupCnt - loopCnt;
            Console.WriteLine($"{loopCnt} {notLoopCnt}");
        }
