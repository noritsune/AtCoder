...
namespace KyoPro {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            long ans = 0;
            var q = new Queue<(int x, int y, HashSet<int> passedNums)>();
            q.Enqueue((0, 0, new HashSet<int> { grid[0][0] }));
            while (q.Count > 0)
            {
                var (x, y, passedNums) = q.Dequeue();
                if (x == W - 1 && y == H - 1)
                {
                    ans++;
                    continue;
                ...
                if (x + 1 < W && !passedNums.Contains(grid[y][x + 1]))
                {
                    var newPassedNums = new HashSet<int>(passedNums) { grid[y][x + 1] };
                    q.Enqueue((x + 1, y, newPassedNums));
                }

                if (y + 1 < H && !passedNums.Contains(grid[y + 1][x]))
                {
                    var newPassedNums = new HashSet<int>(passedNums) { grid[y + 1][x] };
                    q.Enqueue((x, y + 1, newPassedNums));
                }
            }
