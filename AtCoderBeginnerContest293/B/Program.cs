...
namespace KyoPro {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            var hasCalleds = new bool[N];
            for (int i = 0; i < N; i++)
            {
                if (hasCalleds[i]) continue;

                var callIdx = As[i] - 1;
                hasCalleds[callIdx] = true;
            }

            var unCalledIdxes = hasCalleds
                .Select((hasCalled, idx) => (hasCalled, idx))
                .Where(t => !t.hasCalled)
                .Select(t => t.idx + 1)
                .ToArray();

            Console.WriteLine(unCalledIdxes.Length);
            Console.WriteLine(string.Join(" ", unCalledIdxes));
        }
