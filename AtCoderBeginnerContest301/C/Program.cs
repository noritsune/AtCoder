...
public class Solver {
    public void Solve()
    {
        ...
        var atcoderCharSet = "atcoder".ToHashSet();

        var fixedS = S
            .Where(x => !atcoderCharSet.Contains(x) && x != '@')
            .OrderBy(c => c)
            .ToArray();
        var fixedT = T
            .Where(x => !atcoderCharSet.Contains(x) && x != '@')
            .OrderBy(c => c)
            .ToArray();
        if (!fixedS.SequenceEqual(fixedT))
        {
        ...
        var notFixedS = S
            .Where(x => atcoderCharSet.Contains(x))
            .OrderBy(c => c)
            .ToArray();
        var notFixedT = T
            .Where(x => atcoderCharSet.Contains(x))
        ...
        var notFixedSSet = notFixedS.ToHashSet();
        var notFixedTSet = notFixedT.ToHashSet();

        var diffS = notFixedS.Where(c => !notFixedTSet.Contains(c)).ToArray();
        var diffT = notFixedT.Where(c => !notFixedSSet.Contains(c)).ToArray();

        var atCntS = S.Count(x => x == '@');
        var atCntT = T.Count(x => x == '@');

        if (atCntS < diffT.Length || atCntT < diffS.Length)
        {
            Console.WriteLine("No");
            return;
        }
