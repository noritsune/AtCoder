...
namespace AtCoder {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            var ruisekiSeconds = new double[N + 1];
            for (int i = 0; i < N; i++)
            {
                int A = ABs[i][0];
                int B = ABs[i][1];
                ruisekiSeconds[i + 1] = ruisekiSeconds[i] + A / (double)B;
            }

            var ruisekiLength = new int[N + 1];
            for (int i = 0; i < N; i++)
            {
                int A = ABs[i][0];
                ruisekiLength[i + 1] += ruisekiLength[i] + A;
            }

            double halfSeconds = ruisekiSeconds.Last() / 2;
            var lastDoukasenIdx = LowerBoundUnder(ruisekiSeconds, halfSeconds);
            var lastB = ABs[lastDoukasenIdx][1];
            var remainSeconds = halfSeconds - ruisekiSeconds[lastDoukasenIdx];
            double ans = ruisekiLength[lastDoukasenIdx] + remainSeconds * lastB;
