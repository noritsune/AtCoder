...
namespace KyoPro {
    ...
    public class Solver {
        Graph<string> _graph = new Graph<string>(Graph<string>.Type.DirectedGraph);
        Dictionary<string, int> _vertStat = new Dictionary<string, int>();
        public void Solve()
        {
            var N = Rl();
            ...
            {
                _graph.AddEdge(ST[0], ST[1]);
            }

            foreach (var vert in verts)
            {
                _vertStat.Add(vert, 0);
            }

            foreach (var vert in verts)
            {
                if(_vertStat[vert] != 0) continue;

                if (IsAcyclic(vert)) continue;
        ...
        bool IsAcyclic(string vert)
        {
            _vertStat[vert] = 1;

            foreach (var nextVert in _graph.Vertices[vert])
            {
                if(_vertStat[nextVert] == 1) return false;

                if(!IsAcyclic(nextVert)) return false;
            }
