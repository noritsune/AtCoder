...
public class Solver {
    public void Solve()
    {
        ...
        var STileX = CalcTileX(Sx, Sy); var STileY = Sy;
        var TTileX = CalcTileX(Tx, Ty); var TTileY = Ty;

        var moveY = Math.Abs(STileY - TTileY);

        var moveRangeL = Sy % 2 == 0
            ? STileX - moveY / 2
            : STileX - (moveY + 1) / 2;
        var moveRangeR = Sy % 2 == 0
            ? STileX + (moveY + 1) / 2
            : STileX + moveY / 2;

        var moveX = moveRangeL <= TTileX && TTileX <= moveRangeR
            ? 0
            : Math.Min(
                Math.Abs(moveRangeL - TTileX),
                Math.Abs(moveRangeR - TTileX)
            );
