...
namespace KyoPro {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            var minCnt = new List<int[]>();
            for (int i = 0; i <= N; i++)
            {
                for (int j = 0; j <= N; j++)
                {
                    minCnt.Add(Enumerable.Repeat(-1, N + 1).ToArray());
                }
            }

            var q = new Queue<(int x, int y, int cnt)>();
            q.Enqueue((1, 1, 0));
            var posSetInQ = new HashSet<string>();
            posSetInQ.Add("1 1");
            while (q.Count > 0)
            {
                var (x, y, cnt) = q.Dequeue();

                minCnt[y][x] = cnt;

                var posStr = $"{x} {y}";
                posSetInQ.Remove(posStr);
                ...
                foreach (var offset in offsets)
                {
                    ...
                    var minCost = minCnt[nextY][nextX];
                    if(minCost != -1 && minCost <= cnt) continue;

                    var nextPosStr = $"{nextX} {nextY}";
                    if(posSetInQ.Contains(nextPosStr)) continue;

                    q.Enqueue((nextX, nextY, cnt + 1));
                    posSetInQ.Add(nextPosStr);
                }
            }
            ...
            {
                var row = minCnt[i];
                Console.WriteLine(string.Join(" ", row.Skip(1)));
            }
        }
