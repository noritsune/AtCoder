...
namespace KyoPro {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            var xSets = new List<HashSet<int>>();
            for (int i = 0; i < M; i++)
            {
                var xs = Ria().Skip(1);
                xSets.Add(xs.ToHashSet());
            }

            for (int manA = 1; manA <= N; manA++)
            {
                for (int manB = 1; manB <= N; manB++)
                {
                    if(manA == manB) continue;

                    bool isContainsAAndBAny = xSets.Any(xSet =>
                        xSet.Contains(manA) && xSet.Contains(manB)
                    );

                    if (isContainsAAndBAny) continue;
