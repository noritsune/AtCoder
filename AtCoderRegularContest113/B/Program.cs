...
class Sol{
	...
	public void Solve(){
        ...
        List<int[]> firstPalacePatterns = new List<int[]>{
            new int[] {0},
            new int[] {1},
            new int[] {2, 4, 8, 6},
            new int[] {3, 9, 7, 1},
        ...
        int[] powNumPattern = firstPalacePatterns[B % 10];
        long powNum = powNumPattern[(C - 1) % powNumPattern.Length];

        int[] ansPattern = firstPalacePatterns[A % 10];
		Console.WriteLine(ansPattern[(powNum - 1) % ansPattern.Length]);
