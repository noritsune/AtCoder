...
public class Solver {
    public void Solve()
    {
        ...
        var haihunCnt = N % 2 == 0 ? N / 2 - 1 : N / 2;
        var headAndTail = Enumerable.Repeat('-', haihunCnt).ToList();
        var center = Enumerable.Repeat('=', N % 2 == 0 ? 2 : 1).ToList();

        var ans = headAndTail.Concat(center).Concat(headAndTail).ToArray();
