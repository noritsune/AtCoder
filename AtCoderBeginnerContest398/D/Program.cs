...
public class Solver {
    public void Solve()
    {
        ...
        var cToOffsets = new Dictionary<char, Vector2>
        {
            { 'N', new(1, 0) },
            { 'S', new(-1, 0) },
            { 'E', new(0, -1) },
        ...
        var takaPos = new Vector2(R, C);
        var firePos = new Vector2(0, 0);
        var smokePoss = new HashSet<Vector2>();
        smokePoss.Add(firePos);

        for (int i = 0; i < N; i++)
        {
            var c = S[i];
            var offset = cToOffsets[c];
            ...
            Console.Write(smokePoss.Contains(takaPos) ? "1" : "0");
        }
