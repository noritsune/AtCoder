...
public class Solver {
    ...
    {
        var NT = Rsa();
        var N = int.Parse(NT[0]); var T = NT[1];
        var Ss = new string[N];
        for (int i = 0; i < N; i++) Ss[i] = Rs();

        var subStrToIncludeCnt = new Dictionary<string, int>();
        foreach (var S in Ss)
        {
            var TIdx = T.Length - 1;
            var subT = "";
            for (int i = S.Length - 1; i >= 0; i--)
            {
                if (S[i] == T[TIdx])
                {
                    ...
                    subStrToIncludeCnt.TryAdd(subT, 0);
                    subStrToIncludeCnt[subT]++;
        ...
        foreach (var S in Ss)
        {
            ...
            {
                if (S[j] == T[currentTIdx])
                {
                    currentTIdx++;
                }

                if (currentTIdx == T.Length)
                {
                    break;
                }
            }
            ...
            var remainedT = T.Substring(currentTIdx);
            if (subStrToIncludeCnt.TryGetValue(remainedT, out var cnt))
            {
                ans += cnt;
            }
        }
