...
public class Solver {
    public void Solve()
    {
        ...
        long ans = 0;
        var maxNum = long.Parse(new string(S.OrderByDescending(c => c).ToArray()));
        for (long i = 0; i * i <= maxNum; i++)
        {
            var heiho = i * i;

            // N桁になるように先頭ゼロ埋め
            var strHeiho = heiho.ToString();
            while (strHeiho.Length < N)
            {
                strHeiho = "0" + strHeiho;
            }
            var numToCntHeiho = new int[10];
            foreach (var c in strHeiho)
            {
                numToCntHeiho[c - '0']++;
            }
            ...
            for (int j = 0; j < 10; j++)
            {
                if (numToCntS[j] != numToCntHeiho[j])
                {
                    isOk = false;
                    break;
                }
