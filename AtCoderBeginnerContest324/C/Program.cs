...
public class Solver {
    ...
    {
        var NTDash = Rsa();
        var N = int.Parse(NTDash[0]); var TDash = NTDash[1];
        var Ss = new string[N];
    ...
    static bool IsOk(string T, string S)
    {
        if (T == S) return true;
        ...
        else if (T.Length + 1 == S.Length)
        {
            for (int i = 0; i < T.Length; i++)
            {
                if (T[i] != S[i])
                {
                    return T == S.Substring(0, i) + S.Substring(i + 1);
                }
            }
            return true;
        }
        else if (T.Length == S.Length + 1)
        {
            for (int i = 0; i < S.Length; i++)
            {
                if (T[i] != S[i])
                {
                    return T == S.Substring(0, i) + T[i] + S.Substring(i);
                }
            }
            return true;
        }
