...
public class Solver {
    ...
    {
        var N1N2M = Ria();
        var N1 = N1N2M[0]; var N2 = N1N2M[1]; var M = N1N2M[2];

        var dijkstra = new Dijkstra(N1 + N2 + 1);
        for (int i = 0; i < M; i++)
        {
            var abs = Ria();
            var a = abs[0]; var b = abs[1];
            dijkstra.Add(a, b);
            dijkstra.Add(b, a);
        }

        var minCostsFromHead = dijkstra.GetMinCost(1);
        var minCostsFromTail = dijkstra.GetMinCost(N1 + N2);

        var maxCostFromHead = minCostsFromHead.Where(cost => cost != Dijkstra.INF).Max();
        var maxCostFromTail = minCostsFromTail.Where(cost => cost != Dijkstra.INF).Max();

        Console.WriteLine(maxCostFromHead + maxCostFromTail + 1);
    }
...
{
    public const long INF = 1000000000000000000;
