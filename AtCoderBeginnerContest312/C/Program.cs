...
public class Solver {
    public void Solve()
    {
        ...
        var As = Rla().OrderBy(n => n).ToArray();
        var Bs = Rla().OrderBy(n => n).ToArray();

        var prices = As.ToHashSet()
            .Union(Bs.ToHashSet())
            .OrderBy(n => n)
            .ToArray();
        foreach (var rawPrice in prices)
        {
            for (int dP = -1; dP <= 1; dP++)
            {
                var price = rawPrice + dP;
                var sellerCnt = UpperBound(As, price, Comparer<long>.Default);
                var buyerCnt = M - LowerBound(Bs, price, Comparer<long>.Default);
                if (sellerCnt >= buyerCnt)
                {
