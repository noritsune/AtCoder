    ...
    public class Solver {
        public void Solve()
        {
            ...
            for (int i = 1; i < ruiseki.Length; i++)
            {
                ...
                bool isInSectionOld = isInSection;
                isInSection = sum > 0;
                
                if(!isInSectionOld && isInSection)
                {
                    sectionStart = i;
                }
                else if(isInSectionOld && !isInSection)
                {
                    XYs.Add((sectionStart, i));
                }
            }
        ...
        {
            var N = Rl();
            var LRs = new List<(long L, long R)>();
            for (int i = 0; i < N; i++)
            {
                var LR = Rla();
                LRs.Add((LR[0], LR[1]));
