...
namespace KyoPro {
    ...
    public class Solver {
        public void Solve()
        {
            var h1h2h3w1w2w3 = Ria();
            var h1 = h1h2h3w1w2w3[0]; var h2 = h1h2h3w1w2w3[1]; var h3 = h1h2h3w1w2w3[2];
            var w1 = h1h2h3w1w2w3[3]; var w2 = h1h2h3w1w2w3[4]; var w3 = h1h2h3w1w2w3[5];

            var row1Patterns = GenerateRowPatterns(h1);
            var row2Patterns = GenerateRowPatterns(h2);
            var row3Patterns = GenerateRowPatterns(h3);

            long ans = 0;
            foreach (var row1 in row1Patterns)
            {
                foreach (var row2 in row2Patterns)
                {
                    foreach (var row3 in row3Patterns)
                    {
                        var col1Sum = row1[0] + row2[0] + row3[0];
                        var col2Sum = row1[1] + row2[1] + row3[1];
                        var col3Sum = row1[2] + row2[2] + row3[2];
                        if (col1Sum == w1 && col2Sum == w2 && col3Sum == w3)
                        {
                            ans++;
                        }
                    }
        ...
        int[][] GenerateRowPatterns(int sum)
        {
            return Combination.Enumerate(Enumerable.Range(1, sum), 2, withRepetition: false)
                .Where(pat => pat[1] - pat[0] > 1)
                .Select(pat => new[] { pat[0], pat[1] - pat[0] - 1, sum + 1 - pat[1] })
                .ToArray();
        }
