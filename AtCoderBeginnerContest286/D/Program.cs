...
namespace KyoPro {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            // i種類目の硬貨までを使ってj円をぴったり支払えるか
            var dp = new bool[N + 1, X + 1];
            for (int i = 0; i <= N; i++)
            {
                dp[i, 0] = true;
            }
            for (int j = ABs[0].A; j <= Math.Min(X, ABs[0].A * ABs[0].B); j += ABs[0].A)
            {
                dp[1, j] = true;
            }
            ...
            for (int i = 2; i <= N; i++)
            {
                var jToCoinUsedCnt = new long[X + 1];
                for (int j = 1; j <= X; j++)
                {
                    dp[i, j] = dp[i - 1, j];
                    if(dp[i, j]) continue;

                    var prevJ = j - ABs[i - 1].A;
                    // 現在見ている種類のコインを使えないくらい安い
                    if (prevJ < 0) continue;

                    var prevCoinUsedCnt = jToCoinUsedCnt[prevJ];
                    // これ以上現在見ている種類のコインを使えない
                    if (prevCoinUsedCnt >= ABs[i - 1].B) continue;

                    // 現在見ている種類のコインを1枚足してもぴったりにならない
                    if(!dp[i, j - ABs[i - 1].A]) continue;

                    // 現在見ている種類のコインを1枚足したらぴったりになる
                    dp[i, j] = true;
                    jToCoinUsedCnt[j] = prevCoinUsedCnt + 1;
                }
            }
