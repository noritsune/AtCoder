...
namespace KyoPro {
    ...
    public class Solver {
        ...
        public void Solve()
        {
            ...
            for (int shiftCnt = 0; shiftCnt < N; shiftCnt++)
            {
                var shiftedS = S.Skip(shiftCnt).Concat(S.Take(shiftCnt)).ToArray();
                var replaceCnt = CharReplaceCntToMakePalindrome(shiftedS);

                long cost = shiftCnt * A + replaceCnt * B;
        ...
        int CharReplaceCntToMakePalindrome(char[] str)
        {
            var cnt = 0;
