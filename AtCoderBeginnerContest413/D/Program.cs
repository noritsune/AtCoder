...
public class Solver {
    public void Solve()
    {
        ...
        for (int t = 0; t < T; t++)
        {
            ...
            var pCnt = As.Count(A => A == As[0]);
            var nCnt = As.Count(A => A == -As[0]);
            ...
            var absSortedAs = As.OrderBy(Math.Abs).ToArray();
            var isOk = true;
            for (int i = 0; i + 2 < N; i++)
            {
                if (absSortedAs[i] * absSortedAs[i + 2] != absSortedAs[i + 1] * absSortedAs[i + 1])
                {
                    isOk = false;
                    break;
                }
