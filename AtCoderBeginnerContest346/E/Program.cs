...
public class Solver {
    ...
    {
        var HWM = Ria();
        var H = HWM[0]; var W = HWM[1]; var M = HWM[2];
        var TAXs = new List<(int T, int A, int X)>();
        for (int i = 0; i < M; i++)
        {
            var TAX = Ria();
            var T = TAX[0]; var A = TAX[1] - 1; var X = TAX[2];
            TAXs.Add((T, A, X));
        }

        var nToCnt = new Dictionary<int, long>();
        ...
        for (int i = M - 1; i >= 0; i--)
        {
            ...
            if (T == 1)
            {
                if (!usedRows.Add(A)) continue;
                nToCnt.TryAdd(X, 0);
                nToCnt[X] += W - (long)usedCols.Count;
            }
            else
            {
                if (!usedCols.Add(A)) continue;
                nToCnt.TryAdd(X, 0);
                nToCnt[X] += H - (long)usedRows.Count;
            }
        }

        var nonZeroSum = nToCnt
            .Where(kv => kv.Key != 0)
            .Sum(kv => kv.Value);
        nToCnt[0] = (long)H * W - nonZeroSum;
        var anss = nToCnt
            .Where(kv => kv.Value > 0)
            .OrderBy(kv => kv.Key);
        Console.WriteLine(anss.Count());
        foreach (var (n, cnt) in anss)
        {
