...
public class Solver {
    public void Solve()
    {
        ...
        // 010..の数列を作るためのコストの累積和
        var ruiCost01 = new long[N + 1];
        // 101..の数列を作るためのコストの累積和
        var ruiCost10 = new long[N + 1];
        for (int i = 0; i < N; i++)
        {
            ruiCost01[i + 1] = ruiCost01[i];
            ruiCost10[i + 1] = ruiCost10[i];
        ...
        for (int i = 0; i < N - 1; i++)
        {
            // i文字目とi + 1文字目を00に、それ以外を0と1の交互にするパターン
            // i文字目までを01か10にする
            long cost = i % 2 == 0 ? ruiCost01[i] : ruiCost10[i];
            // i文字目を0にする
            if (S[i] == '1') cost += Cs[i];
            // i + 1文字目を0にする
            if (S[i + 1] == '1') cost += Cs[i + 1];
            // i + 2文字目以降を01か10にする
            if (i < N - 2)
            {
                cost += i % 2 == 0
                    ? ruiCost10[N] - ruiCost10[i + 2]
                    : ruiCost01[N] - ruiCost01[i + 2];
            }
            minCost = Math.Min(minCost, cost);

            // i文字目とi + 1文字目を11に、それ以外を0と1の交互にするパターン
            cost = costSum - cost;
            minCost = Math.Min(minCost, cost);
        }
