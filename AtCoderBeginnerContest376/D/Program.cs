...
public class Solver
{
    public void Solve()
    {
        ...
        var (minCosts, _) = dijkstra.GetClosestRoot(1);

        long? ans = null;
        for (int v = 2; v <= N; v++)
        {
            var minCost = minCosts[v];
            if (minCost != 1000000000000000000 && to1Vs.Contains(v))
            {
                ans = Math.Min(ans ?? long.MaxValue, minCost + 1);
            }
        }
