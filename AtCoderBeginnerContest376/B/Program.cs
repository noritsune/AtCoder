...
public class Solver {
    public void Solve()
    {
        ...
        {
            var HT = Rsa();
            var (H, T) = (HT[0], int.Parse(HT[1]) - 1);

            var targetHandPos = H == "L" ? l : r;
            var otherHandPos = H == "L" ? r : l;

            var minCost = int.MaxValue;

            // まずは右回りで辿ってみる
            var pos = targetHandPos;
