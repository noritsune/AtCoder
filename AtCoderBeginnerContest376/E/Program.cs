...
public class Solver {
    public void Solve()
    {
        ...
        for (int i = 0; i < T; i++)
        {
            ...
            var segTreeA = new SegTree<long>(N, Math.Max);
            for (int j = 0; j < N; j++)
            {
                segTreeA.Set(j, As[j]);
            }
            ...
            // Bsの累積和
            var ruiB = new long[N + 1];
            for (int j = 0; j < N; j++)
            {
                ruiB[j + 1] = ruiB[j] + Bs[j];
            }
            ...
            {
                // iからi+KまでのAの最大とBの和
                var maxA = segTreeA.Query(j, j + K);
                var sumB = ruiB[j + K] - ruiB[j];
                var score = maxA.Value * sumB;
