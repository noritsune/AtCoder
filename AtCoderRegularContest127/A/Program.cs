...
namespace util {
    ...
    public class Sol{
        ...
        public void Solve()
        {
            ...
            long ans = 0;
            long x = 0;
            var sumNums = new List<long>(){1, 8};
            long alpha = 0;
            while (x <= N)
            {
                for (int i = 0; i < sumNums.Count; i++)
                {
                    long oneCnt = i < sumNums.Count / 2
                        ? i + 1
                        : sumNums.Count - 1 - i;
                    if (x + sumNums[i] > N)
                    {
                    ...
                    x += sumNums[i];
                    ans += oneCnt * sumNums[i];
                }

                alpha++;
                sumNums.Insert(0, (long)Math.Pow(10, alpha - 1));
                sumNums.Add(8 * (long)Math.Pow(10, alpha));
            }
