...
class TreeNode {
    ...
    public HashSet<TreeNode> _childs = new HashSet<TreeNode>();
    ...
    public void AddFromParent() {
        if(_parent == null) return;

        _score += _parent._score;
    }
}
...
class Sol{
	...
	public void Solve(){
        ...
        for (int i = 0; i < N - 1; i++) es.Add(ria());
        int Q = ri();
        List<long[]> qs = new List<long[]>();
        for (int i = 0; i < Q; i++) qs.Add(rla());

        //頂点を生成
        TreeNode[] treeNodes = new TreeNode[N + 1];
        for (int i = 1; i <= N; i++) treeNodes[i] = new TreeNode(i);
        
        //木を形成
        foreach (int[] e in es)
        {
            TreeNode parent = treeNodes[e.Min()];
            TreeNode child = treeNodes[e.Max()];
        ...
        foreach (long[] q in qs)
        {
            ...
            TreeNode originNode;
            TreeNode againstNode;
            if(t == 1) {
                originNode = treeNodes[e[0]];
                againstNode = treeNodes[e[1]];
            } else {
                originNode = treeNodes[e[1]];
                againstNode = treeNodes[e[0]];
            }
            
            // 加算方向を判定
            if(originNode._parent == againstNode) {
                //下
                originNode._score += x;
            } else {
                //上
                treeNodes[1]._score += x;
                againstNode._score -= x;
            }
        }
        ...
        do {
            ...
            foreach (TreeNode child in currentNode._childs) queue.Enqueue(child);

            if(currentNode._parent == null) continue;

            currentNode._score += currentNode._parent._score;
        } while(queue.Count > 0);

        for (int i = 1; i <= N; i++)
        {
            Console.WriteLine(treeNodes[i]._score);
        }
		Console.ReadLine();
	}
