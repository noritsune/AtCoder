...
public class Solver {
    public void Solve()
    {
        ...
        var boxes = Enumerable.Range(0, N).Select(_ => new List<long>()).ToArray();
        for (int i = 0; i < N; i++)
        {
            boxes[As[i] - 1].Add(Ws[i]);
        }

        foreach (var box in boxes) box.Sort();

        var ans = boxes.Sum(box => box.Take(box.Count - 1).Sum());
