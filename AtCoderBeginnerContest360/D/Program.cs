...
public class Solver {
    public void Solve()
    {
        ...
        var Xs0 = new Set<long>();
        var Xs1 = new List<long>();
        for (int i = 0; i < N; i++)
        {
            var X = Xs[i];
            // 左向き
            if (S[i] == '0') Xs0.Insert(X);
            // 右向き
            else Xs1.Add(X);
        }

        long ans = 0;
        foreach (var X1 in Xs1)
        {
            // Xs0の中でX1以上かつX1 + 2 * T以下のものを数える
            var lb = Xs0.LowerBound(X1);
            var ub = Xs0.UpperBound(X1 + 2 * T);
            long cnt = ub - lb;
            ans += cnt;
        }
