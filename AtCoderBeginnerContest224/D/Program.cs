...
namespace AtCoder {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            for (int i = 0; i < M; i++) uvs.Add(ria());
            var ps = ria().ToList();

            var graph = new Graph<int>(Graph<int>.Type.UndirectedGraph);
            for (int i = 1; i <= 9; i++) graph.AddVertex(i);
            foreach (var uv in uvs) graph.AddEdge(uv[0], uv[1]);

            var goalPPosStr = "912345678";
            var pPosStrHistories = new HashSet<string>();
            var blankPos = Enumerable.Range(1, 9).ToList().Find(n => !ps.Contains(n));
            var pPosState = new int[9];
            pPosState[0] = blankPos;
            for (int i = 0; i < ps.Count; i++) pPosState[i + 1] = ps[i];
            ...
            {
                pPosState = q.Dequeue();
                string pPosStr = new string(pPosState.Select(n => (char)(n + 48)).ToArray());
                if (pPosStr.Equals(goalPPosStr))
                {
                ...
                if(pPosStrHistories.Contains(pPosStr)) continue;
                pPosStrHistories.Add(pPosStr);
                
                blankPos = pPosState[0];
                foreach (var nextBlankPos in graph.Vertices[blankPos])
                {
                    var nextPPosState = new int[pPosState.Length];
                    Array.Copy(pPosState, nextPPosState, pPosState.Length);

                    var nextBlankKomaIndex = Array.IndexOf(pPosState, nextBlankPos);
                    nextPPosState[nextBlankKomaIndex] = blankPos;
                    nextPPosState[0] = nextBlankPos;
                        
                    q.Enqueue(nextPPosState);
                }
            }
