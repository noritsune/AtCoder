...
class Sol{
	...
	public void Solve(){
        ...
        List<char> qChars = new List<char>();
        for (int i = 0; i < S.Length; i++)
        {
            switch(S[i]) {
                case 'o':
                    oChars.Add(i.ToString()[0]);
                    break;
                case 'x':
                    xChars.Add(i.ToString()[0]);
                    break;
                case '?':
                    qChars.Add(i.ToString()[0]);
                    break;
            }
        }
        ...
        {
            string strI = String.Format("{0:D4}", i);
            List<char> iChars = strI.ToList();
            
            bool isNG = false;
            // 使用不可の数字を使用していないか
            foreach (char xChar in xChars)
            {
                if(iChars.Contains(xChar)) {
                    isNG = true;
                    break;
                }
            }
            ...
            // 必須の数字を使用済みか
            foreach (char oChar in oChars)
            {
                if(isNG) {
                    break;
                }

                if(iChars.Contains(oChar)) {
                    iChars.RemoveAll((c) => c == oChar);
                } else {
                    isNG = true;
                    break;
                }
            ...
            // 使用可能な数字で残りの数字を埋められるか
            foreach (char qChar in qChars)
            {
                if(iChars.Count == 0 || isNG) {
                    break;
                }

                if(iChars.Contains(qChar)) {
                    iChars.RemoveAll((c) => c == qChar);
                }
            }
            
            // string result;
            if(!isNG && iChars.Count == 0) {
                ans++;
                // result = "○";
            } else {
                // result = "x";
            ...
            // Console.WriteLine(String.Format("{0}: {1}", strI, result));
        }
