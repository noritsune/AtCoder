...
class Sol{
	...
	public void Solve(){
        ...
        H = HW[0];
        W = HW[1];
        for (int i = 0; i < H; i++) Ass.Add(rs());

        Pos nowPos = new Pos(0, 0);
        Pos goalPos = new Pos(W - 1, H - 1);
        bool isTakaTurn = true;
        int takaPnt = 0;
        int aokiPnt = 0;
        while(nowPos != goalPos) {
            nowPos = getNextPos(nowPos);
            
            if(nowPos == null) break;

            int point = getPosPoint(nowPos);
    ...
    Pos getNextPos(Pos nowPos){
        bool canGoRight  = nowPos._x < W - 1;
        bool canGoBottom = nowPos._y < H - 1;

        Pos rightPos = new Pos(nowPos);
        rightPos._x++;
        Pos bottomPos = new Pos(nowPos);
        bottomPos._y++;
        
        if(canGoRight && canGoBottom) {
            int rightPoint  = getPosPoint(rightPos);
            int bottomPoint = getPosPoint(bottomPos);
        ...
        } else if(canGoRight && !canGoBottom) {
            return rightPos;
        } else if(!canGoRight && canGoBottom) {
            return bottomPos;
        } else {
    ...
    int getPosPoint(Pos pos) {
        char masu = Ass[pos._x][pos._y];
        switch(masu) {
            case '+':
                return 1;
            case '-':
