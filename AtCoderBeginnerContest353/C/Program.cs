...
{
    public const long MOD = 100000000;
}
...
public class Solver {
    ...
    {
        var N = Rl();
        var As = Rla().OrderBy(A => A).ToArray();

        long ans = 0;
        var mSet = new MultiSet<long>();
        foreach (var A in As)
        {
            var lb = mSet.LowerBound(CONST.MOD - A);
            var remain = lb >= 0 ? mSet.Count() - lb : 0;
            ans += A * (N - 1) - CONST.MOD * remain;
            mSet.Insert(A);
        }
