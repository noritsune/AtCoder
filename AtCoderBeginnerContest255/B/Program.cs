...
namespace KyoPro {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            var N = NK[0];
            var K = NK[1];
            var As = Ria().Select(i => i - 1).ToArray();
            var peoplePositions = new List<Vector2>();
            for (int i = 0; i < N; i++)
            {
                var XY = Ria();
                peoplePositions.Add(new Vector2(XY[0], XY[1]));
            }

            var lightPeopleIndex = As.ToHashSet();
            var lightPositions = peoplePositions
                .Where((p, i) => lightPeopleIndex.Contains(i))
                .ToArray();
            
            var minDistanceFromLight = new double[N];
            for (int i = 0; i < N; i++)
            {
                if(lightPeopleIndex.Contains(i)) continue;

                minDistanceFromLight[i] = double.MaxValue;
                foreach (var lightPosition in lightPositions)
                {
                    var distFromLight = lightPosition.DistanceTo(peoplePositions[i]);
                    minDistanceFromLight[i] = Math.Min(minDistanceFromLight[i], distFromLight);
                }
            }

            Console.WriteLine(minDistanceFromLight.Max());
        }
