    ...
    public class Solver {
        ...
        public void Solve()
        {
            ...
            while (min <= max) // 範囲内にある限り探し続ける
            {
                ...
                int compareResult = X.CompareTo(CalcGeneralItem(mid));
                if(isDesc) compareResult *= -1;
                if(compareResult > 0) min = mid + 1;
                else max = mid - 1;
            }
            long lowerBound = Math.Clamp(min, 0, N - 1);
            ...
            long lowerBoundUnder = Math.Clamp(max, 0, N - 1);

            long lowerBoundNum = CalcGeneralItem(lowerBound);
            long lowerBoundUnderNum = CalcGeneralItem(lowerBoundUnder);
            
            long diffWithLowerBound = Math.Abs(X - lowerBoundNum);
            long diffWithLowerBoundUnder = Math.Abs(X - lowerBoundUnderNum);

            Console.WriteLine(Math.Min(diffWithLowerBound, diffWithLowerBoundUnder));
        }

        public long CalcGeneralItem(long index)
        {
            return A + index * D;
        }
