...
namespace KyoPro {
    ...
    public class Solver {
        public void Solve2()
        {
            ...
            var sortedAs = As.OrderBy(x => x).ToArray();
            ...
            for (int i = 0; i < N; i++)
            {
                ruiseki[i + 1] = ruiseki[i] + sortedAs[i];
            }
            ...
            for (long i = 0; i < Q; i++)
            {
                ...
                long lowerBound = LowerBound(sortedAs, X, Comparer<long>.Default);
                long upperBound = UpperBound(sortedAs, X, Comparer<long>.Default);
                
                long lowerDiff = X * lowerBound - ruiseki[lowerBound];
                long upperDiff = (AsSum - ruiseki[upperBound]) - X * (N - upperBound);
                Console.WriteLine(lowerDiff + upperDiff);
            }
        }
        ...
        public void Solve1()
        {
            ...
            var lessAsSums = new long[N + 1];
            for (long i = 0; i < N; i++)
            {
                var delta = sortedAs[i];
                while(i + 1 < N && sortedAs[i] == sortedAs[i + 1])
                {
                    lessAsSums[i + 1] = lessAsSums[i];
                    delta += sortedAs[i];
                    i++;
                }
                
                lessAsSums[i + 1] = lessAsSums[i] + delta;
            }
            
            var moreAsSums = new long[N + 1];
            for (long i = N - 1; i > 0; i--)
            {
                var delta = sortedAs[i];
                while(i - 1 > 0 && sortedAs[i] == sortedAs[i - 1])
                {
                    moreAsSums[i - 1] = moreAsSums[i];
                    delta += sortedAs[i];
                    i--;
                }
                
                moreAsSums[i - 1] = moreAsSums[i] + delta;
            }
            ...
            for (long i = 0; i < Q; i++)
            {
                ...
                var lowerBound = LowerBound(sortedAs, X, Comparer<long>.Default);
                var lessAsSum = lessAsSums[lowerBound];
                var lessDiffSum = X * lowerBound - lessAsSum;
                
                var lowerBoundUnder = LowerBoundUnder(sortedAs, X, Comparer<long>.Default);
                var moreAsSum = lowerBoundUnder >= 0 ? moreAsSums[lowerBoundUnder] : AsSum;
                var moreDiffSum = moreAsSum - X * (N - lowerBoundUnder - 1);
 
                Console.WriteLine(lessDiffSum + moreDiffSum);
            }
        }
        ...
        public void SolveTest()
        {
            ...
            {
                int X = Ri();
                var diffSum = As.Sum(A => Math.Abs(A - X));
