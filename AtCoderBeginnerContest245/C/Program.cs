...
namespace AtCoder {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            for (int i = 0; i < N - 1; i++)
            {
                int diffAA = Math.Abs(As[i] - As[i + 1]);
                int diffAB = Math.Abs(As[i] - Bs[i + 1]);
                int diffBA = Math.Abs(Bs[i] - As[i + 1]);
                int diffBB = Math.Abs(Bs[i] - Bs[i + 1]);
                
                bool canUseANext = canUseA && diffAA <= K || canUseB && diffBA <= K;
                bool canUseBNext = canUseA && diffAB <= K || canUseB && diffBB <= K;

                if (canUseANext || canUseBNext)
                {
                    canUseA = canUseANext;
                    canUseB = canUseBNext;
                }
