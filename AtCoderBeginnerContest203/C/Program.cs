...
class Sol{
	...
	public void Solve(){
        ...
        for (int i = 0; i < N; i++) ABs.Add(rla());
        
        ABs.Sort((a, b) => a[0].CompareTo(b[0]));

        long currentMoney = K;
        long lastIndex = 0;
        foreach (long[] AB in ABs)
        {
            long index      = AB[0];
            long givedMoney = AB[1];
            long lostMoney = index - lastIndex;

            if(currentMoney - lostMoney < 0) {
                break;
            }

            currentMoney -= lostMoney;
            currentMoney += givedMoney;

            lastIndex = index;
        }
        
        lastIndex += currentMoney;
