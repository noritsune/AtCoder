...
namespace KyoPro {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            var totalMinInit = H * 60 + M;

            for (int minOffset = 0; minOffset < 24 * 60; minOffset++)
            {
                var totalMin = totalMinInit + minOffset;
                var currentHour   = totalMin / 60 % 24;
                var currentMinute = totalMin % 60 % 60;

                int Ahour2nd = currentHour / 10;
                int Bhout1st = currentHour % 10;
                int Cmin2nd = currentMinute / 10;
                int Dmin1st = currentMinute % 10;

                var transformedHour = Ahour2nd * 10 + Cmin2nd;
                var transformedMinute = Bhout1st * 10 + Dmin1st;
                if(transformedHour >= 0 && transformedHour < 24
                && transformedMinute >= 0 && transformedMinute < 60)
                {
