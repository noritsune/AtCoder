...
namespace KyoPro {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            var notResetAIdxes = new HashSet<int>();
            for (int i = 0; i < N; i++)
            {
                notResetAIdxes.Add(i);
            }

            var aIdxToAddNum = new long[N];
            Array.Copy(As, aIdxToAddNum, N);

            long latestResetNum = 0;
            for (int qIdx = 1; qIdx <= Q; qIdx++)
            {
                ...
                {
                    case 1:
                        var resetNum = query[1];

                        foreach (var notResetAIdx in notResetAIdxes)
                        {
                        ...
                        break;
                    case 2:
                        var aIdx2 = (int)query[1] - 1;
                        var addNum = query[2];

                        aIdxToAddNum[aIdx2] += addNum;
                        notResetAIdxes.Add(aIdx2);
                        break;
                    case 3:
                        var aIdx3 = (int)query[1] - 1;

                        var ans = latestResetNum + aIdxToAddNum[aIdx3];
