...
namespace KyoPro {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            var follows = new Dictionary<long, HashSet<long>>();
            ...
            for (int i = 0; i < Q; i++)
            {
                ...
                switch (T)
                {
                    case 1:
                        ...
                        {
                            follows[A] = new HashSet<long>();
                        }

                        follows[A].Add(B);
                        break;
                    case 2:
                        if(!follows.ContainsKey(A)) break;

                        follows[A].Remove(B);
                        break;
                    case 3:
                        var isFF = follows.ContainsKey(A) && follows[A].Contains(B)
                                && follows.ContainsKey(B) && follows[B].Contains(A);
                        Console.WriteLine(isFF ? "Yes" : "No");
                        break;
                }
            }
        }
    ...
    public class Graph<T>
    {
        ...
        public bool RemoveEdge(T from, T to)
        {
            ...
            if (GraphType == Type.UndirectedGraph) _vertices[to].Remove(from);
            return true;
        }
