...
namespace KyoPro {
    ...
    public class Solver {
        public void Solve()
        {
            var N = Ri();
            var As = new List<int[]>();
            for (int i = 0; i < N; i++) As.Add(Ria());
            var Bs = new List<int[]>();
            for (int i = 0; i < N; i++) Bs.Add(Ria());

            var grid = As;
            for (int i = 0; i < 4; i++)
            {
                if (IsCovered(grid, Bs))
                {
        ...
        // 1≤i,j≤N を満たすすべての整数の組(i,j) について同時にAi,jをAN+1−j,iで置き換える
        List<int[]> Rotate(List<int[]> inGrid)
        {
            var N = inGrid.Count;
            var outGrid = new List<int[]>();
            for (int i = 0; i < N; i++)
            {
                var tmp = new int[N];
                for (int j = 0; j < N; j++)
                {
                    tmp[j] = inGrid[N - 1 - j][i];
                }
                outGrid.Add(tmp);
            }
            return outGrid;
        ...
        // grid1の1である要素がgrid2の1か
        bool IsCovered(List<int[]> grid1, List<int[]> grid2)
        {
            var N = grid1.Count;
            for (int i = 0; i < N; i++)
            {
                for (int j = 0; j < N; j++)
                {
                    if (grid1[i][j] == 1 && grid2[i][j] == 0) return false;
                }
            }
            return true;
        }
