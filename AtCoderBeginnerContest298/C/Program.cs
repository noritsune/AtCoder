...
namespace KyoPro {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            var boxToCards = new Dictionary<int, MultiSet<int>>();
            var cardToBoxes = new Dictionary<int, Set<int>>();
            for (int i = 0; i < Q; i++)
            {
                var query = Ria();
                int card; int box;
                switch (query[0])
                {
                    case 1:
                        ...
                        boxToCards.TryAdd(box, new MultiSet<int>());
                        boxToCards[box].Insert(card);
                        cardToBoxes.TryAdd(card, new Set<int>());
                        cardToBoxes[card].Insert(box);
                        break;
                    case 2:
                        box = query[1];

                        var cards = boxToCards[box];
                        Console.WriteLine(string.Join(" ", cards.ToList()));
                        break;
                    case 3:
                        card = query[1];

                        var boxes = cardToBoxes[card];
                        Console.WriteLine(string.Join(" ", boxes.ToList()));
                        break;
                }
            }
        }
