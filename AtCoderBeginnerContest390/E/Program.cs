...
public class Solver {
    public void Solve()
    {
        ...
        // i番目までの食べ物を見てjカロリーを摂取したときのビタミンkの摂取量の最大値
        var dp = new long[N + 1, X + 1, 3];

        for (int i = 1; i <= N; i++)
        {
            var VAC = Rla();
            var (V, A, C) = (VAC[0] - 1, VAC[1], VAC[2]);
            for (int j = 0; j <= X; j++)
            {
                if (j < C)
                {
                ...
                {
                    dp[i, j, V] = Math.Max(dp[i - 1, j, V], dp[i, j - C, V] + A);
                }
            }
        }

        Console.WriteLine(Math.Min(dp[N, X, 0], Math.Min(dp[N, X, 1], dp[N, X, 2])));
    }
