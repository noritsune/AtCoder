...
public class Solver
{
    int _N;
    long[] _As;
    HashSet<long> _xors = new HashSet<long>();
    ...
    public void Solve()
    {
        ...
        Console.WriteLine(_xors.Count);
    }

    void Dfs(int mergeFromIdx)
    {
        for (int mergeToIdx = 0; mergeToIdx < _N; mergeToIdx++)
        {
            if (mergeFromIdx == mergeToIdx || _As[mergeFromIdx] == 0 || _As[mergeToIdx] == 0) continue;

            var mergeFrom = _As[mergeFromIdx];
            var mergeTo = _As[mergeToIdx];

            _As[mergeFromIdx] = 0;
            _As[mergeToIdx] = mergeFrom + mergeTo;
            ...
            _As[mergeFromIdx] = mergeFrom;
            _As[mergeToIdx] = mergeTo;
        }
    }
