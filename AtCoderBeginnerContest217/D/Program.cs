...
namespace util {
    ...
    public class Sol{
        ...
        public void Solve()
        {
            ...
            for (int i = 0; i < Q; i++) querys.Add(ria());

            var cutPoints = new List<int> {0, L};
            foreach (var query in querys)
            {
                int x = query[1];
                bool isCut = query[0] == 1;
                if (isCut)
                {
                    int index = LowerBound(cutPoints, x);
                    cutPoints.Insert(index, x);
                }
                else
                {
                    int start = cutPoints.ElementAt(LowerBoundUnder(cutPoints, x));
                    int end   = cutPoints.ElementAt(LowerBound(cutPoints, x));
