...
namespace KyoPro {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            for (int i = 1; i <= N; i++)
            {
                ans += CalcPatternCnt(i);
            }

            Console.WriteLine(ans);
        }

        long CalcPatternCnt(int num)
        {
            var primeFactors = PrimeFactors(num).ToArray();
            ...
            var eachPrimeFactorCounts = primeFactors.GroupBy(x => x)
                .Select(x => x.Count()).ToArray();
                
            var selectPatternCnts = eachPrimeFactorCounts
                .Select(cnt =>
                {
                    int patternCnt = cnt - cnt / 2;
                    if(cnt % 2 == 0) patternCnt++;
                    return patternCnt;
                })
                .ToArray();
                
            var selectPatternCntMulti = selectPatternCnts.Aggregate(1L, (acc, x) => acc * x);
            
            return selectPatternCntMulti * 2 - 1;
        }
        ...
        public void SolveTest()
        {
            ...
            for (int i = 1; i <= N; i++)
            {
                ...
                for (int j = 1; j < i; j++)
                {
                    var multiSqrt = Math.Sqrt(i * j);
                    if(multiSqrt % 1 != 0) continue;
                    
                    sum += 2;
                }

                Console.WriteLine(i + " → " + sum + " (" + CalcPatternCnt(i) + ")");
                
                sumAll += sum;
            }

            Console.WriteLine("sumAll = " + sumAll);
        }
