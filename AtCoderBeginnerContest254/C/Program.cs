...
namespace KyoPro {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            {
                var swappableAGroupUnSorted = new List<long>();
                var swappableAGroupSorted   = new List<long>();
                for (long AsIndex = offset; AsIndex < N; AsIndex += K)
                {
                    swappableAGroupUnSorted.Add(As[AsIndex]);;
                    swappableAGroupSorted.Add(sortedAs[AsIndex]);
                }
                
                swappableAGroupUnSorted.Sort();
                
                var isSame = !swappableAGroupUnSorted.Where((t, i) => t != swappableAGroupSorted[i]).Any();
                if (isSame) continue;
