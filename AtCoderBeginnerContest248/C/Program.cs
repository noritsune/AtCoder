...
namespace AtCoder {
    ...
    public class Solver {
        ...
        public void Solve()
        {
            ...
            long ans = PatternCnt(M, K, N, 0, 1);
            Console.WriteLine(ans);
        }

        long PatternCnt(long M, long K, long remainedNumCnt, long sum, long patternCnt)
        {
            if(memo[remainedNumCnt, sum] > 0) return memo[remainedNumCnt, sum];
            
            long patternCntSum = 0;
            if (remainedNumCnt == 1)
            {
                patternCntSum = patternCnt * Math.Min(M, K - sum) % MOD;
            }
            else
            {
                long iLimit = Math.Min(M, K - sum - remainedNumCnt + 1);
                for (long i = 1; i <= iLimit; i++)
                {
                    patternCntSum += PatternCnt(M, K, remainedNumCnt - 1, sum + i, patternCnt);
                    patternCntSum %= MOD;
                }
            }

            memo[remainedNumCnt, sum] = patternCntSum;
            return patternCntSum;
        }
