...
namespace AtCoder {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            var existIndices = new List<long>[N + 1];
            for (int i = 1; i <= N; i++)
            {
                existIndices[i] = new List<long>{0};
            }
            
            for (long i = 0; i < N; i++)
            {
                existIndices[As[i]].Add(i + 1);
            }
            ...
            for (long i = 0; i < Q; i++)
            {
                ...
                var leftIndex  = LowerBound(existIndices[X], L, Comparer<long>.Default);
                var rightIndex = LowerBoundUnder(existIndices[X], R, Comparer<long>.Default);
                
                if(leftIndex  < 0 || leftIndex  >= existIndices[X].Count
                || rightIndex < 0 || rightIndex >= existIndices[X].Count)
                {
                    Console.WriteLine(0);
                    continue;
                }
