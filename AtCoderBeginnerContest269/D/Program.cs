...
namespace KyoPro {
    ...
    public class Solver {
        ...
        {
            var N = Ri();
            var blackCellPoss = new HashSet<Vector2>();
            for (int i = 0; i < N; i++)
            {
                var blackCellPos = Ria();
                blackCellPoss.Add(new Vector2(blackCellPos[0], blackCellPos[1]));
            }
            ...
            while (blackCellPoss.Count > 0)
            {
                connectedCount++;
                
                var visitedCellPoss = new HashSet<Vector2>();
                var q = new Queue<Vector2>();
                q.Enqueue(blackCellPoss.First());
                while (q.Count > 0)
                {
                    var pos = q.Dequeue();
                    if (visitedCellPoss.Contains(pos)) continue;
                    visitedCellPoss.Add(pos);
                    blackCellPoss.Remove(pos);
                    
                    foreach (var nextOffset in offsets)
                    {
                        var nextPos = pos + nextOffset;
                        if (blackCellPoss.Contains(nextPos))
                        {
                            q.Enqueue(nextPos);
                        }
                    }
            ...
            Console.WriteLine(connectedCount);
        }
