...
namespace KyoPro {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            var raisedBitIdxesInN = new List<int>();
            for (int i = 0; i < 64; i++)
            {
                if ((N & (1L << i)) != 0)
                {
                    raisedBitIdxesInN.Add(i);
                }
            }
            
            if(raisedBitIdxesInN.Count == 0)
            {
                Console.WriteLine(0);
                return;
            }

            var subMasks = new List<long>{ 0, N };
            for (int k = 0; k < raisedBitIdxesInN.Count; k++)
            {
                var patterns = Combination.Enumerate(raisedBitIdxesInN, k, withRepetition:false).ToArray();
            ...
            subMasks.Sort();
            subMasks.ForEach(Console.WriteLine);
        }
