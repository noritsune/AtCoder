...
namespace util {
    ...
    public class Sol{
        ...
        public void Solve(){
            ...
            List<int[]> sortableABs = new List<int[]>(ABs);

            // 行方向で自身より少ないものの個数を数える
            sortableABs.Sort((a, b) => a[0] - b[0]);
            List<int> rowSet = new List<int>();
            foreach (var AB in sortableABs) rowSet.Add(AB[0]);
            rowSet = rowSet.Distinct().ToList();

            int rowCnt = 0;
            Dictionary<int, int> rowCnts = new Dictionary<int, int>();
            foreach (var rowIndex in rowSet)
            {
                rowCnt++;
                rowCnts[rowIndex] = rowCnt;
            }

            // 列方向で自身より少ないものの個数を数える
            sortableABs.Sort((a, b) => a[1] - b[1]);
            List<int> colSet = new List<int>();
            foreach (var AB in sortableABs) colSet.Add(AB[1]);
            colSet = colSet.Distinct().ToList();

            int colCnt = 0;
            Dictionary<int, int> colCnts = new Dictionary<int, int>();
            foreach (var colIndex in colSet)
            {
                colCnt++;
                colCnts[colIndex] = colCnt;
            }
            ...
            // foreach (var AB in ABs)
            // {
            //     int row = AB[0] - (H - rowSet.Count());
            //     int col = AB[1] - (W - colSet.Count());
            //     Console.WriteLine(row + " " + col);
            // }
