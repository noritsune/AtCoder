...
namespace util {
    ...
    public class Sol{
        ...
        public void Solve(){
            int N = ri();
            Graph<int> graph = new Graph<int>(Graph<int>.Type.UndirectedGraph);
            for (int i = 1; i <= N; i++) graph.AddVertex(i);

            for (int i = 0; i < N - 1; i++)
            {
                int[] AB = ria();
                graph.AddEdge(AB[0], AB[1]);
            }

            Dictionary<int, List<int>> nextCityLists = new Dictionary<int, List<int>>();
            for (int i = 1; i < N + 1; i++)
            {
                nextCityLists[i] = graph.Vertices[i].ToList();
                nextCityLists[i].Sort();
            }

            Stack<int> stack = new Stack<int>();
            int[] nextCityIndexes = generateNums(0, N + 1);
            stack.Push(1);
            while(stack.Count() > 0) {
                int curretCity = stack.Pop();
                Console.WriteLine(curretCity);

                int nextCity = nextCityLists[curretCity][nextCityIndexes[curretCity]];
                stack.Push(nextCity);
            }
        }
        ...
        public void SolveTest(){
            ...
            int currentCity = 1;
            int[] nextCityIndexes = generateNums(0, N + 1);
            int[] lastCitys = generateNums(-1, N + 1);
            bool[] isVisited = new bool[N + 1];
            do {
                Console.WriteLine(currentCity);
                
                var nextCityList = nextCityLists[currentCity];
                int nextCityIndex = nextCityIndexes[currentCity];
                if(nextCityIndex < nextCityList.Count()) {
                    nextCityIndexes[currentCity]++;

                    int nextCity = nextCityList[nextCityIndex];

                    if(isVisited[nextCity]) {
                        if(nextCityIndexes[currentCity] < nextCityList.Count()) {
                            nextCity = nextCityList[nextCityIndexes[currentCity]];
                        } else {
                            nextCity = lastCitys[currentCity];
                        }
                    }

                    if(lastCitys[nextCity] < 0) {
                        lastCitys[nextCity] = currentCity;
                    }
                    isVisited[currentCity] = true;
                    currentCity = nextCity;
                } else {
                    currentCity = lastCitys[currentCity];
                }
            } while(currentCity != 1 || nextCityIndexes[currentCity] < nextCityLists[currentCity].Count());
