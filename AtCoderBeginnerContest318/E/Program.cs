...
public class Solver {
    public void Solve()
    {
        ...
        long ans = 0;
        var numToIdxes = new Dictionary<int, List<int>>();
        for (int i = 0; i < N; i++)
        {
            var A = As[i];
            if (numToIdxes.TryGetValue(A, out var prevIdxes))
            {
                long dist = i - prevIdxes.First() - 1;
                long sameNumCnt = prevIdxes.Count - 1;
                ans += dist - sameNumCnt;
            }

            numToIdxes.TryAdd(A, new List<int>());
            numToIdxes[A].Add(i);
        }
