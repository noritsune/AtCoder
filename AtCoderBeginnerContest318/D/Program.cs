...
public class Solver
{
    ...
    int[] _eToV1;
    int[] _eToV2;
    Dictionary<string, long> _memo = new Dictionary<string, long>();

    public void Solve() {
        _N = Ri();
        _eToWeight = new long[(_N + 1) * (_N + 1)];
        _eToV1 = new int[(_N + 1) * (_N + 1)];
        _eToV2 = new int[(_N + 1) * (_N + 1)];
        var es = new List<int>();
        for (int v1 = 1; v1 < _N; v1++)
        {
            var Ds = Ria();
            for (int j = 0; j < _N - v1; j++)
            {
                var v2 = v1 + j + 1;
                var e1 = v1 * _N + v2;
                var e2 = v2 * _N + v1;
                ...
                _eToWeight[e1] = Ds[j];
                _eToWeight[e2] = Ds[j];
                _eToV1[e1] = v1; _eToV1[e2] = v2;
                _eToV2[e1] = v2; _eToV2[e2] = v1;
            }
        }

        var sortedEs = es.OrderByDescending(e => _eToWeight[e]).ToList();

        long ans = Dfs(0, sortedEs, new HashSet<int>());
        Console.WriteLine(ans);
    }

    long Dfs(long currentWeightSum, List<int> remainedEs, HashSet<int> usedVs)
    {
        // remainedVsの[各要素]ビット目を立てた数値を作る
        var memoKey = string.Join(",", remainedEs);
        if (_memo.TryGetValue(memoKey, out var memoVal)) return memoVal;
        _memo[memoKey] = currentWeightSum;

        if (usedVs.Count >= _N - 1) return currentWeightSum;

        long maxWeightSum = 0;
        for (var i = 0; i < remainedEs.Count; i++)
        {
            var e = remainedEs[i];
            var v1 = _eToV1[e];
            var v2 = _eToV2[e];
            if (usedVs.Contains(v1) || usedVs.Contains(v2)) continue;

            remainedEs.Remove(e);
            usedVs.Add(v1); usedVs.Add(v2);

            var nextWeightSum = Dfs(currentWeightSum + _eToWeight[e], remainedEs, usedVs);
            maxWeightSum = Math.Max(maxWeightSum, nextWeightSum);

            remainedEs.Add(e);
            usedVs.Remove(v1); usedVs.Remove(v2);
        }
