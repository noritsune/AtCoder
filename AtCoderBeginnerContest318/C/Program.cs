...
public class Solver {
    public void Solve() {
        var NDP = Rla();
        var N = NDP[0]; int D = (int)NDP[1]; var P = NDP[2];
        var Fs = Rla();

        var descSortedFs = Fs.OrderByDescending(x => x).ToArray();
        long ans = 0;
        bool isPassNeeded = true;
        for (int d = 0; d < N; d++)
        {
            if (isPassNeeded && descSortedFs.Skip(d).Take(D).Sum() >= P) {
                ans += P;
                d += D - 1;
            }
            else
