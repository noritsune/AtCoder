...
public class Solver {
    public void Solve() {
        ...
        var gridA = new List<char[]>();
        for (int h = 0; h < H; h++) gridA.Add(Rs().ToCharArray());
        var gridB = new List<char[]>();
        for (int h = 0; h < H; h++) gridB.Add(Rs().ToCharArray());

        for (int s = 0; s < H; s++)
        {
            var gridAShiftedV = ShiftGridV(gridA, s);

            for (int t = 0; t < W; t++)
            {
                var gridAShiftedVh = ShiftGridH(gridAShiftedV, t);
                if (!IsSameGrid(gridAShiftedVh, gridB)) continue;
    ...
    List<char[]> ShiftGridV(List<char[]> sourceGrid, int shiftCnt)
    {
        var grid = new List<char[]>();
        for (int i = 0; i < sourceGrid.Count; i++)
        {
            grid.Add(new char[sourceGrid[i].Length]);
            for (int j = 0; j < sourceGrid[i].Length; j++)
            {
                grid[i][j] = sourceGrid[(i + shiftCnt) % sourceGrid.Count][j];
            }
        }
        return grid;
    }

    List<char[]> ShiftGridH(List<char[]> sourceGrid, int shiftCnt)
    {
        var grid = new List<char[]>();
        for (int i = 0; i < sourceGrid.Count; i++)
        {
            ...
            for (int j = 0; j < sourceGrid[i].Length; j++)
            {
                grid[i][j] = sourceGrid[i][(j + shiftCnt) % sourceGrid[i].Length];
            }
        }
        return grid;
    }

    bool IsSameGrid(List<char[]> gridA, List<char[]> gridB)
    {
    ...
    // 2つのグリッドのそれぞれの行及び列の#の数が同じならOK
    bool IsOk(List<char[]> gridA, List<char[]> gridB)
    {
        var gridAHashCnt = new int[gridA.Count];
        var gridBHashCnt = new int[gridB.Count];
        var gridAVerticalHashCnt = new int[gridA[0].Length];
        var gridBVerticalHashCnt = new int[gridB[0].Length];
        for (int i = 0; i < gridA.Count; i++)
        {
            for (int j = 0; j < gridA[i].Length; j++)
            {
                ...
                {
                    gridAHashCnt[i]++;
                    gridAVerticalHashCnt[j]++;
                }

                if (gridB[i][j] == '#')
                {
                    gridBHashCnt[i]++;
                    gridBVerticalHashCnt[j]++;
                }
            }
        }

        var gridAHashCntSorted = gridAHashCnt.OrderBy(x => x).ToArray();
        var gridBHashCntSorted = gridBHashCnt.OrderBy(x => x).ToArray();
        var gridAVerticalHashCntSorted = gridAVerticalHashCnt.OrderBy(x => x).ToArray();
        var gridBVerticalHashCntSorted = gridBVerticalHashCnt.OrderBy(x => x).ToArray();

        if (!gridAHashCntSorted.SequenceEqual(gridBHashCntSorted)) return false;
        if (!gridAVerticalHashCntSorted.SequenceEqual(gridBVerticalHashCntSorted)) return false;
