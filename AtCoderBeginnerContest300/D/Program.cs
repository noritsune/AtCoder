...
public class Solver {
    public void Solve() {
        ...
        var primeMax = (long)Math.Ceiling(Math.Pow(Math.Ceiling(N / 12f), 1 / 2f));
        var primes = FindPrimes(primeMax);

        long ans = 0;
        for (int aIdx = 0; aIdx < primes.Count; aIdx++)
        {
            var a = primes[aIdx];
            var aa = a * a;
            if (aa > N) break;

            for (int bIdx = aIdx + 1; bIdx < primes.Count; bIdx++)
            {
                var b = primes[bIdx];
                var aab = aa * b;
                if (aab > N) break;

                for (int cIdx = bIdx + 1; cIdx < primes.Count; cIdx++)
                {
                    var c = primes[cIdx];
                    var aabcc = aab * c * c;
    ...
    // max以下の素数を列挙する
    public List<long> FindPrimes(long max)
    {
