...
namespace KyoPro {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            var gridBefore = new List<char[]>();
            for (int i = 0; i < R; i++)
            {
                gridBefore.Add(Rs().ToCharArray());
            }

            // copy
            var gridAfter = new List<char[]>();
            for (int i = 0; i < R; i++)
            {
                gridAfter.Add(new char[C]);
                for (int j = 0; j < C; j++)
                {
            ...
            for (int i = 0; i < R; i++)
            {
                for (int j = 0; j < C; j++)
                {
                    if(gridBefore[i][j] == '#' || gridBefore[i][j] == '.') continue;

                    var pow = gridBefore[i][j] - '0';
                    for (int yOffset = -pow; yOffset <= pow; yOffset++)
                    {
                        for (int xOffset = -pow; xOffset <= pow; xOffset++)
                        {
                            if (Math.Abs(yOffset) + Math.Abs(xOffset) > pow) continue;
                            if (i + yOffset < 0 || i + yOffset >= R) continue;
                            if (j + xOffset < 0 || j + xOffset >= C) continue;

                            gridAfter[i + yOffset][j + xOffset] = '.';
                        }
                    }
                }
            }
            ...
            for (int i = 0; i < R; i++)
            {
                Console.WriteLine(new string(gridAfter[i]));
            }
        }
