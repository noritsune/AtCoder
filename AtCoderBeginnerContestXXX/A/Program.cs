...
namespace KyoPro {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            var nums = new List<(int n, int x, int y)>();
            for (int i = 1; i <= N; i++)
            {
                var xy = Ria();
                var x = xy[0];
            ...
            var sortedXs = xs.OrderBy(x => x).ToArray();
            var sortedYs = ys.OrderBy(y => y).ToArray();

            var rawXToNewX = new Dictionary<int, int>();
            var rawYToNewY = new Dictionary<int, int>();
            for (int i = 0; i < xs.Count; i++)
            {
                rawXToNewX.Add(sortedXs[i], i);
            }
            for (int i = 0; i < ys.Count; i++)
            {
                rawYToNewY.Add(sortedYs[i], i);
            }

            foreach ((int n, int x, int y) in nums)
            {
                var newX = rawXToNewX[x] + 1;
                var newY = rawYToNewY[y] + 1;
