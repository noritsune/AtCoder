...
public class Solver {
    public void Solve() {
        ...
        var idxToScores = new long[N];
        for (int i = 0; i < N; i++)
        {
            var S = Ss[i];
            var score = S.Select((c, i) => c == 'o' ? As[i] : 0).Sum();
            score += i + 1;
            idxToScores[i] = score;
        }
        var maxScore = idxToScores.Max();

        var idxAndAsSorted = As
            .Select((A, i) => new { A, i })
        ...
        for (int i = 0; i < N; i++)
        {
            var score = idxToScores[i];
            var remainedScore = maxScore - score;
            var S = Ss[i];
            var ans = 0;
            foreach (var tuple in idxAndAsSorted)
            {
                if (remainedScore <= 0) break;

                var idx = tuple.i; var A = tuple.A;
                if (S[idx] == 'o') continue;
