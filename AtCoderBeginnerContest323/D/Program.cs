...
public class Solver {
    public void Solve() {
        var N = Ri();
        var sizeToCnt = new Dictionary<long, long>();
        for (int i = 0; i < N; i++)
        {
            var SC = Rla();
            var S = SC[0]; var C = SC[1];
            sizeToCnt.Add(S, C);
        }

        var ascSortedSizes = new PriorityQueue<long>(false);
        foreach (var size in sizeToCnt.Keys)
        {
            ascSortedSizes.Enqueue(size);
        }

        while (ascSortedSizes.Any())
        {
            var size = ascSortedSizes.Dequeue();
            var cnt = sizeToCnt[size];

            sizeToCnt[size] = cnt % 2;

            if (cnt >= 2)
            {
                var mergedSize = size * 2;
                sizeToCnt.TryAdd(mergedSize, 0);
                sizeToCnt[mergedSize] += cnt / 2;

                ascSortedSizes.Enqueue(mergedSize);
            }
        }

        Console.WriteLine(sizeToCnt.Values.Sum());
    }
