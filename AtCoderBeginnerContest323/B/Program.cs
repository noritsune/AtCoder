...
public class Solver {
    public void Solve()
    {
        ...
        var idxAndWinCnt = new List<(int idx, int winCnt)>();
        for (int i = 0; i < N; i++)
        {
            var S = Rs();
            var winCnt = S.Count(c => c == 'o');
            idxAndWinCnt.Add((i, winCnt));
        }

        var ans = idxAndWinCnt
            .OrderByDescending(x => x.winCnt)
            .ThenBy(x => x.idx);

        Console.WriteLine(string.Join(" ", ans.Select(x => x.idx + 1)));
    }
