...
namespace util {
    ...
    public class Sol{
        ...
        public void Solve()
        {
            ...
            long numA = ParseTo10Base(K, strA);
            long numB = ParseTo10Base(K, strB);
            
            Console.WriteLine(numA * numB);
            Console.ReadLine();
        }

        private long ParseTo10Base(int base_, string strN)
        {
            long num = 0;
            for (int i = 0; i < strN.Length; i++)
            {
                num += (long)(char.GetNumericValue(strN[strN.Length - 1 - i]) * Math.Pow(base_, i));
            }
