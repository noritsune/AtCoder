...
namespace AtCoder {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            var psBeforeK = Ps.Take(K).ToList();
            psBeforeK.Sort();
            
            var hasAppeared = new bool[N + 1];
            psBeforeK.ForEach(p => hasAppeared[p] = true);
            
            var ans = psBeforeK.First();
            Console.WriteLine(ans);
            for (int i = K; i < N; i++)
            {
                hasAppeared[Ps[i]] = true;
        ...
        public void SolveLinkedList()
        {
            ...
            var psAtFirst = Ps.Take(K - 1).ToList();
            psAtFirst.Sort();

            var ps = new LinkedList<int>();
            var nodes = new LinkedListNode<int>[N + 1];
            foreach (var p in psAtFirst)
            {
                nodes[p] = ps.AddLast(p);
            }

            for (int i = K - 1; i < N; i++)
            {
                int insertIndex = LowerBoundUnder(ps, Ps[i], Comparer<int>.Default);

                if (insertIndex >= 0)
                {
                    var beforeNum = ps.ElementAt(insertIndex);
                    var beforeNode = nodes[beforeNum];
                    nodes[Ps[i]] =  ps.AddAfter(beforeNode, Ps[i]);
                }
                else
                {
                    ps.AddFirst(Ps[i]);
                ...
                Console.WriteLine(ps.ElementAt(ps.Count - K));
            }
        }
