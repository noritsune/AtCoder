    ...
    public class Solver {
        public void Solve()
        {
            ...
            for (int startNum = 1; startNum <= 9; startNum++)
            {
                ...
                {
                    string numStr = "";
                    for (int keta = 0; keta < 17; keta++)
                    {
                        int num = startNum + diff * keta;
            ...
            nums.Add(111111111111111111);
            
            var numsList = nums.ToList();
            numsList.Sort();

            var ansIndex = LowerBound(numsList, long.Parse(X), Comparer<long>.Default);
            Console.WriteLine(numsList[ansIndex]);
        }
