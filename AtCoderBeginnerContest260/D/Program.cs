...
namespace KyoPro {
    ...
    public class Solver {
        ...
        public void Solve1()
        {
            ...
            var endTurns = Enumerable.Repeat(-1, N + 1).ToArray();
            var decks = new List<List<int>>();
            for (int turn = 1; turn <= N; turn++)
            {
                var P = Ps[turn - 1];
                
                var mountDeckIdx = -1;
                for (int deckIdx = 0; deckIdx < decks.Count; deckIdx++)
                {
                    var deck = decks[deckIdx];
                    if(deck.Count == K || deck.Last() < P) continue;

                    if (mountDeckIdx <= -1 || deck.Last() < decks[mountDeckIdx].Last())
                    {
                        mountDeckIdx = deckIdx;
                    }
                }
                ...
                if (mountDeckIdx == -1)
                {
                    ...
                    decks.Add(newDeck);
                    mountDeckIdx = decks.Count - 1;
                }

                var mountedDeck = decks[mountDeckIdx];
                mountedDeck.Add(P);
                
                if (mountedDeck.Count == K)
                {
                    foreach (var endNum in mountedDeck) endTurns[endNum] = turn;
                }
            }
