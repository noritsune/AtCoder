...
namespace KyoPro {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            var passNums = new List<int>();
            var isPass = new bool[N];

            nums.Sort((a, b) =>
            {
                return mathScores[a] != mathScores[b]
                    ? mathScores[b] - mathScores[a]
                    : a - b;
            });
            var passNumsByMath = nums.Take(X).ToList();
            passNumsByMath.ForEach(n =>
                {
                    isPass[n] = true;
                    passNums.Add(n);
                });
            ...
            nums.Sort((a, b) =>
            {
                return engScores[a] != engScores[b]
                    ? engScores[b] - engScores[a]
                    : a - b;
            });
            var passNumsByEng = nums.Where(n => !isPass[n]).Take(Y).ToList();
            passNumsByEng.ForEach(n =>
                {
                    isPass[n] = true;
                    passNums.Add(n);
                });
            ...
            nums.Sort((a, b) =>
            {
                var scoreA = mathScores[a] + engScores[a];
                var scoreB = mathScores[b] + engScores[b];
                return scoreA != scoreB
                    ? scoreB - scoreA
                    : a - b;
            });
            var passNumsByEngAndMath = nums.Where(n => !isPass[n]).Take(Z).ToList();
            passNumsByEngAndMath.ForEach(n =>
            {
                isPass[n] = true;
                passNums.Add(n);
            });
            ...
            passNums.Sort();
            passNums.ForEach(n => Console.WriteLine(n + 1));
        }
