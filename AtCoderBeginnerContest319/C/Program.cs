...
public class Solver {
    public void Solve() {
        ...
        var c21c22c23 = Ria();
        var c31c32c33 = Ria();
        var cs = new int[][] { c11c12c13, c21c22c23, c31c32c33 };

        var perms = Enumerable.Range(0, 9).Perm(9).Select(x => x.ToArray()).ToArray();
        long okCnt = 0;
        foreach (var perm in perms)
        {
            var rowToNums = new List<int>[3];
            for (int i = 0; i < 3; i++) rowToNums[i] = new List<int>();
            var colToNums = new List<int>[3];
            for (int i = 0; i < 3; i++) colToNums[i] = new List<int>();
            // 斜めも
            var rightDownNums = new List<int>();
            var rightUpNums = new List<int>();
            ...
            var isOk = perm.All(i => {
                ...
                var rowNums = rowToNums[row];
                if (rowNums.Count == 1 && c == rowNums[0]) return false;
                rowNums.Add(c);

                var colNums = colToNums[col];
                if (colNums.Count == 1 && c == colNums[0]) return false;
                colNums.Add(c);

                if (row == col)
                {
                    if (rightDownNums.Count == 1 && c == rightDownNums[0]) return false;
                    rightDownNums.Add(c);
                }

                if (row + col == 2)
                {
                    if (rightUpNums.Count == 1 && c == rightUpNums[0]) return false;
                    rightUpNums.Add(c);
                }
        ...
        Console.WriteLine(okCnt / (decimal)perms.Length);
    }
