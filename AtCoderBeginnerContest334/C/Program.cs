...
public class Solver {
    public void Solve()
    {
        ...
        var As = Ria().OrderBy(A => A).ToArray();

        var dists = new List<(int dist, int a, int b)>();
        for(int i = 0; i < K - 1; i++)
        {
            dists.Add((As[i + 1] - As[i], i, i + 1));
        }

        dists = dists.OrderBy(d => d.dist).ToList();

        var remainedSet = As.ToHashSet();
        var idx = 0;
        long ans = 0;
        while (idx < dists.Count && remainedSet.Count >= 2)
        {
            var (dist, a, b) = dists[idx];
            if (remainedSet.Contains(As[a]) && remainedSet.Contains(As[b]))
            {
                ans += dist;
                remainedSet.Remove(a);
                remainedSet.Remove(b);
            }
            idx++;
        }
