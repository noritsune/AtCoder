...
public class Solver {
    public void Solve()
    {
        ...
        graph.InitFromStdin(N, N - 1);

        var starLevels = new List<int>();
        var d1Vert = graph.Vertices.First(kv => kv.Value.Count == 1).Key;
        graph.AddVertex(0, new []{d1Vert});
        ...
        while (q.Count > 0)
        {
            var lastEdgeVert = q.Dequeue();
            var edgeVert = graph.Vertices[lastEdgeVert].First(v => !visited[v]);
            visited[edgeVert] = true;
            var centerVert = graph.Vertices[edgeVert].First(v => !visited[v]);
            visited[centerVert] = true;
            var nextVerts = graph.Vertices[centerVert];
            starLevels.Add(nextVerts.Count);
            ...
            foreach (var nextVert in nextVerts)
            {
                ...
                if (graph.Vertices[nextVert].Count == 1) continue;
        ...
        starLevels.Sort();
        Console.WriteLine(string.Join(" ", starLevels));
    }
