...
public class Solver {
    ...
    {
        var NMHK = Ria();
        var N = NMHK[0]; var M = NMHK[1]; var H = NMHK[2]; var K = NMHK[3];
        var S = Rs();
        var healItemPosSet = new HashSet<string>();
        for (int i = 0; i < M; i++)
        {
            var xy = Ria();
            healItemPosSet.Add($"{xy[0]},{xy[1]}");
        }
        ...
        foreach (var s in S)
        {
            ...
            if (healItemPosSet.Contains(posStr) && hp < K)
            {
                hp = K;
                healItemPosSet.Remove(posStr);
            }
        }
