...
public class Solver {
    public void Solve()
    {
        ...
        // Sのi番目(1始まり)をCapsLock無しで打つのがi, 有りで打つのがi + 1に対応する。0と1は初期状態
        var dijkstra = new Dijkstra((S.Length + 1) * 2);
        ...
        for (var i = 1; i <= S.Length; i++)
        {
            ...
            if (S[i - 1] == 'a')
            {
                dijkstra.Add(prev, cur, X);
                dijkstra.Add(prev, curL, Z + Y);
                dijkstra.Add(prevL, cur, Z + X);
                dijkstra.Add(prevL, curL, Y);
            }
            else
            {
                dijkstra.Add(prev, cur, Y);
                dijkstra.Add(prev, curL, Z + X);
                dijkstra.Add(prevL, cur, Z + Y);
                dijkstra.Add(prevL, curL, X);
            }
        }

        var costs0 = dijkstra.GetMinCost(0);
        var costs1 = dijkstra.GetMinCost(1);

        var minCost0 =     Math.Min(costs0[S.Length * 2], costs0[S.Length * 2 + 1]);
        var minCost1 = Z + Math.Min(costs1[S.Length * 2], costs1[S.Length * 2 + 1]);

        var minCost = Math.Min(minCost0, minCost1);
