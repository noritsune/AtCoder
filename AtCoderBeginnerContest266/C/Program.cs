...
namespace AtCoder {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            var remainedPrices = new List<long>(As);
            var remainedCouponCnt = K;
            for (int i = 0; i < N && remainedCouponCnt > 0; i++)
            {
                var couponUseCnt = Math.Min(remainedCouponCnt, remainedPrices[i] / X);
                remainedPrices[i] = Math.Max(As[i] - couponUseCnt * X, 0);
                
                remainedCouponCnt -= couponUseCnt;
            }
            
            remainedPrices.Sort();
            remainedPrices.Reverse();
            ...
            Console.WriteLine(remainedPrices.Sum());
        }
