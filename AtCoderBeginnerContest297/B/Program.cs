...
namespace KyoPro {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            {
                if (!cToIdxes.ContainsKey(S[i]))
                {
                    cToIdxes[S[i]] = new List<int>();
                }
                cToIdxes[S[i]].Add(i);
            }

            var isBsOddAndEven = isOdd(cToIdxes['B'][0]) != isOdd(cToIdxes['B'][1]);
            var isKBetweenRs = cToIdxes['R'][0] < cToIdxes['K'][0] && cToIdxes['K'][0] < cToIdxes['R'][1];

            Console.WriteLine(isBsOddAndEven && isKBetweenRs ? "Yes" : "No");
        }
