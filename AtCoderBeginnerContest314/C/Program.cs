...
public class Solver {
    public void Solve() {
        ...
        var colorToChars = new List<char>[M + 1];
        var sIdxToColorIdx = new int[N];
        for(int i = 0; i <= M; i++) colorToChars[i] = new List<char>();
        for (int i = 0; i < N; i++)
        {
            colorToChars[Cs[i]].Add(S[i]);
            sIdxToColorIdx[i] = colorToChars[Cs[i]].Count - 1;
        }

        var ansBuilder = new StringBuilder();
        for (int i = 0; i < N; i++)
        {
            var sameColorChars = colorToChars[Cs[i]];
            var colorIdx = sIdxToColorIdx[i];
            var shiftedColorIdx = (colorIdx - 1 + sameColorChars.Count) % sameColorChars.Count;
            var shiftedChar = sameColorChars[shiftedColorIdx];
            ansBuilder.Append(shiftedChar);
        }
