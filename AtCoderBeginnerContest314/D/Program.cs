...
public class Solver {
    public void Solve()
    {
        ...
        var queries = new List<(int t, int x, char c)>();
        var lastTransQIdx = -1;
        var lastTransT = -1;
        for (int i = 0; i < Q; i++)
        {
            var txc = Rsa();
            var t = int.Parse(txc[0]); var x = int.Parse(txc[1]); var c = txc[2][0];
            queries.Add((t, x, c));
        ...
        var ans = S
            .Select(c => lastTransT == -1 ? c : lastTransT == 2 ? char.ToLower(c) : char.ToUpper(c))
            .ToArray();
        for (int i = 0; i < Q; i++)
        {
            var (t, x, c) = queries[i];
            ...
            var addC = queries[i].c;
            if (i < lastTransQIdx)
            {
                // lastTransTが2なら小文字、3なら大文字にする
                addC = lastTransT == 2 ? char.ToLower(addC) : char.ToUpper(addC);
            }
            ans[x - 1] = addC;
        }
