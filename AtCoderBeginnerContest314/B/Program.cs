...
public class Solver {
    public void Solve()
    {
        ...
        var players = new List<(int idx, HashSet<int> betNums)>();
        for(int i = 1; i <= N; i++)
        {
            var _ = Ri();
            var As = Ria();
            players.Add((i, As.ToHashSet()));
        }
        var X = Ri();

        var winPlayer = players
            .Where(p => p.betNums.Contains(X));
        if (!winPlayer.Any())
        {
            Console.WriteLine(0);
            Console.WriteLine();
        ...
        var minBetCnt = winPlayer
            .Select(p => p.betNums.Count)
            .Min();
        var ansPlayerIdxes = winPlayer
            .Where(p => p.betNums.Count == minBetCnt)
            .Select(p => p.idx)
            .OrderBy(i => i)
            .ToArray();
        Console.WriteLine(ansPlayerIdxes.Length);
        Console.WriteLine(string.Join(" ", ansPlayerIdxes));
    }
