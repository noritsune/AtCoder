...
public class Solver {
    public void Solve()
    {
        ...
        var roulettes = new List<(decimal C, int P, int[] Ss)>();
        for (int i = 0; i < N; i++)
        {
            var CPSs = Ria();
            var C = CPSs[0]; var P = CPSs[1];
            roulettes.Add((C, P, CPSs.Skip(2).OrderBy(S => S).ToArray()));
        }

        // i個目までのルーレットを見たときにポイントがjになるためのコストの和の期待値
        var dp = new decimal[N + 1, M + 1];
        // 全部-1で初期化
        for (int i = 0; i < N + 1; i++)
        {
            for (int j = 0; j < M + 1; j++)
        ...
        for (int i = 0; i < N; i++)
        {
            ...
            for (int j = 0; j < M; j++)
            {
                ...
                foreach (var S in Ss)
                {
                    var nextPoint = Math.Min(M, j + S);
                    dp[i + 1, nextPoint] = dp[i, j] + C / P;
                }
            }
        }
