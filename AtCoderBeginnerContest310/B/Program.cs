...
public class Solver {
    public void Solve()
    {
        ...
        var merchs = new List<(int P, int C, HashSet<int> Fs)>();
        for (int i = 0; i < N; i++)
        {
            var PCFs = Ria();
            var P = PCFs[0]; var C = PCFs[1]; var F = PCFs.Skip(2).ToHashSet();
            merchs.Add((P, C, new HashSet<int>(F)));
        }
        ...
        for (int i = 0; i < N; i++)
        {
            for (int j = 0; j < N; j++)
            {
                ...
                var mJ = merchs[j];
                if (mI.P >= mJ.P
                && mI.Fs.IsSubsetOf(mJ.Fs)
                && (mI.P > mJ.P || mI.Fs.Count < mJ.Fs.Count)
                )
                {
