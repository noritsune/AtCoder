...
public class Solver
{
    ...
    {
        var NTM = Ria();
        _N = NTM[0]; _T = NTM[1]; var M = NTM[2];
        _graph = new IntGraph(GraphType.Undirected);
        _graph.InitFromStdin(_N, M);

        Console.WriteLine(Dfs(0, 1, new List<HashSet<int>>()));
    }

    long Dfs(int ans, int nextMan, List<HashSet<int>> teams)
    {
        if (nextMan > _N)
        {
            return teams.Count == _T ? ans + 1 : ans;
        }

        var hateMembers = _graph.Vertices[nextMan];
        long ansSum = 0;
        for (int i = 0; i < teams.Count; i++)
        {
            var team = teams[i];
            if (team.Overlaps(hateMembers)) continue;
            team.Add(nextMan);
            ansSum += Dfs(ans, nextMan + 1, teams);
            team.Remove(nextMan);
        }

        if (teams.Count < _T)
        {
            teams.Add(new HashSet<int>() { nextMan });
            ansSum += Dfs(ans, nextMan + 1, teams);
