...
namespace util {
    ...
    public class Sol{
        ...
        public void Solve()
        {
            ...
            var patterns = Enumerable.Range(0, S.Length).Perm().Select(x => x.ToArray()).ToArray();
            for (int i = 0; i < patterns.Length; i++)
            {
                string str = new string(patterns[i].Select(index => SArray[index]).ToArray());
                strList.Add(str);
            }

            var distinctStrList = strList.Distinct().ToArray();

            Console.WriteLine(distinctStrList[K - 1]);
            Console.ReadLine();
        }
