...
namespace AtCoder {
    ...
    public class Solver {
        ...
        {
            var x1y1x2y2 = ria();
            var pos1 = new Vector2(x1y1x2y2[0], x1y1x2y2[1]);
            var pos2 = new Vector2(x1y1x2y2[2], x1y1x2y2[3]);
            
            var distRout5PossFromPos1 = FindDistRout5Poss(pos1);
            var distRout5PossFromPos2 = FindDistRout5Poss(pos2);
            
            for (int i = 0; i < 8; i++)
            {
                for (int j = 0; j < 8; j++)
                {
                    if(!distRout5PossFromPos1[i].Equals(distRout5PossFromPos2[j])) continue;
        ...
        Vector2[] FindDistRout5Poss(Vector2 pos)
        {
            return new[]
            {
