...
namespace AtCoder {
    ...
    public class Solver {
        public void Solve2()
        {
            ...
            var tree = new Graph<int>(Graph<int>.Type.DirectedGraph);
            for (int i = 1; i <= N; i++) tree.AddVertex(i);
            foreach (var AB in ABs)
            {
                int parent = Math.Min(AB.A, AB.B);
                int child  = Math.Max(AB.A, AB.B);
                tree.AddEdge(parent, child);
            }

            var vertNumOrders = new List<int>[N + 1];
            for (int i = 1; i <= N; i++)
            {
                var vertNums = new PriorityQueue<int>();
                var q = new Queue<int>();
                q.Enqueue(i);
                while (q.Count > 0)
                {
                    var vert = q.Dequeue();
                    vertNums.Enqueue(Xs[vert - 1]);
                    foreach (var nextVert in tree.Vertices[vert])
                    {
                        q.Enqueue(nextVert);
                    }
                }

                vertNumOrders[i] = new List<int>();
                for (int j = 0; j < 20; j++)
                {
                    if(vertNums.Count == 0) break;
                    vertNumOrders[i].Add(vertNums.Dequeue());
                }
            }
            ...
            for (int i = 0; i < Q; i++)
            {
                ...
                Console.WriteLine(vertNumOrders[V][K - 1]);
            }
        }
        ...
        public void Solve1()
        {
            ...
            for (int i = 0; i < Q; i++)
            {
                ...
                Console.WriteLine(vertNums.Dequeue());
            }
        }
