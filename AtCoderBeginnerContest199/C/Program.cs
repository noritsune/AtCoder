...
class Sol{
    ...
    public void Solve() {
        ...
        bool isFlip = false;
        int[] swapedIndexes = generateSuretsu(2 * N);
        foreach (int[] TAB in TABs)
        {
            ...
            if(T == 1) {
                if(isFlip) {
                    A = convertToFlipedIndex(N, A);
                    B = convertToFlipedIndex(N, B);
                }

                int tmp = swapedIndexes[A - 1];
                swapedIndexes[A - 1] = swapedIndexes[B - 1];
                swapedIndexes[B - 1] = tmp;
            } else if(T == 2) {
                isFlip = !isFlip;
            }
        }
        ...
        for (int i = 0; i < 2 * N; i++)
        {
            chars[i] = S[swapedIndexes[i]];
        }

        string ans = new string(chars);
        if(isFlip) {
            ans = flipEachHarf(ans);
        }
    ...
    int convertToFlipedIndex(int N, int index) {
        if(index <= N) {
            return N + index;
        } else {
            return index - N;
    ...
    string getQueryResult(string str, int T, int A, int B) {
        switch(T) {
            case 1:
                return swapCharAt(str, A, B);
            case 2:
                return flipEachHarf(str);
            default:
                throw new Exception("Tが不正です。T: " + T);
        }
    }

    string swapCharAt(string str, int A, int B) {
    ...
    string flipEachHarf(string str) {
        string beforeHalf   = str.Substring(0, str.Length / 2);
        string afterHalf    = str.Substring(str.Length / 2, str.Length / 2);
        return afterHalf + beforeHalf;
    }
