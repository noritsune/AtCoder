...
namespace AtCoder {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            var poss = new[]
            {
                new Vector2(x1y1[0], x1y1[1]),
                new Vector2(x2y2[0], x2y2[1]),
                new Vector2(x3y3[0], x3y3[1]),
            };

            var xCnts = new Dictionary<int, int>();
            var yCnts = new Dictionary<int, int>();
            foreach (var pos in poss)
            {
                if(!xCnts.ContainsKey(pos.X))
                {
                    xCnts.Add(pos.X, 0);
                }
                xCnts[pos.X]++;
                
                if(!yCnts.ContainsKey(pos.Y))
                {
                    yCnts.Add(pos.Y, 0);
                }
                yCnts[pos.Y]++;
            ...
            var ansX = 0;
            foreach (var key in xCnts.Keys.Where(key => xCnts[key] == 1))
            {
                ansX = key;
            }
            
            var ansY = 0;
            foreach (var key in yCnts.Keys.Where(key => yCnts[key] == 1))
            {
                ansY = key;
            }
