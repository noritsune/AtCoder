...
namespace AtCoder {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            var N = NK[0];
            var K = NK[1];
            var Ps2dArray = new List<int[]>();
            for (int i = 0; i < N; i++) Ps2dArray.Add(ria());

            var sumScores = new int[N];
            for (int i = 0; i < N; i++)
            {
                sumScores[i] = Ps2dArray[i].Sum();
            }
            
            var sumScoresAscSorted = sumScores.OrderBy(x => x).ToArray();

            for (int i = 0; i < N; i++)
            {
                int score = sumScores[i];
                int maxRank = N - UpperBound(sumScoresAscSorted, score + 300, Comparer<int>.Default) + 1;
                Console.WriteLine(maxRank <= K ? "Yes" : "No");
            }
        }
