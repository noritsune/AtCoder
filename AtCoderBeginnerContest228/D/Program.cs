...
namespace AtCoder {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            var modifiedAs = new Dictionary<int, long>();
            var union = new UnionFind(Pow20 + 1);
            for (int i = 0; i < Q; i++)
            {
                var tx = rla();
                var t = tx[0];
                ...
                var moddedX = (int)(x % Pow20);

                switch (t)
                {
                    case 1:
                        int unionSize = union.Size(moddedX);
                        int nextVoidIndex = (moddedX + unionSize - 1) % Pow20;
                        modifiedAs.Add(nextVoidIndex, x);

                        union.Union(nextVoidIndex, (nextVoidIndex - 1) % Pow20);
                        union.Union(nextVoidIndex, (nextVoidIndex + 1) % Pow20);
                        break;
                    case 2:
                        Console.WriteLine(modifiedAs.ContainsKey(moddedX) ? modifiedAs[moddedX] : -1);
                        break;
                }
            }
        }
