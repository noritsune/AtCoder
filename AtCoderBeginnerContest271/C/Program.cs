...
namespace KyoPro {
    ...
    public class Solver {
        ...
        public void Solve1()
        {
            ...
            var currentComic = 0;
            var surplusComicCnt = 0;
            var soledIdx = As.Count;
            for (int readIdx = 0; readIdx < As.Count; readIdx++)
            {
                if(readIdx >= soledIdx) break;

                var readComic = As[readIdx];
                if (readComic == currentComic + 1)
                {
                    // 次に並んでいる巻が丁度次に読むべき巻
                    currentComic++;
                }
                else if (readComic <= currentComic)
                {
                    // 次に並んでいる巻が既に読んでいる巻
                    surplusComicCnt++;
                }
            ...
            currentComic += surplusComicCnt / 2;

            Console.WriteLine(currentComic);
        }
