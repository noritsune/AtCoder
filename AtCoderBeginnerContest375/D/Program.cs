...
public class Solver {
    public void Solve()
    {
        ...
        var cToIdxes = new Dictionary<char, (long IdxSum, long Cnt)>();
        // 全ての英大文字で初期化
        for (int i = 0; i < 26; i++)
        {
            var c = (char)('A' + i);
        ...
        {
            var c = S[i];
            var (idxSum, cnt) = cToIdxes[c];

            if (cnt > 0)
            {
                var score = cnt * (i - 1) - idxSum;
                ans += score;
            }

            if (i >= 1)
            {
                var (idxSumPrev, cntPrev) = cToIdxes[S[i - 1]];
                cToIdxes[S[i - 1]] = (idxSumPrev + i - 1, cntPrev + 1);
            }
        }
