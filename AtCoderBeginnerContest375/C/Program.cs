...
public class Solver {
    public void Solve()
    {
        ...
        var offsets = new Queue<Vector2>();
        offsets.Enqueue(new Vector2(0, 1));
        offsets.Enqueue(new Vector2(-1, 0));
        offsets.Enqueue(new Vector2(0, -1));
        offsets.Enqueue(new Vector2(1, 0));

        var newOffsets = new List<Vector2>
        {
            new Vector2(1, 0),
            new Vector2(0, 1),
            new Vector2(-1, 0),
        ...
        for (int n = 0; n < N / 2; n++)
        {
            ...
            var newY = n; var newX = n;

            for (int k = 0; k < 4; k++)
            {
                var offset = offsets.Dequeue();
                var newOffset = newOffsets[k];
