...
public class Solver {
    ...
    {
        var N = Ri();
        var ACs = new List<(int Idx, int A, int C)>();
        for (int i = 0; i < N; i++)
        {
            var AC = Ria();
            ACs.Add((i + 1, AC[0], AC[1]));
        }

        var AtoACs = ACs
            .GroupBy(AC => AC.A)
            .OrderByDescending(AC => AC.Key)
            .ToList();

        var minCAll = int.MaxValue;
        var anss = new List<int>();
        foreach (IGrouping<int,(int Idx, int A, int C)> aToACs in AtoACs)
        {
            var minC = int.MaxValue;
            foreach (var (Idx, A, C) in aToACs)
            {
                if (C <= minCAll)
                {
                    anss.Add(Idx);
