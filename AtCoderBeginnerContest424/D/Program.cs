...
public class Solver {
    public void Solve()
    {
        ...
        for (int _ = 0; _ < T; _++)
        {
            ...
            var squareXs = new List<int>();
            var squareYs = new List<int>();
            var posToSquareIdxes = new HashSet<int>[H, W];
            for (int h = 0; h < H; h++) {
                for (int w = 0; w < W; w++) {
                    posToSquareIdxes[h, w] = new HashSet<int>();
                }
            }
                ...
                for (int w = 0; w < W - 1; w++) {
                    ...
                    for (int dh = 0; dh < 2; dh++) {
                        for (int dw = 0; dw < 2; dw++) {
                            var x = w + dw; var y = h + dh;
                            isAllBlack &= grid[y][x] == '#';
                        }
                    }

                    if (!isAllBlack) continue;
                    ...
                    for (int dh = 0; dh < 2; dh++) {
                        for (int dw = 0; dw < 2; dw++) {
                            var x = w + dw; var y = h + dh;
                            posToSquareIdxes[y, x].Add(squareXs.Count - 1);
                        }
                    }
                }
            }
            ...
            {
                var squareX = squareXs[i]; var squareY = squareYs[i];
                if (!posToSquareIdxes[squareY, squareX].Contains(i)) continue;

                var maxSquareCntX = -1; var maxSquareCntY = -1;
                var maxSquareCnt = -1;
                for (int dh = 0; dh < 2; dh++) {
                    for (int dw = 0; dw < 2; dw++) {
                        var x = squareX + dw; var y = squareY + dh;

                        var squareCnt = posToSquareIdxes[y, x].Count;
                        if (maxSquareCnt < squareCnt) {
                            maxSquareCnt = squareCnt;
                            maxSquareCntX = x; maxSquareCntY = y;
                        }
                    }
                }
                ...
                var overlappingSquareIdxes = posToSquareIdxes[maxSquareCntY, maxSquareCntX];
                foreach (var idx in overlappingSquareIdxes) {
                    var overlappingSquareX = squareXs[idx]; var overlappingSquareY = squareYs[idx];

                    for (int dh = 0; dh < 2; dh++) {
                        for (int dw = 0; dw < 2; dw++) {
                            var x = overlappingSquareX + dw; var y = overlappingSquareY + dh;
                            posToSquareIdxes[y, x].Remove(idx);
                        }
                    }
                }
            }

            Console.WriteLine(makeWhiteCnt);
        }
    }
