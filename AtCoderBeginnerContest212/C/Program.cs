...
namespace util {
    ...
    public class Sol{
        ...
        public void Solve(){
            ...
            foreach (int A in As)
            {
                int closestBIndexMore = LowerBound(Bs, A);
                if(closestBIndexMore <= Bs.Length - 1) {
                    int closestB = Bs[closestBIndexMore];
                    ans = Math.Min(Math.Abs(A - closestB), ans);
                }

                int closestBIndexLess = LowerBoundUnder(Bs, A);
                if(closestBIndexLess >= 0) {
                    int closestB = Bs[closestBIndexLess];
                    ans = Math.Min(Math.Abs(A - closestB), ans);
                }
            }
