...
namespace AtCoder {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            var SLoop = Ss.ToList();
            SLoop.Insert(0, SLoop.Last());
            var TLoop = Ts.ToList();
            TLoop.Insert(0, TLoop.Last());

            var shortestTimes = new int[N + 1];
            shortestTimes[0] = Math.Min(SLoop[N - 2] + TLoop[N - 2], Ts.Last());
            for (int i = 1; i <= N; i++)
            {
                shortestTimes[i] = Math.Min(SLoop[i - 1] + shortestTimes[i - 1], TLoop[i]);
            }

            for (int i = 1; i <= N; i++)
            {
                Console.WriteLine(shortestTimes[i]);
            }
        }
