...
public class Solver {
    public void Solve()
    {
        ...
        var snakes = new List<(long Head, long Tail)>();
        var headSnakeIdx = 0;
        var removedLSum = 0L;
        ...
        for (int i = 0; i < Q; i++)
        {
            ...
            switch (query[0])
            {
                case 1:
                    var l = query[1];
                    var nextHead = snakes.Any() ? snakes[^1].Tail : 0;
                    var nextTail = nextHead + l;
                    snakes.Add((nextHead, nextTail));
                    break;
                case 2:
                    var headSnake = snakes[headSnakeIdx];
                    removedLSum += headSnake.Tail - headSnake.Head;
                    headSnakeIdx++;
                    break;
                case 3:
                    var k = (int)query[1];
                    Console.WriteLine(snakes[headSnakeIdx + k - 1].Head - removedLSum);
                    break;
            }
        }
    }
