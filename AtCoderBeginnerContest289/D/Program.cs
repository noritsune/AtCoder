    ...
    public class Solver {
        public void Solve2()
        {
            ...
            while (stack.Count > 0)
            {
                ...
                foreach (var offset in As)
                {
                    var nextIdx = idx + offset;
                    if (nextIdx > X || visited[nextIdx] || stopIdxSet.Contains(nextIdx)) continue;
            ...
            Console.WriteLine(visited[X] ? "Yes" : "No");
        }
        ...
        public void Solve1()
        {
            ...
            var prevTrueIdxes = new List<long> { 0 };
            var stopIdxSet = Bs.ToHashSet();
            for (int i = 1; i <= N; i++)
            {
                var A = As[i - 1];
                var nextTrueIdxes = prevTrueIdxes.ToList();
                foreach (var prevTrueIdx in prevTrueIdxes)
                {
                    var nextIdx = prevTrueIdx + A;
                    dp[i, nextIdx] = true;

                    if (stopIdxSet.Contains(nextIdx)) continue;

                    nextTrueIdxes.Add(nextIdx);
                }

                prevTrueIdxes = nextTrueIdxes;
            }

            Console.WriteLine(dp[N, X] ? "Yes" : "No");
        }
