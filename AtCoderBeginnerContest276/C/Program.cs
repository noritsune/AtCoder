...
namespace KyoPro {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            int sufCnt = N - notAscIdx;
            int preCnt = notAscIdx;
            var postPs = Ps.Skip(preCnt).Take(sufCnt).ToList();
            var descSortedPostPs = postPs.OrderByDescending(x => x).ToList();
            var nextPostPsFirst = descSortedPostPs[descSortedPostPs.IndexOf(postPs.First()) + 1];
            var nextPostPs = new List<int> { nextPostPsFirst };
            descSortedPostPs.Remove(nextPostPsFirst);
            nextPostPs.AddRange(descSortedPostPs);

            var Qs = Ps.Take(preCnt).ToList();
            Qs.AddRange(nextPostPs);
