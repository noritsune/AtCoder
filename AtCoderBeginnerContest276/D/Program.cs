...
namespace KyoPro {
    ...
    public class Solver {
        public void Solve2()
        {
            ...
            var acceptNumToOpeCntSum = new Dictionary<int, long>();

            var primeFactors1 = PrimeFactors(aArray.First()).ToArray();
            var prime2Cnt1 = primeFactors1.Count(p => p == 2);
            var prime3Cnt1 = primeFactors1.Count(p => p == 3);

            var acceptNumMin = aArray.First() / (int)Math.Pow(2, prime2Cnt1) / (int)Math.Pow(3, prime3Cnt1);
            for (int cnt2 = 0; cnt2 <= prime2Cnt1; cnt2++)
            {
                for (int cnt3 = 0; cnt3 <= prime3Cnt1; cnt3++)
                {
                    var acceptNum = acceptNumMin * (int)Math.Pow(2, cnt2) * (int)Math.Pow(3, cnt3);
                    acceptNumToOpeCntSum.Add(acceptNum, primeFactors1.Length - cnt2 - cnt3);
                }
            }
            ...
            for (int i = 1; i < N; i++)
            {
                ...
                var acceptNums = acceptNumToOpeCntSum.Keys.ToArray();
                foreach (var acceptNum in acceptNums)
                {
                    long opeCntSum = acceptNumToOpeCntSum[acceptNum];

                    if(opeCntSum == -1) continue;

                    var mod = a % acceptNum;
                    if (mod == 0)
                    {
                        var primeFactors = PrimeFactors(a / acceptNum).ToArray();
                        var prime2Cnt = primeFactors.Count(p => p == 2);
                        var prime3Cnt = primeFactors.Count(p => p == 3);

                        var opeCnt = prime2Cnt + prime3Cnt;
                        if (opeCnt > 0) acceptNumToOpeCntSum[acceptNum] += opeCnt;
                        else acceptNumToOpeCntSum[acceptNum] = -1;
                    }
                    else
                    {
                        acceptNumToOpeCntSum[acceptNum] = -1;
                    }
                }
            }

            var opeCntSums = acceptNumToOpeCntSum.Values
                .Where(v => v != -1)
                .ToArray();

            if(opeCntSums.Length == 0) Console.WriteLine(-1);
            else Console.WriteLine(opeCntSums.Min());
        }
        ...
        public void Solve1()
        {
            ...
            var acceptNumMin = int.MinValue;
            var acceptNumMax = int.MaxValue;

            for (int i = 0; i < N; i++)
            {
                var a = aArray[i];
                var primeFactors = PrimeFactors(a).ToArray();
                var prime2Cnt = primeFactors.Count(p => p == 2);
                var prime3Cnt = primeFactors.Count(p => p == 3);

                var numMin = a / (int)Math.Pow(2, prime2Cnt) / (int)Math.Pow(3, prime3Cnt);

                acceptNumMin = Math.Max(acceptNumMin, numMin);
                acceptNumMax = Math.Min(acceptNumMax, a);
            }

            if(acceptNumMax < acceptNumMin)
            {
                Console.WriteLine(-1);
                return;
            }
            ...
            {
                var a = aArray[i];
                var dividedA = a / acceptNumMax;
                var primeFactors = PrimeFactors(dividedA).ToArray();
                var prime2Cnt = primeFactors.Count(p => p == 2);
                var prime3Cnt = primeFactors.Count(p => p == 3);

                opeCntSum += prime2Cnt + prime3Cnt;
            }

            Console.WriteLine(opeCntSum);
        }
