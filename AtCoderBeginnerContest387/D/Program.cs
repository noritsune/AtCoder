...
public class Solver {
    ...
    private Vector2[] offsets = new Vector2[] { new (0, 1), new (1, 0), new (0, -1), new (-1, 0) };
    private int[] offsetsIndexOffsets = new int[] { -1, 1 };
    bool[,] visited;
    ...
    public void Solve()
    {
        ...
        for (int i = 0; i < H; i++)
        {
            ...
            if (startIdx != -1) startPos = new Vector2(startIdx, i);
            var goalIdx = row.IndexOf('G');
            if (goalIdx != -1) goalPos = new Vector2(goalIdx, i);
        }
        ...
        for (int i = 0; i < 4; i++)
        {
            var nextX = startPos.X + offsets[i].X;
            var nextY = startPos.Y + offsets[i].Y;

            if (nextX < 0 || nextX >= W || nextY < 0 || nextY >= H) continue;
            if (grid[nextY][nextX] == '#') continue;

            visited[nextY, nextX] = true;
            var resultCost = Dfs(nextX, nextY, 1, i);
            minCost = Math.Min(minCost, resultCost);
        }

        Console.WriteLine(minCost == int.MaxValue ? -1 : minCost);
    }

    int Dfs(int x, int y, int cost, int lastOffsetIdx)
    {

        var minCost = int.MaxValue;
        foreach (var offsetsIndexOffset in offsetsIndexOffsets)
        {
            var offsetIdx = (lastOffsetIdx + offsetsIndexOffset + 4) % 4;

            var nextX = x + offsets[offsetIdx].X;
            var nextY = y + offsets[offsetIdx].Y;
            ...
            visited[nextY, nextX] = true;
            var resultCost = Dfs(nextX, nextY, cost + 1, offsetIdx);
            minCost = Math.Min(minCost, resultCost);
