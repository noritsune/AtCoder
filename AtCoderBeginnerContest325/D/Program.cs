...
public class Solver {
    public void Solve() {
        var N = Rl();
        var TDs = new List<(long T, long D)>();

        // 座標圧縮
        var zahyoSet = new HashSet<long>();

        for (int i = 0; i < N; i++)
        {
            var TD = Rla();
            var endSec = TD[0] + TD[1];
            TDs.Add((TD[0], endSec));

            zahyoSet.Add(TD[0] - 1);
            zahyoSet.Add(TD[0]);
            zahyoSet.Add(TD[0] + 1);
            zahyoSet.Add(endSec - 1);
            zahyoSet.Add(endSec);
            zahyoSet.Add(endSec + 1);
        }
        ...
        var ascSortedZahyos = zahyoSet.OrderBy(z => z).ToList();

        var ans = 0;
        var TDsIdx = 0;
        foreach (var zahyo in ascSortedZahyos)
        {
            var (T, D) = TDs[TDsIdx];
