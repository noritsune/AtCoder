...
public class Solver {
    public void Solve() {
        var NABC = Rla();
        var N = (int)NABC[0]; var A = NABC[1]; var B = NABC[2]; var C = NABC[3];

        // 0~N-1までは車、N~2N-1までは電車
        var dijkstra = new Dijkstra(N * 2);
        for (int i = 0; i < N; i++)
        {
            var uraI = i + N;
            dijkstra.Add(i, uraI, 0);
            ...
            for (int j = 0; j < N; j++)
            {
                ...
                var D = Ds[j];
                var uraJ = j + N;
                dijkstra.Add(i, j, D * A);
                dijkstra.Add(uraI, uraJ, D * B + C);
            }
        }

        var minCost = dijkstra.GetMinCost(0);
        Console.WriteLine(Math.Min(minCost[N - 1], minCost[N * 2 - 1]));
    }
