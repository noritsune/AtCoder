...
namespace AtCoder {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            var ACnt = new Dictionary<int, int>();
            foreach (var A in As)
            {
                if (!ACnt.ContainsKey(A)) ACnt.Add(A, 0);
                ACnt[A]++;
            }
            var BCnt = new Dictionary<int, int>();
            foreach (var B in Bs)
            {
                if (!BCnt.ContainsKey(B)) BCnt.Add(B, 0);
                BCnt[B]++;
            }
            
            foreach (var bCntKey in BCnt.Keys)
            {
                if(ACnt.ContainsKey(bCntKey) && ACnt[bCntKey] >= BCnt[bCntKey]) continue;
