...
namespace AtCoder {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            // Aは昇順
            var As = new SortedDictionary<long, long>();
            for (int i = 0; i < Q; i++)
            {
                var q = rla();
                long x;
                ...
                switch (q[0])
                {
                    case 1:
                        x = q[1];
                        if(!As.ContainsKey(x)) As.Add(x, 0); 
                        As[x]++;
                        break;
                    case 2:
                        x = q[1];
                        ...
                        var lowerBoundUpperIndex = LowerBoundUnder(As, x, Comparer<long>.Default);
                        if(lowerBoundUpperIndex >= k) {
                            Console.WriteLine(As[lowerBoundUpperIndex - k]);
                        } else {
                            Console.WriteLine(-1);
                        }
                        break;
                    case 3:
                        ...
                        var lowerBoundIndex = LowerBound(As, x, Comparer<long>.Default);
                        if(lowerBoundIndex + k < As.Count) {
                            Console.WriteLine(As[lowerBoundIndex + k]);
                        } else {
                            Console.WriteLine(-1);
                        }
                        break;
        ...
        /// </summary>
        /// <param name="dic">昇順ソートされた配列</param>
        /// <param name="target">検索対象</param>
        public static int LowerBound<T>(SortedDictionary<long, T> dic, T target, Comparer<T> comparer)
        {
            // 探索範囲のインデックス
            var min = 0;
            var max = dic.Count - 1;
            ...
            {
                var mid = min + (max - min) / 2;
                int compareResult = comparer.Compare(target, dic[mid]);
        ...
        /// <param name="array">昇順ソートされた配列</param>
        /// <param name="target">検索対象</param>
        public static int LowerBoundUnder<T>(SortedDictionary<long, T> dic, T target, Comparer<T> comparer)
        {
            // 探索範囲のインデックス
            var min = 0;
            var max = dic.Count() - 1;
