...
public class Solver {
    public void Solve()
    {
        ...
        {
            var NMXY = Ria();
            var (N, M, X, Y) = (NMXY[0], NMXY[1], NMXY[2], NMXY[3]);

            graph.InitFromStdin(N, M);

            path.Push(X);
            visited.Add(X);
            Dfs(graph, path, visited, Y);
    ...
    bool Dfs(Graph<int> graph, Stack<int> path, HashSet<int> visited, int goal)
    {
        var current = path.Peek();
        var nexts = graph.Vertices[current]
            .OrderBy(v => v);
        foreach (var next in nexts)
        {
            ...
            var ret = Dfs(graph, path, visited, goal);
            if (ret)
            {
                return true;
            }
