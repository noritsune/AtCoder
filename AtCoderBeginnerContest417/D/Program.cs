...
public class Solver {
    public void Solve()
    {
        ...
        var length = 10000 * 500 + 1;
        // 最初のテンションはLeafのIdxだったけど最終的なテンションはLeafの値になった
        var currentToInitValues = new List<int>[length];
        for (int i = 0; i < length; i++)
        {
            currentToInitValues[i] = new List<int>{i};
        }

        for (int i = 0; i < N; i++)
        {
            var PAB = Ria();
            var (P, A, B) = (PAB[0], PAB[1], PAB[2]);

            for (int j = 0; j <= P; j++)
            {
                currentToInitValues[j + A].AddRange(currentToInitValues[j]);
                currentToInitValues[j].Clear();
            }
        }
