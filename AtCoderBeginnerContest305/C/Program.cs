...
public class Solver {
    public void Solve()
    {
        ...
        int rectTop = int.MaxValue;
        int rectBottom = int.MinValue;
        int rectLeft = int.MaxValue;
        int rectRight = int.MinValue;
        for (int h = 0; h < H; h++)
        {
            for (int w = 0; w < W; w++)
            {
                if (grid[h][w] == '#')
                {
                    rectTop = Math.Min(rectTop, h);
                    rectBottom = Math.Max(rectBottom, h);
                    rectLeft = Math.Min(rectLeft, w);
                    rectRight = Math.Max(rectRight, w);
                }
            }
        }
        ...
        {
            for (int w = rectLeft; w <= rectRight; w++)
            {
