...
public class Solver {
    public void Solve()
    {
        ...
        long elapsedSleep = 0;
        var minToElapsedSleep = new Dictionary<long, long> { { As[0], 0 } };
        for (int i = 1; i < N; i++)
        {
            bool isWakeUp = i % 2 == 0;
            var elapsedSleepDelta = isWakeUp ? As[i] - As[i - 1] : 0;
            elapsedSleep += elapsedSleepDelta;
            minToElapsedSleep.Add(As[i], elapsedSleep);
        }

        var mins = minToElapsedSleep.Keys.ToArray();
        ...
        for (int i = 0; i < Q; i++)
        {
            ...
            var minLIdx = LowerBound(mins, l, Comparer<long>.Default);
            var minRIdx = LowerBoundUnder(mins, r, Comparer<long>.Default);
            var sleepMin = minToElapsedSleep[mins[minRIdx]] - minToElapsedSleep[mins[minLIdx]];

            if (minLIdx % 2 == 0)
            {
                sleepMin += mins[minLIdx] - l;
            }

            if (minRIdx % 2 == 1)
            {
                sleepMin += r - mins[minRIdx];
            }
