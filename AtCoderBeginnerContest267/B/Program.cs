...
namespace KyoPro {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            var standPinCnts = new int[7];
            if(S[6] == '1') standPinCnts[0]++;
            if(S[3] == '1') standPinCnts[1]++;
            if(S[1] == '1') standPinCnts[2]++;
            if(S[7] == '1') standPinCnts[2]++;
            if(S[0] == '1') standPinCnts[3]++;
            if(S[4] == '1') standPinCnts[3]++;
            if(S[2] == '1') standPinCnts[4]++;
            if(S[8] == '1') standPinCnts[4]++;
            if(S[5] == '1') standPinCnts[5]++;
            if(S[9] == '1') standPinCnts[6]++;

            for (int rowAIdx = 0; rowAIdx < 5; rowAIdx++)
            {
                if(standPinCnts[rowAIdx] == 0) continue;

                for (int rowBIdx = rowAIdx + 2; rowBIdx < 7; rowBIdx++)
                {
                    if(standPinCnts[rowBIdx] == 0) continue;

                    for (int rowC = rowAIdx + 1; rowC < rowBIdx; rowC++)
                    {
                        if (standPinCnts[rowC] == 0)
                        {
