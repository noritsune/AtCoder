...
namespace KyoPro {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            long firstBlockSum = 0;
            for (int i = 0; i < M; i++)
            {
                firstBlockSum += (i + 1) * As[i];
            }

            long blockSumMax = firstBlockSum;
            long prevBlockSum = firstBlockSum;
            for (int i = 1; i <= N - M; i++)
            {
                var ruiSum = rui[i + M - 1] - rui[i - 1];
                var blockSum = prevBlockSum - ruiSum + M * As[i + M - 1];
                blockSumMax = Math.Max(blockSumMax, blockSum);

                prevBlockSum = blockSum;
            }

            Console.WriteLine(blockSumMax);
        }
