...
class Sol{
	...
	public void Solve(){
        ...
        int winnerIndex_left = -1;
        int currentRate_left = int.MinValue;
        for (int i = 0; i < As.Length / 2; i++)
        {
            if(As[i] < currentRate_left) continue;

            currentRate_left = As[i];
            winnerIndex_left = i;
        }

        int winnerIndex_right = -1;
        int currentRate_right = int.MinValue;
        for (int i = As.Length / 2; i < As.Length; i++)
        {
            if(As[i] < currentRate_right) continue;

            currentRate_right = As[i];
            winnerIndex_right = i;
        }

        int semiWinnerIndex = currentRate_left > currentRate_right
        ? winnerIndex_right
        : winnerIndex_left;
		
		Console.WriteLine(semiWinnerIndex + 1);
		Console.ReadLine();
	}
