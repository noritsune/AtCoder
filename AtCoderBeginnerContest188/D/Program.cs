...
class Sol{
	...
	public void Solve(){
        ...
        List<FeeDay> feeDays = new List<FeeDay>();
        for (int i = 0; i < N; i++)
        {
            int[] abc = ria();
            int a = abc[0];
            ...
            feeDays.Add(new FeeDay(a - 1, c));
            feeDays.Add(new FeeDay(b, -c));
        }

        feeDays.Sort((a, b) => a.day - b.day);

        long ans = 0;
        long lastFee = 0;
        int lastDay = 0;
        foreach (FeeDay feeDay in feeDays)
        {
            if(feeDay.day != lastDay) {
                ans += Math.Min(C, lastFee) * (feeDay.day - lastDay);
                lastDay = feeDay.day;
            }
