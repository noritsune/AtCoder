...
namespace KyoPro {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            var distinct = ABCDE.Distinct().ToList();
            if (distinct.Count != 2)
            {
            ...
            var distinctCounts = distinct
                .Select(x => ABCDE.Count(y => y == x))
                .OrderBy(x => x)
                .ToList();
            var count1 = distinctCounts[0];
            var count2 = distinctCounts[1];

            Console.WriteLine(count1 == 2 && count2 == 3 ? "Yes" : "No");
        }
