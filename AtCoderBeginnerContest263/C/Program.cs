...
namespace KyoPro {
    ...
    public class Solver
    {
        ...
        public void Solve()
        {
            ...
            var nums = Enumerable.Range(1, M).ToList();
            dfs(new List<int>(), nums);
        }

        void dfs(List<int> nums, List<int> remainedNums)
        {
            if (nums.Count == N)
            {
            ...
            foreach (var num in remainedNums)
            {
                if(nums.Count > 0 && num <= nums.Last()) continue;
                
                var nextNums = new List<int>(nums) { num };
                var nextRemainedNums = new List<int>(remainedNums);
                nextRemainedNums.Remove(num);
                
                dfs(nextNums, nextRemainedNums);
            }
        }
