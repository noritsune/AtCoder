...
namespace KyoPro {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            var As = Enumerable.Range(1, N).ToArray();
            var numIndices = Enumerable.Range(-1, N + 1).ToArray();
            for (int i = 0; i < Q; i++)
            {
                var x = Ri();
                var xIndex = numIndices[x];
                var swapIndex = xIndex < N - 1 ? xIndex + 1 : xIndex - 1;
                
                (As[xIndex], As[swapIndex]) = (As[swapIndex], As[xIndex]);
                
                numIndices[x] = swapIndex;
                numIndices[As[xIndex]] = xIndex;
            }
