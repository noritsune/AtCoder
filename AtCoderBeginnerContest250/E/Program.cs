...
namespace KyoPro {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            var setAs = new HashSet<int>[N + 1];
            var setBs = new HashSet<int>[N + 1];
            var preAs = new List<int>();
            var preBs = new List<int>();
            for (var i = 1; i <= N; i++)
            {
                preAs.Add(As[i - 1]);
                preBs.Add(Bs[i - 1]);
                
                setAs[i] = new HashSet<int>(preAs);
                setBs[i] = new HashSet<int>(preBs);
            }
            ...
            for (int i = 0; i < Q; i++)
            {
                ...
                bool isSame = setA.SetEquals(setB);
