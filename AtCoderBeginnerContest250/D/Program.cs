...
namespace KyoPro {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            int primeLimit = (int)Math.Pow(N / 2.0, 1 / 3.0) + 1;
            var primes = Primes(primeLimit).ToArray();

            long ans = 0;
            int qIndexLimit = primes.Length;
            for (int pIndex = 0; pIndex < primes.Length; pIndex++)
            {
                for (int qIndex = pIndex + 1; qIndex < qIndexLimit; qIndex++)
                {
                    long p = primes[pIndex];
                    long q = primes[qIndex];
                    long k = p * q * q * q;
                    if (k > N)
                    {
                        qIndexLimit = qIndex;
        ...
        static IEnumerable<int> Primes(int max) {
            if (max <= 1) return Enumerable.Range(0, 0);
            
            var sieve = Enumerable.Range(0, max + 1).ToArray();
            sieve[1] = 0;  // 0 : 素数ではない
