...
namespace AtCoder {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            for (int r = 1; r <= N; r++)
            {
                if(!map.ContainsKey(ruiseki[r - 1]))
                {
                    map.Add(ruiseki[r - 1], 0);
                }

                map[ruiseki[r - 1]]++;
                
                if(map.ContainsKey(ruiseki[r] - K))
                {
                    ans += map[ruiseki[r] - K];
                }
            }
