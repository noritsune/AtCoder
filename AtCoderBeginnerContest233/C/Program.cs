...
namespace AtCoder {
    ...
    public class Solver {
        private List<long[]> LaArrays = new List<long[]>();
        private long X;
        private long ans = 0;
        ...
        void dfs(int currentIndex, long multi)
        {
            if (multi > X) return;
            
            if (currentIndex == LaArrays.Count - 1)
            {
                if(multi == X) ans++;
                return;
            }
            ...
            var aArray = LaArrays[currentIndex].Skip(1);
            foreach (var a in aArray)
            {
                dfs(currentIndex, multi * a);
            }
        }
