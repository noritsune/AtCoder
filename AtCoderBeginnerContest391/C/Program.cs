...
public class Solver {
    public void Solve()
    {
        ...
        var pToPos = Enumerable.Range(0, N).ToArray();
        var inNestCnts = Enumerable.Repeat(1, N).ToArray();
        var multiCnt = 0;
        for (int i = 0; i < Q; i++)
        {
            var query = Ria();
            if (query[0] == 1)
            {
                var (P, H) = (query[1] - 1, query[2] - 1);
                var prevPos = pToPos[P];
                if (inNestCnts[prevPos] == 2) multiCnt--;
                if (inNestCnts[H] == 1) multiCnt++;
