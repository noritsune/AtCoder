...
public class Solver {
    public void Solve()
    {
        ...
        var tToMinCostOld = new Dictionary<string, int?>();
        tToMinCostOld[""] = 0;
        for (int i = 1; i <= T.Length; i++)
        {
            tToMinCostOld[T.Substring(0, i)] = null;
        }

        for (int i = 0; i < N; i++)
        {
            var tToMinCost = tToMinCostOld.ToDictionary(x => x.Key, x => x.Value);
            var Ss = Rsa().Skip(1).ToArray();
            foreach (var S in Ss)
            {
                foreach (var (t, tV) in tToMinCostOld)
                {
                    if (!tV.HasValue) continue;

                    var nt = t + S;
                    if (!tToMinCost.TryGetValue(nt, out var ntV)) continue;

                    if (ntV.HasValue)
                    {
                        tToMinCost[nt] = Math.Min(ntV.Value, tV.Value + 1);
                    }
                    else
                    {
                        tToMinCost[nt] = tV + 1;
            ...
            tToMinCostOld = tToMinCost;
        }

        var ans = tToMinCostOld[T].HasValue ? tToMinCostOld[T].Value : -1;
