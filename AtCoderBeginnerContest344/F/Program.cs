...
public class Solver {
    ...
    {
        var N = Ri();
        var pGrid = new long[N][];
        for (int i = 0; i < N; i++) pGrid[i] = Rla();
        var rGrid = new long[N][];
        for (int i = 0; i < N; i++) rGrid[i] = Rla();
        var dGrid = new long[N - 1][];
        for (int i = 0; i < N - 1; i++) dGrid[i] = Rla();

        var minCosts = new long[N, N];
        for (int i = 0; i < N; i++)
        {
            for (int j = 0; j < N; j++)
            {
                minCosts[i, j] = long.MaxValue;
            }
        }
        minCosts[0, 0] = 0;

        var maxPs = new long[N, N];

        var q = new Queue<(int i, int j, long money, long cost)>();
        q.Enqueue((0, 0, 0, 0));
        while (q.Any())
        {
            var (i, j, money, cost) = q.Dequeue();
            if (cost > minCosts[i, j]) continue;

            maxPs[i, j] = Math.Max(pGrid[i][j], maxPs[i, j]);
            var maxP = maxPs[i, j];
            ...
            if (j < N - 1)
            {
                var nextCost = cost;
                var nextMoney = money;
                if (nextMoney < rGrid[i][j])
                {
                    // お金が足りないので稼ぐ
                    var needMoney = rGrid[i][j] - money;
                    var stepCost = (long)Math.Ceiling((double)needMoney / maxP);
                    nextCost += stepCost;
                    nextMoney += stepCost * maxP;
                }
                ...
                if (nextCost < minCosts[i, j + 1])
                {
                    minCosts[i, j + 1] = nextCost;
                    maxPs[i, j + 1] = maxP;
                    q.Enqueue((i, j + 1, nextMoney, nextCost));
                }
            }
            ...
            if (i < N - 1)
            {
                var nextCost = cost;
                var nextMoney = money;
                if (nextMoney < dGrid[i][j])
                {
                    // お金が足りないので稼ぐ
                    var needMoney = dGrid[i][j] - money;
                    var stepCost = (long)Math.Ceiling((double)needMoney / maxP);
                    nextCost += stepCost;
                    nextMoney += stepCost * maxP;
                }
                ...
                if (nextCost < minCosts[i + 1, j])
                {
                    minCosts[i + 1, j] = nextCost;
                    maxPs[i + 1, j] = maxP;
                    q.Enqueue((i + 1, j, nextMoney, nextCost));
                }
            }
        }

        Console.WriteLine(minCosts[N - 1, N - 1]);
    }
