...
namespace util {
    ...
    public class Sol{
        ...
        public void SolveBad2()
        {
            ...
            int A1Cnt = -1;
            int A2Cnt = -1;
            for (int b1 = 1; b1 <= 119; b1++)
            {
                for (int a1 = 0; a1 <= 119 - b1 ; a1++)
                {
                    for (int b2 = 1; b2 <= 119 - b1 - a1; b2++)
                    {
                        long num = ((long)Math.Pow(2, b1) + a1) * (long)Math.Pow(2, b2);
                        if (num > N || num < 0) break;
                    
                        int a2Space = 119 - b1 - a1 - b2;
                        long a2 = N - num;
                        if (a2 <= a2Space)
                        {
                            B1Cnt = b1;
                            ...
                            var B1Str = Enumerable.Repeat('B', B1Cnt);
                            var A1Str = Enumerable.Repeat('A', A1Cnt);
                            var B2Str = Enumerable.Repeat('B', B2Cnt);
                            var A2Str = Enumerable.Repeat('A', A2Cnt);
                            Console.WriteLine("A" + new string(B1Str.Concat(A1Str).Concat(B2Str).Concat(A2Str).ToArray()));
                            Console.ReadLine();
                            return;;
                        }
                    }
        ...
        public void SolveBad()
        {
            ...
            {
                for (int b = 0; b <= 120 - a1; b++)
                {
                    long num = a1 * (long)Math.Pow(2, b);
                    if (num > N) break;
                    
                    int a2Space = 120 - a1 - b;
                    long a2 = N - num;
                    if (a2 <= a2Space)
                    {
                        A1Cnt = a1;
            ...
            var A1Str = Enumerable.Repeat('A', A1Cnt);
            var BStr  = Enumerable.Repeat('B', BCnt );
            var A2Str = Enumerable.Repeat('A', A2Cnt);
            
            Console.WriteLine(new string(A1Str.Concat(BStr).Concat(A2Str).ToArray()));
            Console.ReadLine();
        }
