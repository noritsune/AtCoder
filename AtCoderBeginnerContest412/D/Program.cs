...
public class Solver {
    public void Solve()
    {
        var NM = Ria();
        var (N, M) = (NM[0], NM[1]);
        var originalGraph = new IntGraph(GraphType.Undirected);
        originalGraph.InitFromStdin(N, M);

        var minCost = int.MaxValue;
        var perms = Enumerable.Range(1, N).Perm(N);
        var addVs = new List<int>();
        var removeVs = new List<int>();
        foreach (var perm in perms)
        {
            var graph = originalGraph.Clone();
            var permArr = perm.ToArray();
            var loopHead = permArr.First();
            var cost = 0;
            for (int i = 0; i < N; i++)
            {
                var v = permArr[i];
                var prevV = permArr[(i - 1 + N) % N];
                var nextV = permArr[(i + 1) % N];
                foreach (var adjacentV in graph.Vertices[v])
                {
                    if (adjacentV != prevV && adjacentV != nextV)
                    {
                        removeVs.Add(adjacentV);
                    }
                }
                foreach (var removeV in removeVs)
                {
                    cost++;
                    graph.RemoveEdge(v, removeV);
                }

                if (!graph.Vertices[v].Contains(prevV))
                {
                    cost++;
                    graph.AddEdge(v, prevV);
                }
                if (!graph.Vertices[v].Contains(nextV))
                {
                    cost++;
                    graph.AddEdge(v, nextV);
                }
...
public class Graph<T>
{
    ...
    public bool RemoveEdge(T from, T to)
    {
        ...
        if (GraphType == GraphType.Undirected) _vertices[to].Remove(from);
        return true;
    }
    ...
    {
        var newGraph = new Graph<T>(GraphType);
        foreach (var v in _vertices)
        {
            newGraph.AddVertex(v.Key, v.Value);
        }
        return newGraph;
    }
}
