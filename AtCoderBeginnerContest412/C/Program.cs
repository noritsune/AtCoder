...
public class Solver {
    public void Solve()
    {
        ...
        for (int t = 0; t < T; t++)
        {
            ...
            var betweenSs = new MultiSet<int>();
            for (int i = 1; i < N - 1; i++)
            {
                betweenSs.Insert(Ss[i]);
            }

            var cnt = 2;
            var isOk = true;
            var current = Ss.First();
            while (current * 2 < Ss.Last())
            {
                var ub = betweenSs.UpperBound(current * 2) - 1;
                if (ub < 0)
                {
                    isOk = false;
                    break;
                ...
                var next = betweenSs[ub];
                betweenSs.Remove(next);
                cnt++;
                current = next;
            }

            Console.WriteLine(isOk ? cnt : -1);
        }
    }
