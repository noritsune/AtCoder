...
public class Solver {
    public void Solve()
    {
        ...
        var mod46ToCntInAs = new int[46];
        var mod46ToCntInBs = new int[46];
        var mod46ToCntInCs = new int[46];
        for (int i = 0; i < N; i++)
        {
            mod46ToCntInAs[As[i] % 46]++;
            mod46ToCntInBs[Bs[i] % 46]++;
            mod46ToCntInCs[Cs[i] % 46]++;
        }
        ...
        for (int i = 0; i < 46; i++)
        {
            for (int j = 0; j < 46; j++)
            {
                var remained = (46 - (i + j) % 46) % 46;
                ans += (long)mod46ToCntInAs[i] * mod46ToCntInBs[j] * mod46ToCntInCs[remained];
            }
        }
