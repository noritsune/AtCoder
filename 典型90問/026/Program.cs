    ...
    public class Solver {
        public void Solve()
        {
            ...
            while (q.Count > 0)
            {
                ...
                var nextNodes = graph.Vertices[node].Where(nextNode => !isVisited[nextNode]);
            ...
            var nodesDepthEven = new List<int>();
            var nodesDepthNotEven = new List<int>();
            for (int i = 1; i <= N; i++)
            {
                if (depths[i] % 2 == 0)
                {
                    nodesDepthEven.Add(i);
                }
                else
                {
                    nodesDepthNotEven.Add(i);
                }
            }
            
            var ans = nodesDepthEven.Count >= N / 2
                ? nodesDepthEven.Take(N / 2)
                : nodesDepthNotEven.Take(N / 2);
