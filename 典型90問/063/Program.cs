...
public class Solver {
    public void Solve()
    {
        ...
        var maxSize = 0;
        var rowIdxPats = BitFullSearch(H);
        foreach (var rowPat in rowIdxPats)
        {
            var rows = new List<int[]>();
            for (int row = 0; row < H; row++)
            {
                if (rowPat[row]) rows.Add(grid[row]);
            }
            if (!rows.Any()) continue;

            var nToSi = new int[H * W + 1];
            for (int w = 0; w < W; w++)
            {
                var isAllSame = rows.All(r => r[w] == rows[0][w]);
                if (isAllSame)
                {
                    nToSi[rows[0][w]] += rows.Count;
                    maxSize = Math.Max(maxSize, nToSi[rows[0][w]]);
                }
            }
        }
