...
namespace AtCoder {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            long coinCntSumMin = long.MaxValue;
            for (long aCnt = 0; aCnt < 10000; aCnt++)
            {
                for (long bCnt = 0; bCnt < 10000 - aCnt; bCnt++)
                {
                    long remainedPrice = N - aCnt * A - bCnt * B;
                    if(remainedPrice < 0 || remainedPrice % C != 0) continue;
                    
                    long cCnt = remainedPrice / C;
                    
                    long coinCntSum = aCnt + bCnt + cCnt;
                    coinCntSumMin = Math.Min(coinCntSumMin, coinCntSum);
                }
            }
            
            Console.WriteLine(coinCntSumMin);
        }
