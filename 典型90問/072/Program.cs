...
public class Solver
{
    ...
    int Dfs(int sx, int sy, int x, int y, int score)
    {
        var maxScore = -1;
        for (int i = 0; i < 4; i++)
        {
            var nx = x + dxs[i]; var ny = y + dys[i];
            if (nx < 0 || nx >= W || ny < 0 || ny >= H) continue;
            if (grid[ny][nx] == '#') continue;
            if (nx == sx && ny == sy && score >= 3) return score;
            if (visited[ny, nx]) continue;

            visited[ny, nx] = true;
            var nScore = Dfs(sx, sy, nx, ny, score + 1);
            visited[ny, nx] = false;

            maxScore = Math.Max(maxScore, nScore);
        }
