...
namespace AtCoder {
    ...
    public class Vector3
    {
        ...
        public double AngleTo(Vector3 other)
        {
            ...
            var dot = Dot(other);
            return Math.Acos(Math.Clamp(dot / length, -1, 1));
        }
    }
    ...
    public class Solver {
        public void Solve()
        {
            ...
            var staturePos = new Vector3(X, Y, 0);
            
            for (int i = 0; i < Q; i++)
            {
                var E = ri();

                var theta = -E / (double)T * 2.0 * Math.PI - Math.PI / 2;
                var gondolaPos = new Vector3(
                    0,
                    L / 2.0 * Math.Cos(theta),
                    L / 2.0 * Math.Sin(theta) + L / 2.0
                );

                var gondolaGrand = new Vector3(0, gondolaPos.Y, 0);
                
                var statueToGondolaGrand = gondolaGrand - staturePos;
                var statueToGondola = gondolaPos - staturePos;
                
                var angleInRad = Math.Atan2(gondolaPos.Z,  statueToGondolaGrand.Length());
                var angleInDeg = angleInRad * 180 / Math.PI;
                Console.WriteLine(angleInDeg);
            }
        }
