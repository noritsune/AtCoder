...
namespace AtCoder {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            {
                while (right < N && (numCnt.ContainsKey(As[right]) || numCnt.Keys.Count < K))
                {
                    if (!numCnt.ContainsKey(As[right])) numCnt.Add(As[right], 0);
                    numCnt[As[right]]++;
                ...
                numCnt[As[left]]--;
                if(numCnt[As[left]] == 0) numCnt.Remove(As[left]);
            }
