...
public class Solver {
    ...
    {
        var NK = Rsa();
        var N = NK[0]; var K = long.Parse(NK[1]);

        for (int i = 0; i < K; i++)
        {
            var nBased10 = Convert.ToInt64(N, 8);
            var nBased9 = KisuHenkan(nBased10, 9).ToString();
            N = nBased9.Replace("8", "5");
        }

        Console.WriteLine(N);
    }

    long KisuHenkan(long n, long b)
    {
        long ret = 0;
        long digit = 1;
        while (n > 0)
