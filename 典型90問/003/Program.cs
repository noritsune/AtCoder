...
namespace AtCoder {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            // foreach (var AB in ABs) graph.AddEdge(AB.A, AB.B);
            
            var dijkstra = new Dijkstra(N);
            foreach (var AB in ABs)
            {
                dijkstra.Add(AB.A - 1, AB.B - 1, 1);
                dijkstra.Add(AB.B - 1, AB.A - 1, 1);
            }
            
            var minCostFrom0 = dijkstra.GetMinCost(0).ToList();
            var maxCost = minCostFrom0.Max();
            var leafNodeIndex = minCostFrom0.IndexOf(maxCost);

            var minCostFromLeaf = dijkstra.GetMinCost(leafNodeIndex);
            long maxScore = minCostFromLeaf.Max() + 1;
