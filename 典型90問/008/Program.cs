...
namespace AtCoder {
    ...
    public class Solver
    {
        ...
        public void Solve()
        {
            ...
            // dp[i, j]: i文字目までを見たときに"atcoder"のj文字目までが出来るか
            var dp = new int[N + 1, ATCODER.Length + 1];
            for (int i = 0; i <= N; i++) dp[i, 0] = 1;
            
            for (int i = 0; i < N; i++)
            {
                for (int j = 1; j <= ATCODER.Length; j++)
                {
                    dp[i + 1, j] = dp[i, j];
            ...
            Console.WriteLine(dp[N, ATCODER.Length]);
        }
        ...
        public void SolveTest()
        {
            ...
            int atcoderIndexCurrent = 0;
            int atcoderIndexNext = 1;
            long ans = 0;
            long atcoderCnt = 1;
            int sameCharCnt = 0;
            foreach (char c in S)
            {
                if(c == ATCODER[atcoderIndexCurrent])
                {
                    sameCharCnt++;
                }
                else
                {
                    if(sameCharCnt == 0 || c != ATCODER[atcoderIndexNext]) continue;
                    
                    atcoderCnt *= sameCharCnt;
                    atcoderCnt %= 1000000007;
                    sameCharCnt = 0;
                    
                    atcoderIndexCurrent++;
                    if (atcoderIndexCurrent == ATCODER.Length)
                    {
                        atcoderIndexCurrent = 0;
                        ans += atcoderCnt;
                        atcoderCnt = 1;
                    }
                    
                    atcoderIndexNext = atcoderIndexCurrent + 1;
                    if(atcoderIndexNext == ATCODER.Length) atcoderIndexNext = 0;
