...
namespace AtCoder {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            var gridIsRed = new bool[H, W];
            var unionFind = new UnionFind(H * W);
            for (int i = 0; i < Q; i++)
            {
                var query = ria();
                int t = query[0];
                switch (t)
                {
                    case 1:
                        ...
                        {
                            if (neighbor.X < 0 || neighbor.X >= H || neighbor.Y < 0 || neighbor.Y >= W) continue;
                            if (!gridIsRed[neighbor.X, neighbor.Y]) continue;
                            
                            int idxNeighbor = neighbor.X * W + neighbor.Y;
                            unionFind.Union(idxMe, idxNeighbor);
                        }
                        break;
                    case 2:
                        int ra = query[1] - 1;
                        ...
                        int aIdx = ra * W + ca;
                        int bIdx = rb * W + cb;
                        Console.WriteLine(unionFind.Same(aIdx, bIdx) ? "Yes" : "No");
                        break;
                }
            }
        }
            ...
            for (int i = 0; i < Q; i++)
            {
                ...
                switch (t)
                {
                    ...
                    case 2:
                        ...
                        q.Enqueue(new Vector2(ra - 1, ca - 1));
                        var visited = new bool[H, W];
                        visited[ra - 1, ca - 1] = true;
                        bool isOK = false;
                        while (q.Count > 0)
                        {
                            var p = q.Dequeue();
                            if (p.X == rb - 1 && p.Y == cb - 1)
                            {
                                isOK = true;
                                break;
                            }

                            var nexts = new Vector2[] {
                                new Vector2(p.X - 1, p.Y),
                                new Vector2(p.X + 1, p.Y),
                                new Vector2(p.X, p.Y - 1),
                                new Vector2(p.X, p.Y + 1)
                            };
                            foreach (var next in nexts)
                            {
                                ...
                                if (!gridIsRed[next.X, next.Y]) continue;
                                q.Enqueue(next);
