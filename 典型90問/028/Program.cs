...
namespace AtCoder {
    ...
    public class Solver {
        ...
        {
            int N = ri();
            var papers = new List<(int lx, int ly, int rx, int ry)>();
            for (int i = 0; i < N; i++)
            {
                int[] p = ria();
                papers.Add((p[0], p[1], p[2], p[3]));
            }

            var ruiseki = new int[1000 + 1, 1000 + 1];
            foreach (var paper in papers)
            {
                ruiseki[paper.ly, paper.lx]++;
                ruiseki[paper.ry, paper.rx]++;
                ruiseki[paper.ry, paper.lx]--;
                ruiseki[paper.ly, paper.rx]--;
            }
            ...
            var paperCnts = new int[N + 1];
            for (int i = 0; i <= 1000; i++)
            {
                for (int j = 0; j <= 1000; j++)
                {
                    paperCnts[ruiseki[i, j]]++;
                }
            }

            for (int i = 1; i <= N; i++)
            {
                Console.WriteLine(paperCnts[i]);
            }
        }
