...
namespace AtCoder {
    ...
    public class Solver {
        public void Solve() {
            ...
            {
                int B = ri();
                var lowerBound = LowerBound(As, B, Comparer<int>.Default);
                
                int diffUpper = Math.Abs(B - As[lowerBound]);
                int diffLower = Math.Abs(B - As[Math.Max(lowerBound - 1, 0)]);
                Console.WriteLine(Math.Min(diffUpper, diffLower));
            }
        }
