...
public class Solver {
    public void Solve()
    {
        ...
        var remained = K;
        var n = N;
        var nToVisitedRemained = new long[CONST.MOD];
        while (nToVisitedRemained[n] == 0)
        {
            nToVisitedRemained[n] = remained;
            n = PushBtn(n);
            remained--;
        ...
        var cycleLength = nToVisitedRemained[n] - remained;
        remained %= cycleLength;
        while (remained > 0)
        {
            n = PushBtn(n);
            remained--;
    ...
    {
        long ret = n;
        foreach (var digitC in n.ToString())
        {
            var digit = int.Parse(digitC.ToString());
            ret += digit;
        }
        return ret % CONST.MOD;
    }
