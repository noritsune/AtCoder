...
namespace AtCoder {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            var N = ND[0];
            var D = ND[1];
            var LRs = new List<int[]>();
            for (int i = 0; i < N; i++) LRs.Add(ria());

            var walls = new List<int[]>(LRs);
            walls.Sort((a, b) =>
            {
                int beginDiff = a[1] - b[1];
                if (a[1] != b[1]) return beginDiff;
                return a[0] - b[0];
            });
            ...
            while (wallIndex < N)
            {
                int begin = walls[wallIndex][1];
                int end = begin + D - 1;
                while(wallIndex < N && walls[wallIndex][0] <= end)
                {
                    wallIndex++;
                }
