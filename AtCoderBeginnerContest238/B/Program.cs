...
namespace AtCoder {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            var currentAngle = 0;
            var cutAngles = new List<int>{ 0 };
            foreach (var A in As)
            {
                currentAngle += A;
                currentAngle %= 360;
                cutAngles.Add(currentAngle);
            }
            cutAngles.Add(360);
            ...
            var maxDeltaAngle = 0;
            for (int i = 0; i < cutAngles.Count - 1; i++)
            {
                maxDeltaAngle = Math.Max(maxDeltaAngle, cutAngles[i + 1] - cutAngles[i]);
            }
            
            Console.WriteLine(maxDeltaAngle);
        }
