...
namespace AtCoder {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            BigInteger powerOf10Current = 1;
            BigInteger ans = 0;
            while (powerOf10Current <= N) {
                BigInteger powerOf10last = powerOf10Current - 1;
                powerOf10Current *= 10;
                
                BigInteger min = powerOf10last;
                BigInteger max = Math.Min((long)powerOf10Current - 1, N);
                ans += (max + min) * (max - min + 1) / 2;
                ans -= (max - min + 1) * powerOf10last;
        ...
        public void SolveTest()
        {
            ...
            for (int i = 1; i <= N; i++)
            {
                int ketaCnt = i.ToString().Length;
                long f = i - (long)Math.Pow(10, ketaCnt - 1) + 1; 
                ans += f;
                ans %= 998244353;
            }
