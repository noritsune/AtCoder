...
namespace util {
    ...
    public class Sol{
        ...
        public void Solve()
        {
            ...
            for (int i = 0; i < N; i++) ABs.Add(ria());

            var peopleInOutHistory = new Dictionary<int, int>();
            foreach (var AB in ABs)
            {
                int beginDate = AB[0];
                int endDate = beginDate + AB[1];

                if(!peopleInOutHistory.ContainsKey(beginDate)) peopleInOutHistory.Add(beginDate, 0);
                peopleInOutHistory[beginDate]++;
                
                if(!peopleInOutHistory.ContainsKey(endDate)) peopleInOutHistory.Add(endDate, 0);
                peopleInOutHistory[endDate]--;
            }

            int currentCnt = 0;
            var peopleCntHistory = new Dictionary<int, int> {{0, 0}};
            foreach (var dateCntPair in peopleInOutHistory.ToImmutableDictionary())
            {
                int date = dateCntPair.Key;
                int cnt = dateCntPair.Value;
                
                if(!peopleCntHistory.ContainsKey(date)) peopleCntHistory.Add(date, 0);
                currentCnt += cnt;
                peopleCntHistory[date] += currentCnt;
            }

            var anss = new int[N + 1];
            int lastDate = 0;
            int lastCnt = 0;
            foreach (var dateCntPair in peopleCntHistory.ToImmutableDictionary())
            {
                int date = dateCntPair.Key;
                int cnt = dateCntPair.Value;

                if (cnt == lastCnt) continue;

                anss[lastCnt] += date - lastDate;
