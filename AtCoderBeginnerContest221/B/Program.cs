...
namespace util {
    ...
    public class Sol{
        ...
        public void Solve()
        {
            ...
            {
                string swappedTFirstHalf = T.Substring(0, i);
                string swappedTLatterHalf = T.Substring(i + 2, T.Length - i - 2);
                string swappedTMiddle = T[i + 1].ToString() + T[i];
                string swappedT = swappedTFirstHalf + swappedTMiddle + swappedTLatterHalf;
                if (!S.Equals(swappedT)) continue;
