...
namespace util {
    ...
    public class Sol{
        ...
        public void Solve()
        {
            ...
            var patterns = N.Perm().Select(x => x.ToArray()).ToArray();
            foreach (var pattern in patterns)
            {
                string str = new string(pattern);
                for (int i = 1; i < str.Length; i++)
                {
                    string firstHalfStr = str.Substring(0, i);
                    string latterHalfStr = str.Substring(i, str.Length - i);
                    if(HasLeadingZero(firstHalfStr) || HasLeadingZero(latterHalfStr)) continue;

                    int firstHalfNum = int.Parse(firstHalfStr);
                    int latterHalfNum = int.Parse(latterHalfStr);
                    ans = Math.Max(firstHalfNum * (long)latterHalfNum, ans);
                }
            }
