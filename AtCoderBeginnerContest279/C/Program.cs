...
namespace KyoPro {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            for (int i = 0; i < H; i++) gridS.Add(Rs());
            var gridT = new List<string>();
            for (int i = 0; i < H; i++) gridT.Add(Rs());

            var gridSCols = new List<string>();
            var gridTCols = new List<string>();
            var stringBuilderForS = new System.Text.StringBuilder();
            var stringBuilderForT = new System.Text.StringBuilder();
            for (int i = 0; i < W; i++)
            {
                stringBuilderForS.Clear();
                stringBuilderForT.Clear();

                for (int j = 0; j < H; j++)
                {
                    stringBuilderForS.Append(gridS[j][i]);
                    stringBuilderForT.Append(gridT[j][i]);
                }

                gridSCols.Add(stringBuilderForS.ToString());
                gridTCols.Add(stringBuilderForT.ToString());
            }

            var sortedGridSCols = gridSCols.ToList();
            sortedGridSCols.Sort();
            var sortedGridTCols = gridTCols.ToList();
            sortedGridTCols.Sort();

            for (int i = 0; i < sortedGridSCols.Count; i++)
            {
                if (sortedGridSCols[i] == sortedGridTCols[i]) continue;
