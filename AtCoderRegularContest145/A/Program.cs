...
namespace KyoPro {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            var firstHalf = S[..(int)Math.Ceiling(N / 2.0f)];
            var firstHalfReversed = new string(firstHalf.Reverse().ToArray());
            var lastHalf = S[(N / 2)..];
            var lastHalfReversed = new string(lastHalf.Reverse().ToArray());
            
            if(CanBeSame(firstHalf, lastHalfReversed) || CanBeSame(lastHalf, firstHalfReversed))
            {
        ...
        bool CanBeSame(string s1, string s2)
        {
            int startIdx = 0;
            while(startIdx < s1.Length && s1[startIdx] == 'B' && s2[startIdx] == 'B')
            {
                startIdx++;
            }
            int endIdx = s1.Length - 1;
            while(endIdx >= 0 && s1[endIdx] == 'A' && s2[endIdx] == 'A')
            {
                endIdx--;
            }
            ...
            switch (fixedS1.Length)
            {
                case 1:
                    return fixedS1 == fixedS2;
                case 2:
                    return fixedS1 == fixedS2 || fixedS2 == "AB";
            }
            
            for (int i = 0; i < fixedS1.Length - 1; i++)
            {
                if(fixedS1[i] == 'B' && fixedS1[i + 1] == 'A')
                {
                    return false;
                }
            }
