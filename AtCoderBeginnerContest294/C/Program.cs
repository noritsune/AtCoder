...
namespace KyoPro {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            var CToIdx = new Dictionary<int, int>();
            for (int i = 0; i < Cs.Count; i++)
            {
                CToIdx.Add(Cs[i], i + 1);
            }

            var AtoIdx = As.Select(A => CToIdx[A]);
            var BtoIdx = Bs.Select(B => CToIdx[B]);
            Console.WriteLine(string.Join(" ", AtoIdx));
            Console.WriteLine(string.Join(" ", BtoIdx));
        }
