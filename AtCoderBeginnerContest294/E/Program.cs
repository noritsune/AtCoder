...
namespace KyoPro {
    ...
    public class Solver {
        public void Solve() {
            var LN1N2 = Rla();
            var L = LN1N2[0]; var N1 = LN1N2[1]; var N2 = LN1N2[2];
            var vls1 = new List<(long v, long l)>();
            for (int i = 0; i < N1; i++) {
                var vl = Rla();
                vls1.Add((vl[0], vl[1]));
            }
            var vls2 = new List<(long v, long l)>();
            for (int i = 0; i < N2; i++) {
                var vl = Rla();
                vls2.Add((vl[0], vl[1]));
            }
            
            var sections1 = new List<(long left, long right, long n)>();
            long left1 = 0;
            foreach (var tuple in vls1)
            {
                (long v1, long l1) = tuple;
                var right1 = left1 + l1;
                sections1.Add((left1, right1, v1));
                left1 = right1;
            }
            
            var sections2 = new List<(long left, long right, long n)>();
            long left2 = 0;
            foreach (var tuple in vls2)
            {
                (long v2, long l2) = tuple;
                var right2 = left2 + l2;
                sections2.Add((left2, right2, v2));
                left2 = right2;
            }

            long ans = 0;
            foreach (var tuple in sections1)
            {
                (long l1, long r1, long v1) = tuple;
            
                // section2の左端の要素を探す
                var min1 = 0;
                var max1 = sections2.Count - 1;

                while (min1 <= max1) // 範囲内にある限り探し続ける
                {
                    var mid = min1 + (max1 - min1) / 2;
                    int compareResult = l1.CompareTo(sections2[mid].right);
                    if (compareResult < 0) max1 = mid - 1;
                    else min1 = mid + 1;
                }
                var beginIdx = min1;
                ...
                // section2の右端の要素を探す
                var min2 = 0;
                var max2 = sections2.Count - 1;

                while (min2 <= max2) // 範囲内にある限り探し続ける
                {
                    var mid = min2 + (max2 - min2) / 2;
                    int compareResult = r1.CompareTo(sections2[mid].right);
                    if(compareResult > 0) min2 = mid + 1;
                    else max2 = mid - 1;
                }
                var endIdx = min2;
                ...
                {
                    (long l2, long r2, long v2) = sections2[i];
                    
                    if (v1 != v2) continue;
                    
                    var sameCnt = Math.Min(r1, r2) - Math.Max(l1, l2);
                    ans += sameCnt;   
                }
            }
