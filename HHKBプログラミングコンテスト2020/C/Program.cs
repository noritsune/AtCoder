        ...
        for (int i = 0; i < N; i++)
        {
            ...
            for (int j = 0; j < sorted.Count; j++)
            {
                if(ps[i] == sorted[j]) {
                    hasAdded = true;
                    break;
                } else if(ps[i] == sorted[j] + 1) {
                    if(j < sorted.Count - 1 && sorted[j + 1] != ps[i]) {
                        sorted.Insert(j + 1, ps[i]);
                        hasAdded = true;
                    }
                    break;
                }else if(ps[i] < sorted[j]) {
                    sorted.Insert(j, ps[i]);
                    hasAdded = true;
                    break;
                }
            }
            if(!hasAdded) sorted.Add(ps[i]);
            ...
            {
                if(j == sorted.Count - 1) {
                    Console.WriteLine(sorted.Last() + 1);
                    break;
                }

                if(sorted[j + 1] != sorted[j] + 1) {
                    Console.WriteLine(sorted[j] + 1);
                    break;
                }
            }
        }
