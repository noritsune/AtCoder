...
public class Solver {
    public void Solve()
    {
        ...
        var dishToRemainedCnt = new int[M];
        var AToDishes = new List<int>[N];
        for (int i = 0; i < N; i++)
        {
            AToDishes[i] = new List<int>();
        }
        for (int i = 0; i < M; i++)
        {
            var KAs = Ria();
        ...
        foreach (var B in Bs)
        {
            var dishes = AToDishes[B - 1];
            foreach (var dish in dishes)
            {
                dishToRemainedCnt[dish]--;
                if (dishToRemainedCnt[dish] == 0)
                {
                    ans++;
                }
            }
