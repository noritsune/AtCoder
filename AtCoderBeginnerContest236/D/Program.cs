...
namespace AtCoder {
    ...
    public class Solver {
        private int[,] funMatrix = new int[20,20];
        ...
        public void Solve()
        {
            ...
            for(int i = 1; i <= 2*N-1; i++){
                for(int j = i+1; j <= 2*N; j++){
                    funMatrix[i, j] = As[Aindex];
                    Aindex++;
                }
            }

            var maxScore = CalcScore(new bool[2 * N], new List<int[]>(), N);
            Console.WriteLine(maxScore);
        }

        private int CalcScore(bool[] isUsed, List<int[]> duos, int groupCntLimit)
        {
            if (duos.Count == groupCntLimit || isUsed.All(x => x))
            {
                int score = 0;
                foreach (var duo in duos)
                {
                    score ^= funMatrix[duo[0], duo[1] - 1];
                }
            ...
            for (int i = 0; i < isUsed.Length; i++)
            {
                ...
                duos.Add(duo);
                ans = Math.Max(ans, CalcScore(isUsed, duos, groupCntLimit));
                duos.RemoveAt(duos.Count - 1);
            }
