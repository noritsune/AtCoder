...
namespace AtCoder {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            int N = NM[0];
            int M = NM[1];
            var stationNames = rsa();
            var expressStopStationNames = rsa();
            
            var isExpressStopStation = new Dictionary<string, bool>();
            foreach (var stationName in stationNames)
            {
                isExpressStopStation.Add(stationName, false);
            }
            
            foreach (var expressStopStationName in expressStopStationNames)
            {
                isExpressStopStation[expressStopStationName] = true;
            }
            
            foreach (var stationName in stationNames)
            {
                Console.WriteLine(isExpressStopStation[stationName] ? "Yes" : "No");
            }
        }
