...
public class Solver {
    public void Solve()
    {
        ...
        int serverRefPcIdx = -1;
        var pcStrs = Enumerable.Range(0, N).Select(_ => new List<string>()).ToArray();
        var pcToStrHeadIdx = new int[N];
        var pcToRefPcIdx = Enumerable.Range(0, N).ToArray();
        for (int i = 0; i < Q; i++)
        {
            var query = Rsa();
            var p = int.Parse(query[1]) - 1;
            switch (query[0])
            {
                case "1":
                {
                    pcToStrHeadIdx[p] = pcStrs[p].Count;
                    pcToRefPcIdx[p] = serverRefPcIdx;
                    break;
                }
                case "2":
                {
                ...
                case "3":
                {
                    var targetPcIdx = pcToRefPcIdx[p];
                    while (targetPcIdx != p)
                    {
        ...
        var ansStrs = new List<string>();

        var pcIdx = serverRefPcIdx;
        while (pcIdx != -1)
        {
            ansStrs.AddRange(pcStrs[pcIdx].Skip(pcToStrHeadIdx[pcIdx]).Reverse());
            var nextPcIdx = pcToRefPcIdx[pcIdx];
            if (nextPcIdx == pcIdx) break;
            pcIdx = pcToRefPcIdx[pcIdx];
        }

        ansStrs.Reverse();
        Console.WriteLine(string.Join("", ansStrs));
    }
