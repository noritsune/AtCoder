...
public class Solver {
    public void Solve()
    {
        ...
        var manToScore = new long[N + 1];
        var scoreToCnt = new Dictionary<long, int> { [0] = N };
        for (int i = 0; i < T; i++)
        {
            var AB = Rla();
            var A = AB[0]; var B = AB[1];

            var oldScore = manToScore[A];
            scoreToCnt[oldScore]--;
            if (scoreToCnt[oldScore] == 0) scoreToCnt.Remove(oldScore);

            manToScore[A] += B;
            var newScore = manToScore[A];
            scoreToCnt.TryAdd(newScore, 0);
            scoreToCnt[newScore]++;

            Console.WriteLine(scoreToCnt.Count);
        }
    }
