...
public class Solver {
    public void Solve()
    {
        ...
        var seg = new SegTree(N, Node.Merge);
        for (int i = 0; i < N; i++)
        {
            var node = new Node(As[i]);
            seg.Set(i, node);
        }
        ...
        for (int i = 0; i < Q; i++)
        {
            ...
            {
                case 1:
                    var p = query[1] - 1; var x = query[2];
                    var lazy = new Node(x);
                    seg.Set(p, lazy);
                    break;
                case 2:
                    var l = query[1] - 1; var r = query[2] - 1;
                    var res = seg.Query(l, r + 1);
                    Console.WriteLine(res.Value.n2Cnt);
                    break;
            }
        }
    }
...
public class SegTree
{
    ...
    readonly Func <Node, Node, Node> _fx;

    public SegTree(int dataCnt, Func<Node, Node, Node> fx)
    {
        _dataCnt = dataCnt;
        _fx = fx;
    ...
    Node? QueryRec(int a, int b, int k, int l, int r)
    {
    ...
    Node? ExecFunc(Func<Node, Node, Node> func, Node? a, Node? b)
    {
