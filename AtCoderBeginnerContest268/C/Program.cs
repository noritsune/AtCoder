...
namespace KyoPro {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            var matchCountMax = int.MinValue;
            for (int dist = 0; dist < N; dist++)
            {
                var prev  = distToCount[(dist - 1 + N) % N];
                var current = distToCount[dist];
                var next    = distToCount[(dist + 1) % N];

                var matchCount = prev + current + next;
                matchCountMax = Math.Max(matchCountMax, matchCount);
            }

            Console.WriteLine(matchCountMax);
        }
