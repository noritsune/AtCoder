...
namespace KyoPro {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            var SsSum = Ss.Sum(S => S.Length);
            var jointLengthLimit = 16 - SsSum;

            var userNames = new List<string>();
            var patterns = Enumerable.Range(0, N).Perm().Select(x => x.ToArray());
            foreach (var ssIdxes in patterns)
            {
                var stack = new Stack<(string name, int jointLength, int sCnt)>();
                stack.Push((Ss[ssIdxes.First()], 0, 1));
                while (stack.Count > 0)
                {
                    (string name, int jointLength, int sCnt) = stack.Pop();
                    if(sCnt == N)
                    {
                        if (jointLength <= jointLengthLimit && name.Length >= 3 && name.Length <= 16)
                        {
                            userNames.Add(name);
                        }
                        continue;
                    ...
                    var nextS = Ss[ssIdxes[sCnt]];
                    var remainedSCnt = N - sCnt - 1;
                    var usableJointLength = jointLengthLimit - jointLength - remainedSCnt;
                    var joint = "";
                    for (int i = 0; i < usableJointLength; i++)
                    {
                        joint += "_";
                        var nextName = name + joint + nextS;
                        stack.Push((nextName, jointLength + joint.Length, sCnt + 1));
                    }
                }
            }

            var Tset = new HashSet<string>(Ts);
            foreach (var userName in userNames)
            {
                if(Tset.Contains(userName)) continue;
