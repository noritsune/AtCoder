...
public class Span {
    ...
    public override int GetHashCode() => L ^ R;

    public static Span operator +(Span a, Span b) => new Span(a.L + b.L, a.R + b.R);
    public static Span operator -(Span a, Span b) => new Span(a.L - b.L, a.R - b.R);
    public static Span operator *(Span a, int b) => new Span(a.L * b, a.R * b);

    public double DistanceTo(Span other) => Math.Sqrt(Math.Pow(L - other.L, 2) + Math.Pow(R - other.R, 2));

    public double Length => Math.Sqrt(Math.Pow(L, 2) + Math.Pow(R, 2));
}
