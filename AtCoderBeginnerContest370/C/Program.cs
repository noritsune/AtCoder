...
public class Solver {
    public void Solve()
    {
        ...
        while (new string(current) != T)
        {
            var swapIdx = 0;
            while(swapIdx < current.Length && current[swapIdx] <= T[swapIdx])
            {
                swapIdx++;
            }

            if (swapIdx == current.Length)
            {
                swapIdx--;
                while(swapIdx >= 0 && current[swapIdx] == T[swapIdx])
                {
                    swapIdx--;
                }
