...
namespace KyoPro {
    ...
    public class Solver {
        Graph<int> _graph = new Graph<int>(Graph<int>.Type.DirectedGraph);
        Dictionary<int, bool> _isConfirmed = new Dictionary<int, bool>();

        public void Solve() {
            var NM = Ria();
            var N = NM[0]; var M = NM[1];

            var startVerts = Enumerable.Range(0, N).ToHashSet();
            for (int i = 0; i < N; i++)
            {
                _graph.AddVertex(i);
            }
            for (int i = 0; i < M; i++) {
                var XY = Ria();
                _graph.AddEdge(XY[0] - 1, XY[1] - 1);

                startVerts.Remove(XY[1] - 1);
            }

            if (startVerts.Count != 1)
            {
            ...
            var isOk = Visit(startVerts.First(), sortedVerts);
            if (!isOk)
            {
                Console.WriteLine("No");
                return;
        ...
        bool Visit(int curVert, List<int> sortedVerts)
        {
            if (_isConfirmed.TryGetValue(curVert, out var isConfirmed))
            {
                return isConfirmed;
            }

            _isConfirmed[curVert] = false;

            var nextVerts = _graph.Vertices[curVert];
            if (nextVerts.Count > 1) return false;

            foreach (var nextVert in nextVerts)
            {
                var isOk = Visit(nextVert, sortedVerts);
                if (!isOk)
                {
                    return false;
                }
            ...
            _isConfirmed[curVert] = true;
            sortedVerts.Add(curVert);
            return true;
        }
