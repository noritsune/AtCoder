...
namespace KyoPro {
    ...
    public class Solver {
        Dictionary<char, Vector2> cToOffset = new Dictionary<char, Vector2> {
            {'R', new Vector2(1, 0)},
            {'L', new Vector2(-1, 0)},
            {'U', new Vector2(0, 1)},
            {'D', new Vector2(0, -1)},
        ...
        public void Solve()
        {
            ...
            var visitedPosSet = new HashSet<string> { "0 0" };
            foreach (var c in S)
            {
                var offset = cToOffset[c];
                var nextX = curX + offset.X; var nextY = curY + offset.Y;
                var nextPos = $"{nextX} {nextY}";
                if (visitedPosSet.Contains(nextPos))
                {
                    Console.WriteLine("Yes");
                    return;
                }
                visitedPosSet.Add(nextPos);
