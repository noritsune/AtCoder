    ...
    public void Solve()
    {
        ...
        for (int i = 0; i < Q; i++)
        {
            ...
            switch (query[0])
            {
                case 1:
                {
                    var u = query[1]; var v = query[2];

                    if (graph.Vertices[u].Count == 0) isolateVCnt--;
                    if (graph.Vertices[v].Count == 0) isolateVCnt--;
                ...
                case 2:
                {
                    ...
                    foreach (var to in graph.Vertices[v])
                    {
                        if (graph.Vertices[to].Count == 1) isolateVCnt++;
                    }

                    graph.RemoveAllEdges(v);
                    break;
                }
            }

            Console.WriteLine(isolateVCnt);
        }
    }
...
public class Graph<T>
{
    ...
    public bool RemoveAllEdges(T from)
    {
        if (!_vertices.ContainsKey(from)) return false;

        foreach (var to in _vertices[from])
        {
            _vertices[to].Remove(from);
        }
        _vertices[from].Clear();
        return true;
    }
