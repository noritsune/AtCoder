...
public class Solver {
    public void Solve() {
        var NMD = Rla();
        var N = NMD[0]; var M = NMD[1]; var D = NMD[2];
        var As = Rla();
        var Bs = Rla();

        var sortedAs = As.Distinct().OrderBy(x => x).ToArray();
        var sortedBs = Bs.Distinct().OrderBy(x => x).ToArray();


        long sumMax = -1;
        foreach (var A in sortedAs)
        {
            var gtBIdx = LowerBound(sortedBs, A - D, Comparer<long>.Default);
            if (gtBIdx < sortedBs.Length)
            {
                var greaterB = sortedBs[gtBIdx];
                if (greaterB - A <= D)
                {
                    sumMax = Math.Max(sumMax, A + greaterB);
                }
            }

            var ltBIdx = LowerBoundUnder(sortedBs, A + D, Comparer<long>.Default);
            if (ltBIdx >= 0)
            {
                var lessB = sortedBs[ltBIdx];
                if (A - lessB <= D)
                {
                    sumMax = Math.Max(sumMax, A + lessB);
                }
            }
        }
