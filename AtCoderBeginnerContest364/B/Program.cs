...
public class Solver {
    public void Solve()
    {
        ...
        var SiSj = Ria();
        var Si = SiSj[0]; var Sj = SiSj[1];
        var grid = new string[H];
        for (int i = 0; i < H; i++) grid[i] = Rs();
        var Xs = Rs();

        var dirToXOffset = new Dictionary<char, int> {
            {'U', 0},
            {'D', 0},
            {'L', -1},
            {'R', 1}
        };
        var dirToYOffset = new Dictionary<char, int> {
            {'U', -1},
            {'D', 1},
            {'L', 0},
            {'R', 0}
        ...
        var x = Si - 1; var y = Sj - 1;
        foreach (var X in Xs)
        {
            var dx = dirToXOffset[X]; var dy = dirToYOffset[X];
