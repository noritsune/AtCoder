...
public class Solver {
    public void Solve() {
        ...
        var graph = new Graph<int>(GraphType.Undirected);
        var fromAndToToOffset = new Dictionary<int, Dictionary<int, Vector2>>();
        for (int i = 1; i <= N; i++) graph.AddVertex(i);
        for (int i = 0; i < M; i++)
        {
            var ABXY = Rla();
            var A = (int)ABXY[0]; var B = (int)ABXY[1]; var X = ABXY[2]; var Y = ABXY[3];

            graph.AddEdge(A, B);

            fromAndToToOffset.TryAdd(A, new Dictionary<int, Vector2>());
            fromAndToToOffset[A].TryAdd(B, new Vector2(X, Y));
            fromAndToToOffset.TryAdd(B, new Dictionary<int, Vector2>());
            fromAndToToOffset[B].TryAdd(A, new Vector2(-X, -Y));
        }

        var poss = new Vector2[N + 1];
        poss[1] = new Vector2(0, 0);
        ...
        while (queue.Count > 0)
        {
            ...
            {
                if (poss[to] != null) continue;

                poss[to] = poss[from] + fromAndToToOffset[from][to];
                queue.Enqueue(to);
            }
        }
        ...
        {
            var pos = poss[i];
            Console.WriteLine(pos != null ? $"{pos.X} {pos.Y}" : "undecidable");
        }
    }
