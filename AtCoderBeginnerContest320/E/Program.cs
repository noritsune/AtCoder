...
public class Solver {
    public void Solve() {
        ...
        var availablePeople = new PriorityQueue<int>(false);
        for (int i = 1; i <= N; i++) availablePeople.Enqueue(i);

        var idxToWSum = new long[N + 1];
        var nonAvailablePeople = new SortedDictionary<long, List<int>>();
        for (int i = 0; i < M; i++)
        {
            var TWS = Rla();
            var T = TWS[0]; var W = TWS[1]; var S = TWS[2];

            while (nonAvailablePeople.Any() && nonAvailablePeople.First().Key <= T)
            {
                var returnPeople = nonAvailablePeople.First().Value;
                foreach (var returnPerson in returnPeople)
                {
                    availablePeople.Enqueue(returnPerson);
                }
                nonAvailablePeople.Remove(nonAvailablePeople.First().Key);
            }

            if (!availablePeople.Any()) continue;

            var headIdx = availablePeople.Dequeue();
            idxToWSum[headIdx] += W;
            var returnT = T + S;
            nonAvailablePeople.TryAdd(returnT, new List<int>());
            nonAvailablePeople[returnT].Add(headIdx);
        }

        for (int i = 1; i <= N; i++)
        {
            Console.WriteLine(idxToWSum[i]);
        }
    }
