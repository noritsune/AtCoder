...
public class Solver {
    public void Solve2()
    {
        ...
        var numToIdxes123 = new List<int>[10, 3];
        for (int i = 0; i < 10; i++)
        {
            for (int j = 0; j < 3; j++)
            {
                numToIdxes123[i, j] = new List<int>();
            }
        }
        for (int i = 0; i < M * 3; i++)
        {
            for (int j = 0; j < 3; j++)
            {
                numToIdxes123[Ss[j][i % M] - '0', j].Add(i);
            }
        }
        ...
        for (int i = 0; i < 10; i++)
        {
            ...
            for (int j = 0; j < 3; j++)
            {
                isContainAll &= numToIdxes123[i, j].Any();
            }
            if (!isContainAll) continue;
            ...
            for (int j1 = 0; j1 < 3; j1++)
            {
                for (int j2 = 0; j2 < 3; j2++)
                {
                    for (int j3 = 0; j3 < 3; j3++)
                    {
                        ...
                        var t1 = numToIdxes123[i, j1][0];
                        var t2Ub = UpperBound(numToIdxes123[i, j2], t1, Comparer<int>.Default);
                        var t2 = numToIdxes123[i, j2][t2Ub];
                        var t3Ub = UpperBound(numToIdxes123[i, j3], t2, Comparer<int>.Default);
                        var t3 = numToIdxes123[i, j3][t3Ub];

                        minT = Math.Min(minT, t3);
                    }
                }
            }
        }

        Console.WriteLine(minT == int.MaxValue ? -1 : minT);
    }

    public void Solve1()
    {
        var M = Ri();
        var S1 = Rs(); var S2 = Rs(); var S3 = Rs();
        ...
        for (int i1 = 0; i1 < M; i1++)
        {
            ...
            bool isFoundSame2 = false;
            for (int i2Offset = t; i2Offset < M; i2Offset++)
            {
                var i2 = (t + i2Offset) % M;
                if (S2[i2] == num1)
                {
            ...
            if (!isFoundSame2) continue;

            bool isFoundSame3 = false;
            for (int i3Offset = t; i3Offset < M; i3Offset++)
            {
                var i3 = (t + i3Offset) % M;
                if (S3[i3] == num1)
                {
