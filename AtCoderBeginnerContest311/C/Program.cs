...
public class Solver
{
    ...
    public void Solve() {
        ...
        _graph = new Graph<int>(GraphType.Directed);
        for (int i = 1; i <= N; i++) _graph.AddVertex(i);
        for (int i = 0; i < N; i++)
        {
            _graph.AddEdge(i + 1, As[i]);
        }
        ...
        for (int i = 1; i <= N; i++)
        {
            ...
            var (isOk, loopPath) = Dfs(i, new bool[N + 1], new bool[N + 1], new Stack<int>());
            if (isOk)
            {
                Console.WriteLine(loopPath.Count);
                Console.WriteLine(string.Join(" ", loopPath));
                return;
            }
        }

        throw new Exception("閉路が見つからない");
    }



    (bool isOk, Stack<int> loopPath) Dfs(int v, bool[] iki, bool[] kaeri, Stack<int> path)
    {
        _visited[v] = true;
        iki[v] = true;
        path.Push(v);

        foreach (var nextV in _graph.Vertices[v])
        {
            if (kaeri[nextV]) continue;
            if (iki[nextV] && !kaeri[nextV])
            {
                var ansPath = new Stack<int>();
                while (path.Any())
                {
                    var prevV = path.Pop();
                    ansPath.Push(prevV);
                    if (prevV == nextV) break;
                }
                return (true, ansPath);
            }
            var (isOk, loopPath) = Dfs(nextV, iki, kaeri, path);
            if (isOk) return (true, loopPath);
        }
