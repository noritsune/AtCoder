...
public class Solver {
    public void Solve()
    {
        ...
        var hasStopped = new bool[N, M];
        var canStep = new bool[N, M];
        ...
        while (q.Any())
        {
            ...
            foreach (var (dx, dy) in offsets)
            {
                ...
                {
                    canStep[nextY, nextX] = true;
                    nextX += dx; nextY += dy;
                }

                var stopX = nextX - dx; var stopY = nextY - dy;
                if (hasStopped[stopY, stopX]) continue;
                hasStopped[stopY, stopX] = true;

                q.Enqueue((stopX, stopY));
            }
        }
