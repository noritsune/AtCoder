...
public class Solver
{
    ...
    {
        var HWN = Ria();
        var H = HWN[0]; var W = HWN[1]; var N = HWN[2];
        var abs = new List<(int a, int b)>();
        for (int i = 0; i < N; i++)
        {
            var ab = Ria();
            abs.Add((ab[0] - 1, ab[1] - 1));
        }

        var isHole = new bool[H, W];
        var yToHoleXs = new Dictionary<int, List<int>>();
        foreach ((int y, int x) in abs)
        {
            isHole[y, x] = true;

            yToHoleXs.TryAdd(y, new List<int>());
            yToHoleXs[y].Add(x);
        }

        foreach (var value in yToHoleXs.Values)
        {
            value.Sort();
        }

        long[] sizeToSquareSum = new long[Math.Min(H, W) + 1];
        for (int i = 1; i < sizeToSquareSum.Length; i++)
        {
            sizeToSquareSum[i] = sizeToSquareSum[i - 1] + i * i;
        }
        ...
        for (int y = 0; y < H; y++)
        {
            for (int x = 0; x < W; x++)
            {
                if (isHole[y, x]) continue;
                // if (visited[y, x]) continue;
                visited[y, x] = true;

                var size = 1;
                var nextY = y; var nextX = x;
                var sizeLimit = long.MaxValue;
                while (nextY < H && nextX < W)
                {
                    long right = 0;
                    if (yToHoleXs.TryGetValue(nextY, out var holeXs))
                    {
                        var ub = LowerBound(holeXs, x, Comparer<int>.Default);
                        right = ub < holeXs.Count ? holeXs[ub] : W;
                    }
                    else
                    {
                        right = W;
                    ...
                    long rowSize = right - x;
                    sizeLimit = Math.Min(sizeLimit, rowSize);
                    if (size >= sizeLimit)
                    {
                        size--;
                        break;
