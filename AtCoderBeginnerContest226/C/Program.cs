...
namespace AtCoder {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            for (int i = 0; i < N; i++) skills.Add(ria());
            ...
            {
                var skill = skills[i];
                var preSkills = skill.Skip(2);
                foreach (var preSkill in preSkills)
                {
                    graph.AddEdge(i + 1, preSkill);
                }
            }

            var q = new Queue<int>();
            q.Enqueue(N);
            var needSkillIds = new HashSet<int>();
            while (q.Count > 0)
            {
                int currentSkillId = q.Dequeue();
                needSkillIds.Add(currentSkillId);

                var nextSkillIds = graph.Vertices[currentSkillId];
                foreach (var nextSkillId in nextSkillIds)
                {
                    if(needSkillIds.Contains(nextSkillId)) continue;
                    q.Enqueue(nextSkillId);
                }
            }

            long ans = 0;
            foreach (var needSkillId in needSkillIds)
            {
                ans += skills[needSkillId - 1][0];
            }
            Console.WriteLine(ans);
        }
        
        public void SolveDijekstra()
        {
            int N = ri();
            var skills = new List<int[]>();
            for (int i = 0; i < N; i++) skills.Add(ria());
            ...
            for (int i = 0; i < N; i++)
            {
                ...
                foreach (var preSkill in preSkills)
                {
                    int preSkillCost = skills[preSkill - 1][0];
                    dijkstra.Add(i + 1, preSkill, preSkillCost);
                }
            }

            var minCosts = dijkstra.GetMinCost(N);

            var lastCost = skills.Last()[0];
            var ans = minCosts.Where(cost => cost != 1000000000000000000).Sum() + lastCost;
            Console.WriteLine(ans);
        }
