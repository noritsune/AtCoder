...
namespace AtCoder {
    ...
    public class BallInfo
    {
        ...
        public BallInfo(int value, int cnt)
        {
            this.value = value;
            this.cnt = cnt;
        }
    ...
    public class Solver {
        public void Solve()
        {
            ...
            var cylinder = new Queue<BallInfo>();
            for (int i = 0; i < Q; i++)
            {
                var query = ria();
                int x, c;
                switch (query[0])
                {
                    case 1:
                        ...
                        cylinder.Enqueue(new BallInfo(x, c));
                        break;
                    case 2:
                        c = query[1];
                        
                        long valueSum = 0;
                        long remainedBallCnt = c;
                        var nextBallInfo = cylinder.Peek();
                        while (cylinder.Count > 0 && remainedBallCnt >= nextBallInfo.cnt)
                        {
                            remainedBallCnt -= nextBallInfo.cnt;
                            valueSum += nextBallInfo.value * nextBallInfo.cnt;
                            cylinder.Dequeue();
                            
                            if (cylinder.Count == 0) break;
                            
                            nextBallInfo = cylinder.Peek();
                        }

                        if (cylinder.Count > 0)
                        {
                            valueSum += nextBallInfo.value * remainedBallCnt;
                            nextBallInfo.cnt -= remainedBallCnt;
                        }
