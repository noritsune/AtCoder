...
namespace AtCoder {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            var sts = new List<(string s, string t)>();
            for (int i = 0; i < N; i++)
            {
                var st = rsa();
                sts.Add((st[0], st[1]));
            ...
            for (int i = 0; i < N; i++)
            {
                bool canUseS = !nickNames.Contains(sts[i].s);
                bool canUseT = !nickNames.Contains(sts[i].t);
                for (int j = 0; j < N; j++)
                {
                    if(i == j) continue;
                    canUseS &= sts[i].s != sts[j].s && sts[i].s != sts[j].t;
                    canUseT &= sts[i].t != sts[j].s && sts[i].t != sts[j].t;
                }
                
                if (canUseS)
                {
                    nickNames.Add(sts[i].s);
                }
                else if (canUseT)
                {
                    nickNames.Add(sts[i].t);
                }
                else
                {
