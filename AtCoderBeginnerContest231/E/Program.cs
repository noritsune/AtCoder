...
namespace AtCoder {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            Array.Sort(As, (a, b) => b.CompareTo(a));
            ...
            for (int i = 0; i < N; i++)
            {
                ...
                long count_for_pay = X / A;
                if (count_for_pay * A < X) count_for_pay++;
                
                count_all += count_for_pay;
                
                long change = count_for_pay * A - X;
                long count_for_change = 0;
                for (int j = i + 1; j < N; j++)
                {
                    long cnt = change / As[j];
                    count_for_change += cnt;
                    change -= cnt * As[j];
                    
                    if(change == 0) break;
                }
                if(change != 0) continue;
                count_all += count_for_change;

                ans = Math.Min(ans, count_all);
            }
