...
class Sol{
	...
	public void Solve(){
        ...
        if(strN.Length / 2.0 == strN.Length / 2) {
            long before = long.Parse(strN.Substring(0, strN.Length / 2));
            long after = long.Parse(strN.Substring(strN.Length / 2));

            ans += before;
            if(after < before) ans--;
        } else {
            ans = (long)Math.Pow(10, (int)(strN.Length / 2)) - 1;
        }
    ...
    public void Solve2() {
        ...
        for (int i = 11; i <= N; i++)
        {
            ...
            if(strI.Length / 2 != (int)(strI.Length / 2)) return;

            string before = strI.Substring(0, strI.Length / 2);
            string after = strI.Substring(strI.Length / 2);
            if(before == after) {
                ans++;
            }
        }
