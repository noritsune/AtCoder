...
namespace util {
    ...
    public class Sol{
        ...
        public void SolveTest()
        {
            ...
            long ans = 0;
            var ranges = new Vector2[N + 1];
            ranges[0] = new Vector2(1, 1);
            for (int i = 1; i <= N; i++)
            {
                ans = 0;
                int min = Math.Min(aArray[i - 1], bArray[i - 1]);
                int max = Math.Max(aArray[i - 1], bArray[i - 1]);
                ranges[i] = new Vector2(min, max);

                int lastMin = ranges[i - 1]._x;
                int lastMax = ranges[i - 1]._y;
                
                for (int j = lastMin; j <= lastMax; j++)
                {
                    ans += max - Math.Max(j, min) + 1;
