...
namespace util {
    ...
    public class Sol{
        ...
        public void Solve()
        {
            ...
            for (int i = 0; i < 2 * N; i++) Ass.Add(rs());

            var ranking = new List<Vector2>();
            for (int i = 0; i < 2 * N; i++) ranking.Add(new Vector2(i, 0));
            
            for (int i = 0; i < M; i++)
            {
                for (int j = 0; j < ranking.Count; j += 2)
                {
                    char aHand = Ass[ranking[j    ]._x][i];
                    char bHand = Ass[ranking[j + 1]._x][i];
                    
                    if (isAWin(aHand, bHand))   ranking[j    ]._y++;
                    if (isAWin(bHand, aHand)) ranking[j + 1]._y++;
                }
                
                ranking.Sort((a, b) =>
                {
                    var scoreDiff = b._y - a._y;
                    if (scoreDiff != 0) return scoreDiff;
                    return a._x - b._x;
                });
            }
            
            foreach (var person in ranking)
            {
                Console.WriteLine(person._x + 1);
            }
        }

        private bool isAWin(char a, char b)
        {
            switch (a)
            {
                case 'G':
            ...
            throw new SystemException("手が未定義です");
        }
