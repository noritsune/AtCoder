...
public class Solver {
    public void Solve()
    {
        ...
        var xCenter = (xs.Max() + xs.Min()) / 2.0;
        var yCenter = (ys.Max() + ys.Min()) / 2.0;

        var xCenterDiff = xs.Select(x => Math.Abs(x - xCenter));
        var yCenterDiff = ys.Select(y => Math.Abs(y - yCenter));

        var xCenterDiffMax = Math.Ceiling(xCenterDiff.Max());
        var yCenterDiffMax = Math.Ceiling(yCenterDiff.Max());

        Console.WriteLine(Math.Max(xCenterDiffMax, yCenterDiffMax));
    }
