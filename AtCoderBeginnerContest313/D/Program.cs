...
public class Solver {
    public void Solve()
    {
        ...
        var xIdxes = Enumerable.Range(1, K).ToHashSet();

        Console.WriteLine($"? {string.Join(" ", xIdxes)}");
        Console.Clear();
        var prevRet = Ri();
        ...
        for (int i = 0; i < N - 1; i++)
        {
            xIdxes.Remove(removeNum);
            xIdxes.Add(addNum);

            Console.WriteLine($"? {string.Join(" ", xIdxes.OrderBy(n => n))}");
            Console.Clear();
            var ret = Ri();

            if (ret == prevRet)
            {
                uf.Union(removeNum, addNum);
            }

            prevRet = ret;
            removeNum++;
            addNum = addNum == N ? 1 : addNum + 1;
        }

        var firstNum = prevRet;
        var notFirstNum = firstNum == 0 ? 1 : 0;
        var ans = new int[N];
        for (int i = 1; i <= N; i++)
        {
            ans[i - 1] = uf.Same(1, i) ? firstNum : notFirstNum;
        }
