...
public class Solver
{
    ...
    public void Solve() {
        ...
        _cs = _S.ToCharArray();
        Console.WriteLine(Dfs(0, _N) == _N ? "Yes" : "No");
    }

    int Dfs(int begin, int end)
    {
        var replacedCnt = 0;
        for (int i = begin; i <= end - _M; i++)
        {
            for (int j = 0; j < _M; j++)
            {
                if (_cs[i] != '#' && _cs[i] != _T[j]) continue;

                bool isOk = true;
                for (int k = j; k < _M; k++)
                {
                    if (_cs[i + k] != '#' && _cs[i + k] != _T[k])
                    {
                        isOk = false;
                        break;
                    }
                ...
                replacedCnt += Dfs(begin, Math.Min(_N, i + _M - 1));
                replacedCnt += Dfs(Math.Max(0, i + _M - 1 - 1), end);
                break;
            }
        }
