...
public class Solver {
    public void Solve() {
        ...
        var elecCntToSortedMans = new SortedSet<int>[M + 1];
        for (int i = 0; i <= M; i++)
        {
            elecCntToSortedMans[i] = new SortedSet<int>();
        }

        var manToElecCnt = new int[N + 1];
        for (int i = 1; i <= N; i++)
        {
            manToElecCnt[i] = 0;
        }
        ...
        foreach (var A in As)
        {
            var oldElecCnt = manToElecCnt[A];
            elecCntToSortedMans[oldElecCnt].Remove(A);
            elecCntToSortedMans[oldElecCnt + 1].Add(A);
            manToElecCnt[A]++;

            maxElecCnt = Math.Max(maxElecCnt, oldElecCnt + 1);

            Console.WriteLine(elecCntToSortedMans[maxElecCnt].First());
        }
    }
