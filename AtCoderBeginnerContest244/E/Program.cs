...
namespace AtCoder {
    ...
    public class Solver
    {
        ...
        public void Solve()
        {
            ...
            // 頂点Sから辺をi回通って頂点jまで行く方法の数。kは頂点Xを通った回数のmod2
            var dp = new long[K + 1, N + 1, 2];
            dp[0, S, 0] = 1;
            for (int i = 0; i < K; i++)
            {
                for (int j = 1; j <= N; j++)
                {
                    for (int k = 0; k <= 1; k++)
                    {
                        foreach (var neighbor in graph.Vertices[j])
                        {
                            int kNext = neighbor == X ? (k + 1) % 2 : k;
                            
                            dp[i + 1, neighbor, kNext] += dp[i, j, k];
                            dp[i + 1, neighbor, kNext] %= MOD;
                        }
                    }
                }
            }

            Console.WriteLine(dp[K, T, 0]);
        }
