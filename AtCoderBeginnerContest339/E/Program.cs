...
public class Solver {
    public void Solve2()
    {
        ...
        // dp[n]: 末尾の要素がnであるような数列の最大長
        var dp = SegTree.CreateMaxSegTree(AsMax);
        foreach (var A in As)
        {
            var l = Math.Max(0, A - D);
            var r = Math.Min(AsMax, A + D);
            var maxLen = dp.QueryRange(l, r + 1);
            dp.Set(A, maxLen + 1);
        }

        Console.WriteLine(dp.QueryRangeAll());
    }
    ...
    public void Solve1()
    {
        ...
        var numToCnt = new int[500002];
        foreach (var A in As)
        {
            numToCnt[A]++;
        }

        var rui = new int[500002];
        for (int i = 1; i < 500001; i++)
        {
            rui[i] = rui[i - 1] + numToCnt[i];
        }

        var ascSortedNums = As.Distinct().OrderBy(x => x).ToArray();

        var maxLen = 0;
        foreach (var m in ascSortedNums)
        {
            var lIdx = LowerBoundUnder(ascSortedNums, m - D, Comparer<int>.Default);
            var rIdx = LowerBoundUnder(ascSortedNums, m + D, Comparer<int>.Default);
            var l = lIdx == -1 ? ascSortedNums[0] : ascSortedNums[lIdx];
            var r = rIdx == -1 ? ascSortedNums[0] : ascSortedNums[rIdx];
            var len = rui[r] - rui[l - 1];
