...
namespace util {
    ...
    public class Sol{
        ...
        public void Solve()
        {
            ...
            var XIndexMap = new Dictionary<char, int>();
            for (var i = 0; i < X.Length; i++)
            {
                XIndexMap.Add(X[i], i + 1);
            }
            
            var SsTranslated = new List<dynamic[]>();
            foreach (var S in Ss)
            {
                var STranslated = S.Select(c => XIndexMap[c]).ToArray();
                SsTranslated.Add(new dynamic[]{ S, STranslated });
            }
            
            SsTranslated.Sort((aArray, bArray) =>
            {
                int[] a = aArray[1];
                int[] b = bArray[1];
            ...
            foreach (var STranslated in SsTranslated)
            {
                Console.WriteLine(STranslated[0]);
            }
        ...
        public void SolveTest()
        {
            ...
            var chatToXMap = new Dictionary<char, char>();
            for (var i = 0; i < X.Length; i++)
            {
                chatToXMap.Add((char)('a' + i), X[i]);
            }

            var SsTranslated = new List<string[]>();
            foreach (var S in Ss)
            {
                var STranslated = new string(S.Select(c => chatToXMap[c]).ToArray());
                SsTranslated.Add(new string[]{ S, STranslated });
            }
            
            SsTranslated.Sort((a, b) => string.Compare(a[1], b[1], StringComparison.Ordinal));
