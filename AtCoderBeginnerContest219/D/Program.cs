...
namespace util {
    ...
    public class Sol{
        ...
        public void Solve()
        {
            ...
            int[,,] dp = new int[N + 1, Xreq + 1, Yreq + 1];
            for (int i = 0; i <= N; i++)
            {
                for (int j = 0; j <= Xreq; j++)
                {
                    for (int k = 0; k <= Yreq; k++) dp[i, j, k] = int.MaxValue - 1;
                }
            }
            ...
            for (int i = 1; i <= N; i++)
            {
                for (int j = 0; j <= Xreq; j++)
                {
                    for (int k = 0; k <= Yreq; k++)
                    {
                        int X = XYs[i - 1][0];
                        int Y = XYs[i - 1][1];
                        dp[i, Math.Min(j + X, Xreq), Math.Min(k + Y, Yreq)] = Math.Min(
                            dp[i, Math.Min(j + X, Xreq), Math.Min(k + Y, Yreq)],
                            dp[i - 1, j, k] + 1
                        );
            ...
            int ans = dp[N, Xreq, Yreq];
            Console.WriteLine(ans >= int.MaxValue - 1 ? -1 : ans);
            Console.ReadLine();
        }
