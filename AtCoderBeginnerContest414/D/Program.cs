...
public class Solver {
    public void Solve()
    {
        var NM = Rla();
        var (N, M) = (NM[0], NM[1]);
        var Xs = Rla().Distinct().OrderBy(x => x).ToArray();

        var distAndLRs = new List<(long Dist, long L, long R)>();
        for (int i = 0; i < Xs.Length - 1; i++)
        {
            var L = Xs[i]; var R = Xs[i + 1];
            var dist = R - L;
            distAndLRs.Add((dist, L, R));
        }

        distAndLRs.Sort((a, b) => a.Dist.CompareTo(b.Dist));

        var coveredXs = new HashSet<long>();
        long denpaTowerCnt = 0;
        long denpaTowerPowerSum = 0;
        foreach (var (dist, l, r) in distAndLRs)
        {
            var nonCoveredCnt = Xs.Length - coveredXs.Count;
            if (nonCoveredCnt + denpaTowerCnt <= M) break;

            if (coveredXs.Contains(l) && coveredXs.Contains(r))
            {
                denpaTowerCnt--;
            }
            else if (!coveredXs.Contains(l) && !coveredXs.Contains(r))
            {
                denpaTowerCnt++;
            }

            denpaTowerPowerSum += dist;

            coveredXs.Add(l);
            coveredXs.Add(r);
        }

        Console.WriteLine(denpaTowerPowerSum);
    }
