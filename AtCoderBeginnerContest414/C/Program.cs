...
public class Solver
{
    ...
    public void Solve()
    {
        ...
        {
            var iStr = i.ToString();
            var iStrReversed = new string(i.ToString().Reverse().ToArray());

            var kaibun1 = iStr + iStrReversed;
            if (IsValid(kaibun1))
            {
                ans += long.Parse(kaibun1);
            }

            var kaibun2 = iStr + iStrReversed.Substring(1);
            if (IsValid(kaibun2))
            {
                ans += long.Parse(kaibun2);
            }
        }
    ...
    bool IsValid(string numStr)
    {
        ...
        return num <= N && IsKaibun(ConvertBase(num));
    }
    ...
    string ConvertBase(long num)
    {
        if (num == 0) return "0";
        var result = new List<char>();
        while (num > 0)
        {
            result.Add((char)('0' + num % A));
            num /= A;
        }
        result.Reverse();
