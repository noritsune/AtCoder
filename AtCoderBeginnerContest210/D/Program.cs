...
namespace util {
    ...
    public class Sol{
        ...
        public void SolveTest(){
            ...
            for (int i = 0; i < H; i++) As.Add(ria());

            long allCostMin = int.MaxValue;
            for (int i = 0; i < H; i++)
            {
                for (int j = 0; j < W; j++)
                {
                    int startCost = As[i][j];
                    for (int y = 0; y < H; y++)
                    {
                        for (int x = 0; x < W; x++)
                        {
                            ...
                            int endCost = As[y][x];
                            long dist = Math.Abs(i - y) + Math.Abs(j - x);
                            long railCost = C * dist;
                            long allCost = startCost + endCost + railCost;
                            allCostMin = Math.Min(allCost, allCostMin);
                        }
                    }
                }
            }
            
            Console.WriteLine(allCostMin);
            Console.ReadLine();
        }
