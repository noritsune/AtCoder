...
namespace util {
    ...
    public class Sol{
        ...
        public void Solve(){
            ...
            int colorCntMax = int.MinValue;
            for (int i = 0; i < N; i++)
            {
                int addColor = cs[i];
                if(colors.ContainsKey(addColor)) {
                    colors[addColor]++;
                } else {
                    colors.Add(addColor, 1);
                }

                if(i >= K) {
                    int removeColor = cs[i - K];
                    if(colors[removeColor] > 1) {
                        colors[removeColor]--;
                    } else {
                ...
                colorCntMax = Math.Max(colors.Keys.Count(), colorCntMax);
            }
            
            Console.WriteLine(colorCntMax);
            Console.ReadLine();
        }
