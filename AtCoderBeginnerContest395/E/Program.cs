...
public class Solver {
    ...
    {
        var NMX = Rla();
        var (N, M, X) = ((int)NMX[0], NMX[1], NMX[2]);

        var dijkstra = new Dijkstra(N * 2);
        for (int i = 0; i < M; i++)
        {
            var uv = Ria();
            var (u, v) = (uv[0] - 1, uv[1] - 1);
            dijkstra.Add(u, v, 1);
            dijkstra.Add(v + N, u + N, 1);
        }

        for (int i = 0; i < N; i++)
        {
            dijkstra.Add(i, i + N, X);
            dijkstra.Add(i + N, i, X);
        }

        var (costs, _) = dijkstra.GetClosestRoot(0);
        var ans = Math.Min(costs[N - 1], costs[2 * N - 1]);
