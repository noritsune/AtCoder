...
public class Solver {
    public void Solve()
    {
        ...
        var hatoToSu = Enumerable.Range(0, N).ToArray();
        var altSuToSu = Enumerable.Range(0, N).ToArray();
        var suToAltSu = Enumerable.Range(0, N).ToArray();
        ...
        for (int i = 0; i < Q; i++)
        {
            ...
            switch (query[0])
            {
                case 1:
                {
                    ...
                    hatoToSu[a] = altSuToSu[b];
                    break;
                }
                case 2:
                {
                    var (a, b) = (query[1] - 1, query[2] - 1);
                    (altSuToSu[a], altSuToSu[b]) = (altSuToSu[b], altSuToSu[a]);

                    suToAltSu[altSuToSu[a]] = a;
                    suToAltSu[altSuToSu[b]] = b;
                    break;
                }
                case 3:
                {
                    var a = query[1] - 1;
                    var su = hatoToSu[a];
                    var altSu = suToAltSu[su];
                    Console.WriteLine(altSu + 1);
                    break;
                }
            }
        }
