...
class Sol{
    ...
    double[,,] memo = new double[101, 101, 101];
	public void Solve(){
		int[] ABC = ria();
        int A = ABC[0];
        int B = ABC[1];
    ...
    double func(int a, int b, int c) {
        ...
        double ans = 0;
        int sum = a + b + c;
        ans += ((func(a + 1, b    , c    ) + 1) * a / sum);
        ans += ((func(a    , b + 1, c    ) + 1) * b / sum);
        ans += ((func(a    , b    , c + 1) + 1) * c / sum);
...
public static class Combination {
    ...
    public static IEnumerable<T[]> Enumerate<T>(this IEnumerable<T> items, int k, bool withRepetition) {
        if (k == 1) {
