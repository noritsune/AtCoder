	...
	public void Solve(){
        ...
        if(r1 == r2 && c1 == c2) {
            ans = 0;
                    ...
                    r1 - c1 == r2 - c2 ||
                    Math.Abs(r1 - r2) + Math.Abs(c1 - c2) <= 3) { //そのまま行けるとき
            ans = 1;
        } else if ( Math.Abs(r1 - c1) % 2 == Math.Abs(r2 - c2) % 2 || //斜め2回で行ける時
        // } else if ( (r1 + c1 + r2 + c2) % 2 == 0 || //斜め2回で行ける時
                    Math.Abs((r1 + c1) - (r2 + c2)) <= 3 || //上下左右→斜めで行ける時
                    Math.Abs((r1 - c1) - (r2 - c2)) <= 3 || //上下左右→斜めで行ける時
                    Math.Abs(r1 - r2) + Math.Abs(c1 - c2) <= 6) { //上下左右→上下左右で行ける時
            ans = 2;
        } else {
            ans = 3;
        }
