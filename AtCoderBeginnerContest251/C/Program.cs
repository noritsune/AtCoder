...
namespace KyoPro {
    ...
    public class Solver {
        ...
        {
            int N = Ri();
            var STs = new List<(string S, int T)>();
            for (int i = 0; i < N; i++)
            {
                var ST = Rsa();
                var S = ST[0];
            ...
            var maxOriginalScore = int.MinValue;
            int maxOriginalScoreIndex = -1;
            var existSs = new HashSet<string>();
            for (int i = 0; i < N; i++)
            {
                string S = STs[i].S;
                int T = STs[i].T;
                
                if(existSs.Contains(S)) continue;
                existSs.Add(S);
                
                if(T <= maxOriginalScore) continue;
                
                maxOriginalScore = T;
                maxOriginalScoreIndex = i;
            }

            Console.WriteLine(maxOriginalScoreIndex + 1);
        }
