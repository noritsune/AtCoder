...
namespace KyoPro {
    ...
    public class Solver {
        public void Solve2()
        {
            ...
            var knownFloors = new HashSet<int> { 1 };
            for (int i = 0; i < N; i++)
            {
                var AB = Ria();
                var A = AB[0];
            ...
            var idxToFloor = knownFloors.OrderBy(n => n).ToList();
            var floorToIdx = new Dictionary<int, int>();
            for (int i = 0; i < idxToFloor.Count; i++)
            {
                floorToIdx[idxToFloor[i]] = i;
            }

            var uf = new UnionFind(knownFloors.Count);
            foreach ((int A, int B) in ABs)
            {
                var AIdx = floorToIdx[A];
                var BIdx = floorToIdx[B];
                uf.Union(AIdx, BIdx);
            }

            var reachableFloors = new List<int>();
            for (int idx = 0; idx < knownFloors.Count; idx++)
            {
                if(uf.Same(0, idx))
                {
                    reachableFloors.Add(idxToFloor[idx]);
                }
            }

            var maxReachableFloor = reachableFloors.Max();
            Console.WriteLine(maxReachableFloor);
        }
        ...
        public void Solve1()
        {
            ...
            var knownFloors = new HashSet<int>();
            for (int i = 0; i < N; i++)
            {
                var AB = Ria();
                var A = AB[0];
            ...
            foreach (var knownFloor in knownFloors)
            {
                graph.AddVertex(knownFloor);
            }
            foreach ((int A, int B) in ABs)
            {
                graph.AddEdge(A, B);
            ...
            var usedLadders = new HashSet<long>();
            var maxFloor = 1;
            var q = new Queue<int>();
            q.Enqueue(1);
            while (q.Count > 0)
            {
                var currentFloor = q.Dequeue();
                maxFloor = Math.Max(maxFloor, currentFloor);

                var nextFloors = graph.Vertices.GetValueOrDefault(currentFloor);
                if(nextFloors == null) continue;

                foreach (var nextFloor in nextFloors)
                {
                    long ladder = (long)currentFloor * 1000000000 + nextFloor;
                    long reverseLadder = (long)nextFloor * 1000000000 + currentFloor;
                    if (usedLadders.Contains(ladder) || usedLadders.Contains(reverseLadder))
                    {
                        continue;
                    }
