...
namespace KyoPro {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            var dijkstra = new Dijkstra(N * 2 + 1);
            for (int i = 0; i < M; i++)
            {
                var uva = Ria();
                var u = uva[0];
                ...
                {
                    dijkstra.Add(u + N, v + N);
                    dijkstra.Add(v + N, u + N);
                }
            }
            ...
            if (K > 0)
            {
                ...
                foreach (var s in ss)
                {
                    dijkstra.Add(s, s + N, 0);
                    dijkstra.Add(s + N, s, 0);
                }
            }

            var minCosts = dijkstra.GetMinCost(1);
            var minCostInOpened = minCosts[N];
            var minCostInClosed = minCosts[N * 2];

            var ans = Math.Min(minCostInOpened, minCostInClosed);
