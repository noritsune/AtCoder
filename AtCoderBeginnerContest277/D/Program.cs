...
namespace KyoPro {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            var sortedCards = As.OrderBy(n => n).ToArray();

            var deletableCardGroups = new List<List<long>>
            {
                new List<long> { sortedCards.First() }
            };

            for (int i = 0; i < sortedCards.Length; i++)
            {
                var currentCard = sortedCards[i];
                var nextCard = sortedCards[(i + 1) % sortedCards.Length];
                if(currentCard == nextCard || (currentCard + 1) % M == nextCard)
                {
                    if(i == sortedCards.Length - 1)
                    {
                        if (deletableCardGroups.Count > 1)
                        {
                            // merge first and last
                            deletableCardGroups[0].AddRange(deletableCardGroups.Last());
                            deletableCardGroups.RemoveAt(deletableCardGroups.Count - 1);
                        }
                    }
                    else
                    {
                        deletableCardGroups.Last().Add(nextCard);
                    }
                }
                else
                {
                    if (i < sortedCards.Length - 1)
                    {
                        deletableCardGroups.Add(new List<long> { nextCard });
                    }
                }
            }

            var groupSums = deletableCardGroups.Select(g => g.Sum()).ToArray();
            var maxGroupSum = groupSums.Max();

            var sumAll = groupSums.Sum();
            var remainedSum = sumAll - maxGroupSum;

            Console.WriteLine(remainedSum);
        }
