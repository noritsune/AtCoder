...
namespace KyoPro {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            var isInTramp = Ss.All(S =>
            {
                var firstChars = new[] { 'H', 'D', 'C', 'S' };
                var secondChars = new[] { 'A', '2', '3', '4', '5', '6', '7', '8', '9', 'T', 'J', 'Q', 'K' };
                return firstChars.Contains(S[0]) && secondChars.Contains(S[1]);
            });

            var isNotSameAll = Ss.Distinct().Count() == Ss.Count;

            Console.WriteLine(isInTramp && isNotSameAll ? "Yes" : "No");
        }
