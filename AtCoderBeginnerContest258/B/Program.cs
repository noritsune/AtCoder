...
namespace KyoPro {
    ...
    {
        int _N;
        readonly List<long[]> _grid = new List<long[]>();
        long _moveHistoryMax = long.MinValue;
        ...
        public void Solve2()
        {
            ...
            for (int i = 0; i < _N; i++)
            {
                _grid.Add(Rs().Select(c => long.Parse(c.ToString())).ToArray());
            }
            ...
            for (int y = 0; y < _N; y++)
            {
                for (int x = 0; x < _N; x++)
                {
                    var hasVisitedStart = Enumerable.Range(0, _N).Select(_ => new bool[_N]).ToArray();
                    dfs(0, new Vector2(x, y), 0, hasVisitedStart);
                }
            }
            
            Console.WriteLine(_moveHistoryMax);
        }

        void dfs(int moveCnt, Vector2 pos, long moveHistory, bool[][] hasVisited) 
        {
            if (moveCnt == _N)
            {
                _moveHistoryMax = Math.Max(_moveHistoryMax, moveHistory);
                return;
            }
            ...
            for (int offsetY = -1; offsetY <= 1; offsetY++)
            {
                for (int offsetX = -1; offsetX <= 1; offsetX++)
                {
                    ...
                    if(hasVisited[nextPos.Y][nextPos.X]) continue;
                    
                    var nextNum = _grid[nextPos.Y][nextPos.X];
                    var moveHistoryTmp = moveHistory + nextNum * (long)Math.Pow(10, _N - (moveCnt + 1));
                    if(moveHistoryTmp < _moveHistoryMax) continue;
                    
                    var nextHasVisited = new bool[_N][];
                    for (int y = 0; y < _N; y++)
                    {
                        nextHasVisited[y] = new bool[_N];
                        Array.Copy(hasVisited[y], nextHasVisited[y], _N);
                    }
                    nextHasVisited[nextPos.Y][nextPos.X] = true;

                    dfs(moveCnt + 1, nextPos, moveHistoryTmp, nextHasVisited);
                }
            }
        }
        ...
        public void Solve1()
        {
            ...
            string moveHistoryMax = string.Join("", Enumerable.Repeat('0', N).ToArray());
            for (int i = 0; i < N; i++)
            {
                for (int j = 0; j < N; j++)
                {
                    var hasVisitedStart = Enumerable.Range(0, N).Select(_ => new bool[N]).ToArray();
                    
                    var stack = new Stack<(int moveCnt, Vector2 pos, string moveHistory, bool[][] hasVisited)>();
                    stack.Push((0, new Vector2(j, i), string.Join("", Enumerable.Repeat('0', N).ToArray()), hasVisitedStart));
                    while(stack.Count > 0)
                    {
                        var (moveCnt, pos, moveHistory, hasVisited) = stack.Pop();
                        
                        var currentNum = grid[pos.Y][pos.X];
                        var moveHistoryTmp = moveHistory.ToCharArray();
                        moveHistoryTmp[moveCnt] = currentNum;
                        moveHistory = new string(moveHistoryTmp);
                        
                        moveCnt++;
                        
                        if (moveCnt == N)
                        {
                            if(string.Compare(moveHistory, moveHistoryMax, StringComparison.Ordinal) > 0)
                            {
                                moveHistoryMax = moveHistory;
                            }
                            continue;
                        }
                        
                        hasVisited[pos.Y][pos.X] = true;
                        ...
                        for (int offsetY = -1; offsetY <= 1; offsetY++)
                        {
                            for (int offsetX = -1; offsetX <= 1; offsetX++)
                            {
                                ...
                                var nextNum = grid[nextPos.Y][nextPos.X];
                                var nextMoveHistoryTmp = moveHistory.ToCharArray();
                                nextMoveHistoryTmp[moveCnt] = nextNum;
                                if(string.Compare(new string(nextMoveHistoryTmp), moveHistoryMax, StringComparison.Ordinal) < 0) continue;
                                
                                var nextHasVisited = new bool[N][];
                                for (int y = 0; y < N; y++)
                                {
                                    nextHasVisited[y] = new bool[N];
                                    Array.Copy(hasVisited[y], nextHasVisited[y], N);
                                }
                                nextHasVisited[nextPos.Y][nextPos.X] = true;
                                
                                stack.Push((moveCnt, nextPos, moveHistory, nextHasVisited));
                            }
                        }
                    }
                }
            ...
            Console.WriteLine(moveHistoryMax);
        }
        ...
        public void Solve0()
        {
            ...
            for (int i = 0; i < N; i++)
            {
                grid.Add(Rs().Select(c => long.Parse(c.ToString())).ToArray());
            }
            
            long moveHistoryMax = int.MinValue;
            for (int i = 0; i < N; i++)
            {
                for (int j = 0; j < N; j++)
                {
                    for (int offsetY = -1; offsetY <= 1; offsetY++)
                    {
                        for (int offsetX = -1; offsetX <= 1; offsetX++)
                        {
                            ...
                            long moveHistory = 0;
                            for (int moveCnt = 0; moveCnt < N; moveCnt++)
                            {
                                var currentNum = grid[pos.Y][pos.X];
                                moveHistory += currentNum * (long)Math.Pow(10, N - moveCnt - 1);
                            ...
                            moveHistoryMax = Math.Max(moveHistoryMax, moveHistory);
                        }
                    }
                }
            }
