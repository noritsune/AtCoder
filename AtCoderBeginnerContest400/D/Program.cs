...
public class Solver
{
    ...
    public void Solve()
    {
        ...
        var ABCD = Ria();
        (A, B, C, D) = (ABCD[0] - 1, ABCD[1] - 1, ABCD[2] - 1, ABCD[3] - 1);
        ...
        Console.WriteLine(cost[C, D]);
    }

    void Dfs(int x, int y)
    {
        var currentCost = cost[y, x];

        for (int i = 0; i < 4; i++)
        {
            var ny = y + offsetYs[i]; var nx = x + offsetXs[i];
            if (ny == C && nx == D)
            {
                return;
            }
            ...
            if (grid[ny][nx] == '#')
            {
                ...
                var nny = ny + offsetYs[i]; var nnx = nx + offsetXs[i];
