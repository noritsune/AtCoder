...
class Sol{
	...
	public void Solve2(){
        ...
        int ans = int.MaxValue;
        int[] indexes = Enumerable.Range(0, N - 1).ToArray();
        for (int i = 1; i <= indexes.Length; i++)
        {
            int[][] bordersPatterns = Combination.Enumerate(indexes, i, withRepetition:false).ToArray();
            foreach (int[] borders in bordersPatterns)
            {
                List<int[]> sections = new List<int[]>();
                int startIndex = 0;
                for (int j = 0; j < borders.Length; j++)
                {
                    int count = borders[j] - startIndex + 1;
                    sections.Add(As.GetRange(startIndex, count).ToArray());

                    startIndex = borders[j] + 1;
                }
                sections.Add(As.GetRange(startIndex, As.Count() - startIndex).ToArray());

                List<int> ORs = new List<int>();
                foreach (int[] section in sections)
                {
                    int OR = 0;
                    foreach (int A in section) OR |= A;
                    ORs.Add(OR);
                }
