...
class Sol{
	...
	public void Solve(){
        ...
        for (int i = 0; i < H; i++) Ss[i] = rs();

        int ans = 1;
        ans += CountVisibleMasu(Ss, Y, X, -1,  0);
        ans += CountVisibleMasu(Ss, Y, X,  1,  0);
        ans += CountVisibleMasu(Ss, Y, X,  0, -1);
        ans += CountVisibleMasu(Ss, Y, X,  0,  1);
    ...
    int CountVisibleMasu(string[] Ss, int x, int y, int dirX, int dirY) {
        for (int i = 0; i < 100; i++)
        {
            int newX = x + dirX * (i + 1);
            int newY = y + dirY * (i + 1);
            
            if(newY < 0 || newY >= Ss.Length) return i;
            if(newX < 0 || newX >= Ss[newY].Length) return i;
            if(Ss[newY][newX] == '#') return i;
        }
