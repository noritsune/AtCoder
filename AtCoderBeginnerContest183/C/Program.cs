...
class Sol{
	...
	public void Solve(){
        ...
        int K = NK[1];
        int[][] Tss = new int[N][];
        for (int i = 0; i < N; i++) Tss[i] = ria();

        int ans = 0;
        var patterns = Enumerable.Range(1, N - 1).Perm().Select(x => x.ToArray()).ToArray();
        foreach (int[] pattern in patterns)
        {
            long dist = 0;
            int now = 0;
            bool isOver = false;
            for (int nextIndex = 0; nextIndex < pattern.Count(); nextIndex++)
            {
                int next = pattern[nextIndex];
                dist += Tss[now][next];
