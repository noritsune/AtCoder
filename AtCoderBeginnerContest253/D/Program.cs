...
namespace KyoPro {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            var multipleACnt = N / A;
            var multipleBCnt = N / B;
            var multipleASum = A * multipleACnt * (multipleACnt + 1) / 2;
            var multipleBSum = B * multipleBCnt * (multipleBCnt + 1) / 2;

            var ABLcm = Lcm(A, B);
            var multipleABLcmCnt = N / ABLcm;
            var multipleABLcmSum = ABLcm * multipleABLcmCnt * (multipleABLcmCnt + 1) / 2;
            
            Console.WriteLine(sumAll - multipleASum - multipleBSum + multipleABLcmSum);
        }
