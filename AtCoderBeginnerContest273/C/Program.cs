...
namespace KyoPro {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            var largerCnt = new int[N];
            for (int i = 0; i < As.Length; i++)
            {
                var binarySearch = Array.BinarySearch(sortedAs, As[i]);
                largerCnt[i] = sortedAs.Length - binarySearch - 1;
            }

            var cntDic = largerCnt
                .GroupBy(x => x)
                .ToDictionary(
                    x => x.Key,
                    x => x.Count()
            ...
            for (int i = 0; i < N; i++)
            {
                Console.WriteLine(cntDic.GetValueOrDefault(i));
            }
        }
