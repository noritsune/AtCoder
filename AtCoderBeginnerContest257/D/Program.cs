...
namespace KyoPro {
    ...
    public class Solver {
        ...
        {
            var N = Ri();
            var poss = new Vector2[N];
            var pows = new long[N];
            for (int i = 0; i < N; i++)
            {
                var xyP = Rla();
                poss[i] = new Vector2(xyP[0], xyP[1]);
                pows[i] = xyP[2];
            }

            var sAndUnionPairSets = new SortedDictionary<long, List<(int a, int b)>>();
            for (int aIdx = 0; aIdx < N; aIdx++)
            {
                for (int bIdx = 0; bIdx < N; bIdx++)
                {
                    ...
                    var dist = Math.Abs(aPos.X - bPos.X) + Math.Abs(aPos.Y - bPos.Y);
                    var s = (long)Math.Ceiling(dist / (decimal)aPow);
                    if (!sAndUnionPairSets.ContainsKey(s))
                    {
                        sAndUnionPairSets[s] = new List<(int a, int b)>();
                    }
                    
                    sAndUnionPairSets[s].Add((aIdx, bIdx));
                }
            }
            
            var uf = new UnionFind(N);
            foreach (var (s, aAndB) in sAndUnionPairSets)
            {
                foreach ((int a, int b) in aAndB)
                {
                    uf.Union(a, b);
                }
            ...
            Console.WriteLine(sAndUnionPairSets.Last().Key);
        }
