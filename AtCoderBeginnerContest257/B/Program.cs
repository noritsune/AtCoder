...
namespace KyoPro {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            var K = NKQ[1];
            var Q = NKQ[2];
            var As = Ria().Select(L => L - 1).ToArray();
            var Ls = Ria().Select(L => L - 1).ToArray();
            ...
            for (int i = 0; i < Q; i++)
            {
                ...
                if (poss[L] >= N - 1) continue;
                if (L + 1 < K && poss[L + 1] == poss[L] + 1) continue;
                
                poss[L]++;
            }

            Console.WriteLine(string.Join(" ", poss.Select(pos => pos + 1)));
        }
