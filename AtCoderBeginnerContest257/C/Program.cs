...
namespace KyoPro {
    ...
    internal class IsChildAndWeight
    {
        public readonly bool IsAdult;
        public readonly long Weight;
        
        public IsChildAndWeight(bool isAdult, long weight)
        {
            IsAdult = isAdult;
            Weight = weight;
        }
    ...
    public class Solver {
        public void Solve2()
        {
            ...
            var isAdultAndWeights = new List<IsChildAndWeight>();
            for (int i = 0; i < N; i++)
            {
                isAdultAndWeights.Add(new IsChildAndWeight(S[i] == '1', Ws[i]));
            }
            
            var weightAndAdultCnts = new SortedDictionary<long, long>();
            foreach (var isAdultAndWeight in isAdultAndWeights)
            {
                if (!weightAndAdultCnts.ContainsKey(isAdultAndWeight.Weight))
                {
                    weightAndAdultCnts.Add(isAdultAndWeight.Weight, 0);
                }
                
                weightAndAdultCnts[isAdultAndWeight.Weight] += isAdultAndWeight.IsAdult ? 1 : -1;
            }
            
            long adultCount = isAdultAndWeights.Count(x => x.IsAdult);
            long childCount = isAdultAndWeights.Count(x => !x.IsAdult);
            long currentAns = adultCount;
            long maxAns = Math.Max(adultCount, childCount);
            foreach (var weightAndAdultCnt in weightAndAdultCnts)
            {
                currentAns -= weightAndAdultCnt.Value;
        ...
        public void Solve1()
        {
            ...
            isAdultAndWeights.Sort((a, b) => a.Weight.CompareTo(b.Weight));

            var adultCount = isAdultAndWeights.Count(x => x.IsAdult);
            var currentAns = adultCount;
            var maxAns = 0;
            foreach (var isChildAndWeight in isAdultAndWeights)
            {
                if(isChildAndWeight.IsAdult)
                {
                    currentAns--;
                }
                else
