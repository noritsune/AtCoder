...
public class Solver {
    ...
    {
        var NMK = Rla();
        long N = NMK[0]; long M = NMK[1]; long K = NMK[2];

        long l = 0;
        long r = Math.Max(N, M) * K;
        long mid = -1;
        while (l <= r) // 範囲内にある限り探し続ける
        {
            mid = l + (r - l) / 2;

            var divCntByN = mid / N;
            var divCntByM = mid / M;
            var divCntByNM = mid / Lcm(N, M);
            var divCnt = divCntByN + divCntByM - divCntByNM * 2;
