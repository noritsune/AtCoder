...
public class Solver {
    public void Solve()
    {
        ...
        var zahyoToCnt = new Dictionary<int, int>();
        foreach (var A in As)
        {
            zahyoToCnt.TryAdd(A, 0);
            zahyoToCnt[A]++;
        }

        var zahyos = zahyoToCnt.Keys
            .OrderBy(z => z)
            .ToArray();

        var rui = new int[zahyos.Length + 1];
        for (int i = 0; i < zahyos.Length; i++)
        {
            rui[i + 1] = rui[i] + zahyoToCnt[zahyos[i]];
        }

        var cntMax = 0;
        for (int lIdx = 0; lIdx < zahyos.Length; lIdx++)
        {
            var l = zahyos[lIdx];
            var r = l + M - 1;
            var lb = UpperBound(zahyos, r, Comparer<int>.Default);
            var cnt = rui[lb] - rui[lIdx];
