...
public class Solver
{
    ...
    string _R; string _C;
    char[,] _grid;
    List<char>[] _rows; List<char>[] _cols;
    ...
    public void Solve() {
        ...
        _rows = new List<char>[_N];
        _cols = new List<char>[_N];
        for (int i = 0; i < _N; i++)
        {
            _rows[i] = new List<char>();
            _cols[i] = new List<char>();
        }

        var isExistAns = Dfs(0, 0);
    ...
    bool Dfs(int i, int j)
    {
        if (i == _N)
        {
            return _rows.All(row => row.Count == 3)
                && _cols.All(col => col.Count == 3);
        }

        var row = _rows[i];
        var col = _cols[j];
        var nextI = j < _N - 1 ? i : i + 1;
        var nextJ = j < _N - 1 ? j + 1 : 0;

        for (int k = 0; k < 3; k++)
        {
            var c = ABC[k];
            if (row.Contains(c) || col.Contains(c)) continue;
            if (!row.Any() && _R[i] != c) continue;
            if (!col.Any() && _C[j] != c) continue;
