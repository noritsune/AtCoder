...
public class Solver
{
    ...
    void Dfs(int x, int y, long score)
    {
        ...
        // 置けても置けなくて置かないパターンに派生
        Dfs(x + 1, y, score);

        if (x < W - 1 && !used[y, x] && !used[y, x + 1])
        {
            // 横向きで置くパターン場合に派生
            used[y, x] = true;
            used[y, x + 1] = true;

            var newScore = score ^ grid[y, x + 1] ^ grid[y, x];
            Dfs(x + 2, y, newScore);
        ...
        if (y < H - 1 && !used[y, x] && !used[y + 1, x])
        {
            // 縦向きで置くパターン場合に派生
            used[y, x] = true;
            used[y + 1, x] = true;

            var newScore = score ^ grid[y + 1, x] ^ grid[y, x];
            Dfs(x + 1, y, newScore);
