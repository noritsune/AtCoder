...
public class Solver {
    ...
    {
        var NHW = Ria();
        var N = NHW[0]; var H = NHW[1]; var W = NHW[2];
        ...
        var tileIdxPerms = Enumerable.Range(0, N).Perm().Select(x => x.ToArray()).ToArray();
        var turnTileIdxPats = BitFullSearch(N).ToArray();
        foreach (var tileIdxPerm in tileIdxPerms)
        {
            foreach (var turnTileIdxPat in turnTileIdxPats)
            {
                var isCovered = new bool[H, W];
                var coveredCnt = 0;
                var h = 0; var w = 0;
                for (int i = 0; i < N; i++)
                {
                    var (A, B) = ABs[tileIdxPerm[i]];
                    if (turnTileIdxPat[i]) (A, B) = (B, A);
