...
public class Solver {
    public void Solve()
    {
        ...
        var tests = new (int C, int[] As, char R)[M];
        for (int i = 0; i < M; i++)
        {
            var CAsR = Rsa();
            var C = int.Parse(CAsR[0]);
            var As = CAsR.Skip(1).Take(C).Select(int.Parse).ToArray();
            var R = CAsR.Last()[0];
            tests[i] = (C, As, R);
        }

        var bitFullSearch = BitFullSearch(N);
        long ans = 0;
        foreach (var isTrueKeys in bitFullSearch)
        {
            var isAllTestsCorrect = true;
            foreach (var (C, As, R) in tests)
            {
                var usedTrueKeyCnt = 0;
                foreach (var A in As)
                {
                    if (isTrueKeys[A - 1]) usedTrueKeyCnt++;
                }

                if (R == 'o' && usedTrueKeyCnt < K
                 || R == 'x' && usedTrueKeyCnt >= K)
                {
                    isAllTestsCorrect = false;
                    break;
                }
            }

            if (isAllTestsCorrect) ans++;
        }
