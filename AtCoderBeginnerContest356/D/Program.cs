...
public class Solver {
    ...
    {
        var NM = Rla();
        BigInteger N = NM[0]; BigInteger M = NM[1];
        ...
        for (int i = 0; i < 60; i++)
        {
            ...
            BigInteger pow0 = (BigInteger)Math.Pow(2, i);
            BigInteger pow1 = (BigInteger)Math.Pow(2, i + 1);
            ...
            var rankInGroup = N % pow1 + 1 - pow0;
            rankInGroup %= CONST.MOD;
            var b = rankInGroup < 0 ? 0 : rankInGroup;
            ans += b;
            ans %= CONST.MOD;
        }
