...
class Sol{
	...
	public void Solve(){
        ...
        ruiseki[0] = As[0];
        for (int i = 1; i < N; i++) ruiseki[i] = ruiseki[i - 1] + As[i];
        
        long[] ruisekiMax = new long[N];
        ruisekiMax[0] = ruiseki[0];
        for (int i = 1; i < N; i++) {
            ruisekiMax[i] = Math.Max(ruisekiMax[i - 1], ruiseki[i]);
        }
        ...
        for (int i = 0; i < N; i++)
        {
            ans = Math.Max(ans, now + ruisekiMax[i]);
