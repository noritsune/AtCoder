...
class Sol{
	...
	public void Solve(){
        ...
        double iLimit = Math.Sqrt(N);
        HashSet<double> powerNums = new HashSet<double>();
        for (int i = 2; i <= iLimit; i++)
        {
            int nTh = 2;
            double powerNum = Math.Pow(i, nTh);
            while (powerNum <= N)
            {
                powerNums.Add(powerNum);

                powerNum = Math.Pow(i, nTh);
                nTh++;
            }
        }
		
		Console.WriteLine(N - powerNums.Count());
		Console.ReadLine();
	}
