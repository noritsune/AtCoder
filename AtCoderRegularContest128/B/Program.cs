...
namespace AtCoder {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            var colors = generateSuretsu(3);
            foreach (var case_ in cases)
            {
                Array.Sort(case_);

                if (case_[1] == case_[0] || case_[1] == case_[2])
                {
                    Console.WriteLine(case_[1]);
                    continue;
                }

                double minusCntDouble = (case_[2] - case_[0]) / 3.0;
                ...
                case_[2] -= minusCnt;
                case_[0] += minusCnt * 2;
                if (case_[1] < 0 || case_[2] < 0)
                {
                    Console.WriteLine(-1);
                    continue;
                }

                if (case_[0] == case_[1] || case_[0] == case_[2])
                {
                    Console.WriteLine(minusCnt + case_[0]);
                    continue;
                }
        ...
        public void Solve1()
        {
            ...
            foreach (var @case in cases)
            {
                bool canDoIt = false;
                Array.Sort(@case);
                for (int addColor = 0; addColor < 3; addColor++)
                {
                    var minusColors = new List<int>(colors);
                    minusColors.Remove(addColor);
                    
                    int minusCol1 = minusColors[0];
                    int minusCol2 = minusColors[1];
                    
                    if (@case[minusCol1] == @case[minusCol2])
                    {
                        Console.WriteLine(@case[minusCol1]);
                        canDoIt = true;
                        break;
                    }

                    int minusCnt = (@case[minusCol2] - @case[addColor]) / 3;
                    int addColorCnt = @case[addColor] + minusCnt * 2;
                    int minusColor2Cnt = @case[minusCol2] - minusCnt;
                    if(addColorCnt != minusColor2Cnt) continue;
                    
                    Console.WriteLine(@case[minusCol2]);
                    canDoIt = true;
                    break;
                }
