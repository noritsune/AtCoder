...
namespace KyoPro {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            for (int i = 0; i < M; i++) Ts.Add(Rs());

            var TsSet = new HashSet<string>(Ts);

            var last3CharsInSs = Ss.Select(s => s.Substring(s.Length - 3)).ToList();
            var cnt = last3CharsInSs.Count(s => TsSet.Contains(s));
