...
namespace KyoPro {
    ...
    public class Solver {
        public void Solve()
        {
            ...
            for (int i = 1; i <= N; i++)
            {
                var deg = graph.Vertices[i].Count;
                if (deg == 1)
                {
                    terminals.Add(i);
                }
            ...
            var start = terminals.First();
            var end = terminals.Last();
            ...
            while (queue.Count > 0)
            {
                ...
                if (now == end)
                {
                    ...
                    {
                        var vMe = vList[i];
                        var verts = graph.Vertices[vMe];

                        var nextVs = new HashSet<int>();
                        if (i > 0) nextVs.Add(vList[i - 1]);
                        ...
                        {
                            if(nextVs.Contains(vert)) continue;
                ...
                var nextVertices = graph.Vertices[now]
